'use strict';
var observa = "";
app.reporteCCl = kendo.observable({
    onShow: function () {
        try {
            var dataItemDoc = JSON.parse(localStorage.getItem("dataItemCCA"));
            localStorage.removeItem("griddataCCL");
            $("#NoOrdenCCL").text("Número de OT:  " + dataItemDoc.numero_orden);
            document.getElementById("tabstripCCL").style.display = "none";
            document.getElementById("btnFooterCCL").innerHTML = "<button id='btnEstadoCCL' onclick='cabiaestadoCCL();' class='w3-btn w3-red'><i class='fa-caret-square-o-down' aria-hidden='true'></i>CAMBIAR DE ESTADO</button>" +
                "<button id='btnGuadaCLC' onclick='guardarCCJT();' class='w3-btn w3-red'><i class='fa fa-pencil' aria-hidden='true'></i> GUARDAR</button>";
            var arrLi = ["liVideoRCCL","liDatosRCCL","liActividadesRCCL", "liDetalleRCCL","liFirmaASRCCL"];
            var idLi = 1;
            for (var x = 0; x < arrLi.length; x++) {
                document.getElementById(arrLi[x]).removeAttribute("style");
                if (parseInt(localStorage.getItem("ls_dimensionW").toLocaleString()) < 361) {
                    document.getElementById("l" + idLi).style.fontSize = "12px";
                    idLi++;
                }
            }    
            ConsultaControlCalidadL(dataItemDoc);
            datosotCCA(dataItemDoc);
            infoDetalleOTCC(dataItemDoc);
            actividadesOTCCA(dataItemDoc);
            //<button onclick='imprime();' class='w3-btn w3-red'><i class='fa fa-print' aria-hidden='true'></i> IMPRIMIR</button>
            // "<button id='btnGuadaCC' onclick='guardarCC();' class='w3-btn w3-red'><i class='fa fa-pencil' aria-hidden='true'></i> GUARDAR</button>"+ "  " + confirmFirmaOTCCLA

            // FIRMA DIGITAL ASESOR>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            document.getElementById('confirmFirmaOTCCLJT').addEventListener('click', function () {
                firmaDigitalOTCCJT();
            });
            document.getElementById('cancelFirmaOTCCLJT').addEventListener('click', function () {
                document.getElementById("firmaOTCCLJT").value = "";
                document.getElementById("imgFirmaOTCCLJT").innerHTML = "";
            });
            //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><

        } catch (error) {
            alert("1" + error);
        }

    },
    afterShow: function () {
    }
});
app.localization.registerView('reporteCCl');

function actividadesOTCCA(dataItem) {
    var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/ga/Garantias.svc/ga64RevisionGet/1," + localStorage.getItem("ls_idempresa").toLocaleString() + ";" + dataItem.tipo_mantenimiento + ";";
    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", Url);
    var infor;
    $.ajax({
        url: Url,
        type: "GET",
        async: false,
        dataType: "json",
        success: function (data) {
            try {
                infor = (JSON.parse(data.ga64RevisionGetResult));
            } catch (e) {
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>38 ERROR</center>", e);
                return;
            }
        },
        error: function (err) {
            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>39 ERROR</center>", err);
            return;
        }
    });
    document.getElementById("actividadesOTCCA").innerHTML = "";
    try {
        kendo.ui.progress($("#reporteCCScreen"), true);
        setTimeout(function () {
            var htmlActividad = "&nbsp;&nbsp;&nbsp;&nbsp;<table width='100%' cellpadding='0' cellspacing='0' border='0px'>" +
                "<tr style='border:1px solid'><td><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;CODIGO:&nbsp;&nbsp;</label></td>" +
                "<td><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;ACTIVIDAD:&nbsp;&nbsp;</label></td></tr>";
            for (let index = 0; index < infor.length; index++) {
                htmlActividad += "<tr style='border-bottom:1px solid'><td style='width: 25%;'>" +
                    "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + infor[index].CodigoClase + "&nbsp;&nbsp;</label>" +
                    "</td><td style='width: 100%;'><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + infor[index].NombreClase + "&nbsp;&nbsp;</label></td></tr>";
            }
            document.getElementById("actividadesOTCCA").innerHTML = htmlActividad + "</table>";
        }, 1000);
    } catch (error) {
        alert(error);
    }

}

function infoDetalleOTCC(dataItem) {
    kendo.ui.progress($("#reporteCCScreen"), true);
    setTimeout(function () {
        // precarga ***********************

        var descri = (parseInt(localStorage.getItem("ls_dimensionW").toLocaleString()) * 40) / 100;
        var cant = (parseInt(localStorage.getItem("ls_dimensionW").toLocaleString()) * 7) / 100;
        var cla = (parseInt(localStorage.getItem("ls_dimensionW").toLocaleString()) * 15) / 100;

        var urlDot = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl07DetalleOTGet/" + dataItem.codigo_empresa + "," + dataItem.anio + "," + dataItem.secuencia_orden + ",," + localStorage.getItem("ls_ussucursal").toLocaleString() + "," + localStorage.getItem("ls_usagencia").toLocaleString();
        var UrlDetalleOT = urlDot;

        /* window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", UrlDetalleOT); */

        var infordetEVCC;
        $.ajax({
            url: UrlDetalleOT,
            type: "GET",
            async: false,
            dataType: "json",
            success: function (data) {
                try {
                    if (inspeccionar(data).includes("DetalleOT01") == true) {
                        infordetEVCC = (JSON.parse(data.tl07DetalleOTGetResult)).DetalleOT01;

                        if (inspeccionar(infordetEVCC).length > 0) {
                            $("#imgFirmaOTCCA").kendoGrid({
                                dataSource: {
                                    pageSize: 20,
                                    data: infordetEVCC,
                                    aggregate: [
                                        {field: "Descripcion", aggregate: "count", width: descri},
                                        {field: "Cantidad", aggregate: "sum", width: cant},
                                        {field: "Total", aggregate: "sum", width: cant}
                                    ]
                                },
                                pageable: false,
                                columns: [
                                    {
                                        field: "Descripcion",
                                        title: "Descripci&oacute;n",
                                        footerTemplate: "TOTAL:",
                                        width: descri
                                    },
                                    {field: "clase_registro", title: "Clase Registro", width: cla},
                                    {field: "Cantidad", title: "Cantidad", width: cant},
                                    {
                                        field: "Total",
                                        format: "{0:c}",
                                        title: "Total",
                                        footerTemplate: "#= kendo.toString(sum, '$0.00') #",
                                        width: cant
                                    }
                                ]
                            });

                            kendo.ui.progress($("#reporteCCScreen"), false);
                        } else {
                            kendo.ui.progress($("#reporteCCScreen"), false);
                            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>1 ALERTA</center>", "<center>No existe el detalle de la<br/>Orden de Trabajo <b>" + dataItem.numero_orden + "</b></center>");
                            //   kendo.mobile.application.navigate("components/admOt/view.html");
                        }

                    } else {
                        kendo.ui.progress($("#reporteCCScreen"), false);
                        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "<center>No existe el detalle de la<br/>Orden de Trabajo <b>" + dataItem.numero_orden + "</b></center>");
                        //    kendo.mobile.application.navigate("components/admOt/view.html");
                    }

                } catch (e) {
                    kendo.ui.progress($("#reporteCCScreen"), false);
                    // window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "No existe el detalle de la Orden de Trabajo");
                    return;
                }
            },
            error: function (err) {
                kendo.ui.progress($("#reporteCCScreen"), false);
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", err);
                return;
            }
        });

        // precarga ***********************
    }, 1000);

}

/*--------------------------------------------------------------------
Fecha: 15/02/2019
Descripcion: Presenta datos de la OT
Parametros:
--------------------------------------------------------------------*/
function datosotCCA(dataItem) {
    document.getElementById("datosOT").innerHTML = "";
    try {

        kendo.ui.progress($("#reporteCCScreen"), true);
        setTimeout(function () {
            var kilm1 = new Intl.NumberFormat("en-IN", {maximumSignificantDigits: 3}).format(dataItem.kilometraje);
            document.getElementById("datosOT").innerHTML = "&nbsp;&nbsp;&nbsp;&nbsp;<table width='100%' cellpadding='0' cellspacing='0' border='0px'>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;OT:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.numero_orden + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;NOMBRE DEL CLEINTE:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.nombre_cliente + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;TELEFONO:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.telefono_cliente + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;CELULAR:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.celular_cliente + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;FECHA:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.fecha_recepcion + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;VEHICULO:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.nombre_modelo + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;SECCION:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.seccion_orden_trabajo + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;TIPO DE TRABAJO:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.tipo_trabajo + " - " + dataItem.nombre_tipo_trabajo + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;TIPO DE MANTENIMIENTO:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.tipo_mantenimiento + " - " + dataItem.nombre_mantenimiento + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;KILOMETRAJE:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + kilm1 + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;REPARACIONES SOLICITADAS:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.reparaciones_solicitadas + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;OBSERVACIONES:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.observaciones + "&nbsp;&nbsp;</label>" +
                "</td></tr></table>";
        }, 1000);
    } catch (error) {
        alert(error);
    }

}


function cabiaestadoCCL() {
    try {
        var validaCC = validaTablaCCL();
        var validCC = validaCC.split(';');
        if (validCC[0] == 1) {
            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "Pregunta sin resposnter: " + validCC[1]);
            return;
        }
        /* if(document.getElementById("titFirmaPregCCA").innerText.trim() == "Desea registrar la firma del asesor ?"){
            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "Debe Registrar Firma Asesor");
            return;
        }

        document.getElementById("titFirmaPregCCA").innerHTML = "Desea registrar la firma del asesor ?<br />";*/
        var dataRespuesta = JSON.parse(localStorage.getItem("griddataCCL"));
        var para = [];
        para[0] = {
            "codigo_empresa": dataRespuesta[0].codigo_empresa,
            "codigo_sucursal": dataRespuesta[0].codigo_sucursal,
            "codigo_taller": dataRespuesta[0].codigo_taller,
            "anio_tl06": dataRespuesta[0].anio_tl06,
            "secuencia_orden_tl06": dataRespuesta[0].secuencia_orden_tl06,
            "secuencia_tl43": "",
            "orden_presentacion": "REVISADO",
            "pregunta": "",
            "tipo_formulario": "",
            "seccion_formulario": "",
            "tipo_respuesta": "",
            "lista_respuesta": "",
            "lista_etiquetas": "",
            "tipo_valor": "",
            "lista_valores": "",
            "respuesta": "",
            "secuencia_vh65": "",
            "corden_seccion": "",
            "selRespt": "",
            "sevRespt": "",
            "erRespt": "",
            "evRespt": "",
            "nombre_seccion": "",
            "observacion_calidad": "",
            "observacion_calidad_cs": "",
            "nombre_cliente_csi": "",
            "telefono_fijo_csi": "",
            "telefono_movil_csi": ""
        };

        var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl43PreguntasCalidadSet";
        //var Url = "http://localhost:4044" + "/Services/TL/Taller.svc/tl43PreguntasCalidadSet";
        $.ajax({
            url: Url,
            type: "POST",
            data: JSON.stringify(para),
            async: false,
            dataType: "json",
            //Content-Type: application/json
            headers: {
                'Content-Type': 'application/json;charset=UTF-8'
            },
            success: function (datas) {
                //alert(datas);
                if (datas.substr(0, 1) == "1") {
                    alert("se actualizaron los datos");
                    desactivaTablaCCL(true);
                    document.getElementById("btnEstadoCCL").disabled = true;
                    document.getElementById("btnGuadaCLC").disabled = true;
                } else {
                    alert(datas.substr(2, datas.length - 2));
                    return;
                }
            },
            error: function (err) {
                alert("HOY" + inspeccionar(err));
                alert("Error en servicio clientes");
                return;
            } //alert(err);
        });

    } catch (e) {
        alert("salio" + e);
    }
}

function validaTablaCCL() {
    var dataRespuesta = JSON.parse(localStorage.getItem("griddataCCL"));
    try {
        var bandera = 0;
        var pregunta = "";
        for (var i = 0; i <= dataRespuesta.length - 1; i++) {
            if (dataRespuesta[i].tipo_respuesta == "SELECCION" || dataRespuesta[i].tipo_respuesta == "SEV") {
                if (dataRespuesta[i].tipo_respuesta == "SEV") {
                    for (var l = 0; l < dataRespuesta[i].sevResp.length; l++) {
                        if (document.getElementById("SEV" + i + "" + l) == null) {
                            bandera = 1;
                            pregunta = dataRespuesta[i].pregunta.toLocaleString();
                            return bandera + ";" + pregunta;
                        }
                    }
                }
                if (document.getElementById("SEL" + i).value == "Seleccione") {
                    bandera = 1;
                    pregunta = dataRespuesta[i].pregunta.toLocaleString();
                    return bandera + ";" + pregunta;
                }

            }
            if (dataRespuesta[i].tipo_respuesta == "ER") {
                for (var k = 0; k < dataRespuesta[i].erResp.length; k++) {
                    if (document.getElementById("ER" + i + "" + k) == null) {
                        bandera = 1;
                        pregunta = dataRespuesta[i].pregunta.toLocaleString();
                        return bandera + ";" + pregunta;
                    }
                }
                if (document.getElementById("SEL" + i).value == "Seleccione") {
                    bandera = 1;
                    pregunta = dataRespuesta[i].pregunta.toLocaleString();
                    return bandera + ";" + pregunta;
                }

            }
            if (dataRespuesta[i].tipo_respuesta == "EV") {

                for (var m = 0; m < dataRespuesta[i].evResp.length; m++) {
                    if (document.getElementById("EV" + i + "" + m) == null) {
                        bandera = 1;
                        pregunta = dataRespuesta[i].pregunta.toLocaleString();
                        return bandera + ";" + pregunta;
                    }
                }
            }

        }
        return bandera + ";" + pregunta
    } catch (e) {
        alert("dis" + e);
    }
}

function desactivaTablaCCL(sn) {
    var dataRespuesta = "";
    dataRespuesta = JSON.parse(localStorage.getItem("griddataCCL"));
    try {
        for (var i = 0; i <= dataRespuesta.length - 1; i++) {
            if (dataRespuesta[i].tipo_respuesta == "SELECCION" || dataRespuesta[i].tipo_respuesta == "SEV") {
                if (dataRespuesta[i].tipo_respuesta == "SEV") {
                    for (var l = 0; l < dataRespuesta[i].sevResp.length; l++) {
                        if (document.getElementById("SEV" + i + "" + l) != null) {
                            document.getElementById("SEV" + i + "" + l).disabled = sn;
                        }
                    }
                }
                document.getElementById("SEL" + i).disabled = sn;

            }
            if (dataRespuesta[i].tipo_respuesta == "ER") {
                for (var k = 0; k < dataRespuesta[i].erResp.length; k++) {
                    if (document.getElementById("ER" + i + "" + k) != null) {
                        document.getElementById("ER" + i + "" + k).disabled = sn;
                    }
                }
                document.getElementById("SEL" + i).disabled = sn;

            }
            if (dataRespuesta[i].tipo_respuesta == "EV") {

                for (var m = 0; m < dataRespuesta[i].evResp.length; m++) {
                    if (document.getElementById("EV" + i + "" + m) != null) {
                        document.getElementById("EV" + i + "" + m).disabled = sn;
                    }
                }
            }

        }
        document.getElementById("observacionCCL").disabled = sn;
        //document.getElementById("btnEstadoCCL").disabled = sn;
        //document.getElementById("btnGuadaCLC").disabled = sn;
    } catch (e) {
        alert("dis" + e);
    }
}


/*--------------------------------------------------------------------
Fecha: 06/09/2018
Descripcion: Presenta dialog para ingresar la firma electronica
--------------------------------------------------------------------*/
function firmaDigitalOTCC() {
    // OT - FIRMA
    try {
        var dialogfirmaDigitalOTCC = $("#dialogfirmaDigitalOTJT").data("kendoDialog");
        dialogfirmaDigitalOTCC.close();
    } catch (ed010) {
    }
    var htmlFirmaOTCC = "<table align='center' width='100%' style='border-bottom:1px solid'>" +
        "<tr>" +
        "<td>" +
        "<div id='idwrapOTCC' class='wrapper'><canvas id='signature-padCC' width='400' height='300'></canvas></div>" +
        "</td>" +
        "</tr>" +
        "</table>";

    dialogfirmaDigitalOTCC = $("#dialogfirmaDigitalOTJT").kendoDialog({
        width: "450px",
        buttonLayout: "normal",
        title: "INGRESE LA FIRMA",
        closable: false,
        modal: false,
        content: htmlFirmaOTCC,
        actions: [
            {
                text: '<font style=\"font-size:12px\"> <button id=\"save-jpeg\" class=\"w3-btn w3-red\"> &nbsp;&nbsp;CONFIRMAR&nbsp;&nbsp;</button></font>',
                action: accFirmConfCC
            },
            {
                text: '<font style=\"font-size:12px\"><button id=\"clear\" class=\"w3-btn w3-red\"> CANCELAR</button></font>',
                primary: true
            }
        ]
    });
    dialogfirmaDigitalOTCC.data("kendoDialog").open();
    canvas = document.getElementById('signature-padCC');
    signaturePad = new SignaturePad(canvas, {
        minWidth: 0.5,
        maxWidth: 0.5,
        dotSize: 1,
        backgroundColor: 'rgb(255, 255, 255)'
    });
}

/*--------------------------------------------------------------------
Fecha: 06/09/2018
Descripcion: Presenta dialog para ingresar la firma electronica
--------------------------------------------------------------------*/
function firmaDigitalOTCCJT() {

    // OT - FIRMA
    try {
        var dialogfirmaOTJT = $("#dialogfirmaOTJT").data("kendoDialog");
        dialogfirmaOTJT.close();
    } catch (e) {
    }
    var htmlFirmaOTJT = "<table align='center' width='100%' style='border-bottom:1px solid'>" +
        "<tr>" +
        "<td>" +
        "<div id='idwrapOTJT' class='wrapper'><canvas id='signature-padCCA' width='400' height='300'></canvas></div>" +
        "</td>" +
        "</tr>" +
        "</table>";
    dialogfirmaOTJT = $("#dialogfirmaOTJT").kendoDialog({
        width: "450px",
        buttonLayout: "normal",
        title: "INGRESE LA FIRMA JEFE DE TALLER",
        closable: false,
        modal: false,
        content: htmlFirmaOTJT,
        actions: [
            {
                text: '<font style=\"font-size:12px\"> <button id=\"save-jpeg\" class=\"w3-btn w3-red\"> &nbsp;&nbsp;CONFIRMAR&nbsp;&nbsp;</button></font>',
                action: accFirmConfJT
            },
            {
                text: '<font style=\"font-size:12px\"><button id=\"clear\" class=\"w3-btn w3-red\"> CANCELAR</button></font>',
                primary: true
            }
        ]
    });

    dialogfirmaOTJT.data("kendoDialog").open();
    canvas = document.getElementById('signature-padCCA');
    signaturePad = new SignaturePad(canvas, {
        minWidth: 0.5,
        maxWidth: 0.5,
        dotSize: 1,
        backgroundColor: 'rgb(255, 255, 255)'
    });
}

/*--------------------------------------------------------------------
Fecha: 06/09/2018
Descripcion: Registra la firma en una imagen
--------------------------------------------------------------------*/
function accFirmConfJT() {
    if (signaturePad.isEmpty()) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "<center>Ingrese la firma correspondiente</center>");
        return;
    }

    document.getElementById("firmaOTCCLJT").value = "";
    document.getElementById("imgFirmaOTCCLJT").innerHTML = "";

    var data = signaturePad.toDataURL('image/jpeg');
    document.getElementById("firmaOTCCLJT").value = data;

    signaturePad.clear();

    // OT - FIRMA
    try {
        var dialogfirmaOTAS = $("#dialogfirmaOTJT").data("kendoDialog");
        dialogfirmaOTAS.close();
    } catch (ed010) {
    }

    document.getElementById("imgFirmaOTCCLJT").innerHTML = "<img src='" + document.getElementById("firmaOTCCLJT").value + "' width='400' height='300'>";
}

/*--------------------------------------------------------------------
Fecha: 06/09/2018
Descripcion: Registra la firma en una imagen
--------------------------------------------------------------------*/
function accFirmConfCC() {
    if (signaturePad.isEmpty()) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "<center>Ingrese la firma correspondiente</center>");
        return;
    }

    document.getElementById("firmaOTCC").value = "";
    document.getElementById("imgFirmaOTCC").innerHTML = "";

    var data = signaturePad.toDataURL('image/jpeg');
    document.getElementById("firmaOTCC").value = data;

    signaturePad.clear();

    // OT - FIRMA
    try {
        var dialogfirmaDigitalOTCC = $("#dialogfirmaDigitalOTCC").data("kendoDialog");
        dialogfirmaDigitalOTCC.close();
    } catch (ed010) {
    }

    document.getElementById("imgFirmaOTCC").innerHTML = "<img src='" + document.getElementById("firmaOTCC").value + "' width='400' height='300'>";
}

function imprime() {
    formatoImpConCal(false);
    //cargadoc(JSON.parse(localStorage.getItem("dataItem")));fa fa-print
}

function guardarCCJT() {

    generarOrdenPDFCCL(JSON.parse(localStorage.getItem("dataItemCCA")).numero_orden, "O");
    /* var dataRespuesta = JSON.parse(localStorage.getItem("griddataCCL"));
       try {
        for (var i = 0; i < dataRespuesta.length; i++) {
            var sevRespts = [];
            var erRespts = [];
            var evRespts = [];
            if (dataRespuesta[i].tipo_respuesta == "SELECCION" || dataRespuesta[i].tipo_respuesta == "SEV") {
                if (dataRespuesta[i].tipo_respuesta == "SEV") {

                    for (var l = 0; l < dataRespuesta[i].sevResp.length; l++) {
                        if (document.getElementById("SEV" + i + "" + l) != null) {
                            //dataRespuesta[i].sevResp[l] = document.getElementById("SEV" + i + "" + l).value.toLocaleString();
                            sevRespts[l] = document.getElementById("SEV" + i + "" + l).value.toLocaleString();
                            //alert(i + "sev " + inspeccionar(dataRespuesta[i].sevResp[l]));
                        }
                    }
                }
                dataRespuesta[i].selRespt = document.getElementById("SEL" + i).value.toLocaleString();
            }
            if (dataRespuesta[i].tipo_respuesta == "ER") {
                for (var k = 0; k < dataRespuesta[i].erResp.length; k++) {
                    if (document.getElementById("ER" + i + "" + k) != null) {
                        //dataRespuesta[i].erResp[l].erR = document.getElementById("ER" + i + "" + l).value;
                        erRespts[k] = document.getElementById("ER" + i + "" + k).value;
                    }
                    //alert(inspeccionar(erRespts));
                }
                dataRespuesta[i].selRespt = document.getElementById("SEL" + i).value.toLocaleString();
            }
            if (dataRespuesta[i].tipo_respuesta == "EV") {
                //var sevUsa = "<td><table style='width: 100%;'><tr>";
                for (var m = 0; m < dataRespuesta[i].evResp.length; m++) {
                    if (document.getElementById("EV" + i + "" + m) != null) {
                        //dataRespuesta[i].evResp[l].evR = document.getElementById("EV" + i + "" + l).value;
                        evRespts[m] = document.getElementById("EV" + i + "" + m).value;
                    }
                }
            }
            dataRespuesta[i].sevRespt = sevRespts;
            dataRespuesta[i].erRespt = erRespts;
            dataRespuesta[i].evRespt = evRespts
        }

        //alert("termino");
        var para = [];
        if (dataRespuesta.length > 0) {
            try {
                for (var i = 0; i < dataRespuesta.length; i++) {
                    var sevRespuesta = dataRespuesta[i].sevRespt;
                    var erRespuesta = dataRespuesta[i].erRespt;
                    var evRespuesta = dataRespuesta[i].evRespt;
                    var sevRespuestaS = "";
                    if (sevRespuesta != null) {
                        for (var j = 0; j < sevRespuesta.length; j++) {
                            sevRespuestaS += sevRespuesta[j];
                            sevRespuestaS += ",";
                        }
                    }
                    var erRespuestaS = "";
                    if (erRespuesta != null) {
                        for (var j = 0; j < erRespuesta.length; j++) {
                            erRespuestaS += erRespuesta[j];
                            erRespuestaS += ",";
                        }
                    }

                    var evRespuestaS = "";
                    if (evRespuesta != null) {
                        for (var j = 0; j < evRespuesta.length; j++) {
                            evRespuestaS += evRespuesta[j];
                            evRespuestaS += ",";
                        }
                    }

                    para[i] = {
                        "codigo_empresa": dataRespuesta[i].codigo_empresa,
                        "codigo_sucursal": dataRespuesta[i].codigo_sucursal,
                        "codigo_taller": dataRespuesta[i].codigo_taller,
                        "anio_tl06": dataRespuesta[i].anio_tl06,
                        "secuencia_orden_tl06": dataRespuesta[i].secuencia_orden_tl06,
                        "secuencia_tl43": dataRespuesta[i].secuencia_tl43,
                        "orden_presentacion": dataRespuesta[i].orden_presentacion,
                        "pregunta": dataRespuesta[i].pregunta,
                        "tipo_formulario": dataRespuesta[i].tipo_formulario,
                        "seccion_formulario": dataRespuesta[i].seccion_formulario,
                        "tipo_respuesta": dataRespuesta[i].tipo_respuesta,
                        "lista_respuesta": dataRespuesta[i].lista_respuesta,
                        "lista_etiquetas": dataRespuesta[i].lista_etiquetas,
                        "tipo_valor": dataRespuesta[i].tipo_valor,
                        "lista_valores": dataRespuesta[i].lista_valores,
                        "respuesta": dataRespuesta[i].respuesta,
                        "secuencia_vh65": dataRespuesta[i].secuencia_vh65,
                        "corden_seccion": dataRespuesta[i].corden_seccion,
                        "selRespt": dataRespuesta[i].selRespt,
                        "sevRespt": sevRespuestaS,
                        "erRespt": erRespuestaS,
                        "evRespt": evRespuestaS,
                        "nombre_seccion": dataRespuesta[i].nombre_seccion,
                        "observacion_calidad": document.getElementById("observacionCCL").value
                    };
                    //alert(inspeccionar(para[i]));
                }

            } catch (e) {
                alert(e);
            }

            var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl43PreguntasCalidadSet";
            //var Url = "http://localhost:4044" + "/Services/TL/Taller.svc/tl43PreguntasCalidadSet";
            $.ajax({
                url: Url,
                type: "POST",
                data: JSON.stringify(para),
                async: false,
                dataType: "json",
                //Content-Type: application/json
                headers: {
                    'Content-Type': 'application/json;charset=UTF-8'
                },
                success: function (datas) {
                    //alert(datas);
                    if (datas.substr(0, 1) == "1") {
                        alert("se actualizaron los datos");
                        ConsultaControlCalidadL(JSON.parse(localStorage.getItem("dataItemCCA")));
                        //vaciaCamposUsa();
                        //return;
                    } else { alert(datas.substr(2, datas.length - 2)); return; }
                },
                error: function (err) { alert("HOY"+inspeccionar(err)); alert("Error en servicio clientes"); return; } //alert(err);
            });
        }
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(inforUsa));

    } catch (e) { alert("salio"+e); }*/
}

function ConsultaControlCalidadL(inforusa) {
    try {
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(inforusa));
        var tabstripCCL = $("#tabstripCCL").kendoTabStrip().data("kendoTabStrip");
        tabstripCCL.select(0);
        var respOT = "0";
        var tipo_doc = "CONTROL_CALIDAD";
        if (inforusa.chasis.substr(7, 1) == "E") {
            tipo_doc = "CONTROL_CALIDAD_ELEC"
        }
        var UrlMotorEscape = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl43PreguntasCalidadGet/1,json;" +
            inforusa.codigo_empresa + ";" + inforusa.codigo_sucursal + ";" + inforusa.codigo_taller + ";" + inforusa.anio + ";" +
            inforusa.secuencia_orden + ";" + tipo_doc + ";;;;;;";
        var infME;
        /* document.getElementById("firmaGrafCCL").value = "";
        document.getElementById("imgFirmaOTCCL").innerHTML = "";

        document.getElementById("confirmFirmaOTCCL").style.display = 'initial';
        document.getElementById("cancelFirmaOTCCL").style.display = 'initial'; */

        document.getElementById("firmaGrafCCLJT").value = "";
        document.getElementById("imgFirmaOTCCLJT").innerHTML = "";

        document.getElementById("confirmFirmaOTCCLJT").style.display = 'initial';
        document.getElementById("cancelFirmaOTCCLJT").style.display = 'initial';
        $.ajax({
            url: UrlMotorEscape,
            type: "GET",
            async: false,
            dataType: "json",
            success: function (data) {

                try {
                    infME = (JSON.parse(data.tl43PreguntasCalidadGetResult)).ttl43;   //(JSON.parse(data.ComboParametroEmpGetResult));
                    /* window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>0 ALERTA</center>", inspeccionar(infME[0])); */
                    localStorage.setItem("inforImprime", JSON.stringify(infME));
                    respOT = "1";
                } catch (e) {
                    return;
                }
            },
            error: function (err) {
                alert(err);
                return;
            }
        });

        var obsrvaCC = infME[0].observacion_calidad;
        if (respOT == "1") {
            try {
                var griddata = [];
                tamano = infME.length;
                for (var t = 0; t < infME.length; t++) {
                    if (infME[t].orden_presentacion < 10) {
                        infME[t].orden_presentacion = "0" + infME[t].orden_presentacion;
                    }
                }

                infME.sort(function myfunction(a, b) {
                    return (a.corden_seccion + "" + a.orden_presentacion) - (b.corden_seccion + "" + b.orden_presentacion)
                });
                for (var i = 0; i < infME.length; i++) {
                    var selecResp = new Array();
                    var sevResp = new Array();
                    var erResp = new Array();
                    var evResp = new Array();
                    //alert(infME[i].tipo_respuesta);
                    if (infME[i].tipo_respuesta == "SELECCION" || infME[i].tipo_respuesta == "SEV") {
                        if (infME[i].tipo_respuesta == "SEV") {
                            //alert("etiqu"+infME[i].lista_etiquetas);
                            sevResp = infME[i].lista_etiquetas.split(",");
                            //alert(inspeccionar(sevResp));
                            var sevRes = [];
                            for (var k = 0; k < sevResp.length; k++) {
                                sevRes.push({sevR: sevResp[k]});
                                //alert(inspeccionar(sevRes[k]));
                            }
                            //alert(inspeccionar(sevRes[0]));
                        }
                        //alert("resp"+infME[i].lista_respuesta)
                        selecResp = infME[i].lista_respuesta.split(",");
                        var selRes = [];
                        selRes.push({sele: "Seleccione"});
                        for (var j = 0; j < selecResp.length; j++) {
                            selRes.push({sele: selecResp[j]});
                        }
                    } else {
                        if (infME[i].tipo_respuesta == "ER") {
                            //alert("else eti" + infME[i].lista_etiquetas);
                            erResp = infME[i].lista_etiquetas.split(",");
                            var erRes = [];
                            for (var l = 0; l < erResp.length; l++) {
                                erRes.push({erR: erResp[l]});
                            }
                            //alert("else resp" + infME[i].lista_respuesta);
                            selecResp = infME[i].lista_respuesta.split(",");
                            var selRes = [];
                            selRes.push({sele: "Seleccione"});
                            for (var j = 0; j < selecResp.length; j++) {
                                selRes.push({sele: selecResp[j]});
                            }
                        } else {
                            if (infME[i].tipo_respuesta == "EV") {
                                //alert("else ev"+infME[i].lista_etiquetas);
                                evResp = infME[i].lista_etiquetas.split(",");
                                //alert((evResp.length));
                                var evRes = [];
                                for (var hh = 0; hh < evResp.length; hh++) {
                                    evRes.push({evR: evResp[hh]});
                                }
                                //alert("0:" + evRes[0].evR);
                                //alert("1:" + evRes[1].evR);
                                //alert("2:" + evRes[2].evR);
                                //alert("3:" + evRes[3].evR);
                            }
                        }
                    }

                    griddata.push({
                        codigo_empresa: infME[i].codigo_empresa,
                        codigo_sucursal: infME[i].codigo_sucursal,
                        codigo_taller: infME[i].codigo_taller,
                        anio_tl06: infME[i].anio_tl06,
                        secuencia_orden_tl06: infME[i].secuencia_orden_tl06,
                        secuencia_tl43: infME[i].secuencia_tl43,
                        orden_presentacion: infME[i].orden_presentacion,
                        pregunta: infME[i].pregunta,
                        tipo_formulario: infME[i].tipo_formulario,
                        seccion_formulario: infME[i].seccion_formulario,
                        tipo_respuesta: infME[i].tipo_respuesta,
                        lista_respuesta: infME[i].lista_respuesta,
                        lista_etiquetas: infME[i].lista_etiquetas,
                        tipo_valor: infME[i].tipo_valor,
                        nombre_seccion: infME[i].nombre_seccion,
                        lista_valores: infME[i].lista_valores,
                        respuesta: infME[i].respuesta,
                        secuencia_vh65: infME[i].secuencia_vh65,
                        corden_seccion: infME[i].corden_seccion,
                        selResp: selRes,
                        sevResp: sevRes,
                        erResp: erRes,
                        evResp: evRes,
                        selRespt: infME[i].selRespt,
                        sevRespt: infME[i].sevRespt,
                        erRespt: infME[i].erRespt,
                        evRespt: infME[i].evRespt,
                        nombre_seccion: infME[i].nombre_seccion,
                        imagen: infME[i].imagen,
                        observacion_calidad: obsrvaCC
                    });

                    //if (infME[i].tipo_respuesta == "EV") { alert(griddata[i].evResp[0].evR); alert(griddata[i].evResp[1].evR); alert(griddata[i].evResp[2].evR); alert(griddata[i].evResp[3].evR); }
                    //alert("sevRespt" + infME[i].sevRespt);
                    //alert("sevRes" + inspeccionar(sevRes));
                    //alert("sevResp"+inspeccionar(griddata[i].sevResp));
                    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", inspeccionar(griddata[i].sevResp));

                }
            } catch (e) {
                alert("1" + e);
            }
            //localStorage.setItem("griddataCCL", JSON.stringify(griddata));
            var tablaINP = cargatablaCCL(griddata);
            document.getElementById("observacionCCL").value = griddata[0].observacion_calidad;
            var registroME = "";
            generarOrdenPDFCCL(inforusa.numero_orden, "O");
            document.getElementById("tabstripCCL").style.display = 'block';
            //alert(inforusa.estado_calidad);
            if (inforusa.estado_calidad == "POR_REVISAR" && inforusa.estado_calidad !== "") {
                desactivaTablaCCL(true);
            }
            return tablaINP;
        }
    } catch (e) {
        alert("21" + e);
        return;
    }
}

function parametrosCCLPath() {
    try {
        var accResp = "";
        //http://192.168.1.50:8089/concesionario/Services/TG/Parametros.svc/ParametroEmpGet/6,;TG;APP_ENTREGAS;;PATH_ENTREGAS
        var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TG/Parametros.svc/ParametroEmpGet/6,;TL;PATH_CONTROL_CALIDAD;;PATH_CONTROL_CALIDAD";
        var respPar;
        $.ajax({
            url: Url,
            type: "GET",
            dataType: "json",
            async: false,
            success: function (data) {
                try {
                    accResp = JSON.parse(data.ParametroEmpGetResult).tmpParamEmp;
                    respPar = accResp[0];
                } catch (e) {
                    window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>36 ERROR</center>", "parametrosCCLPath" + inspeccionar(e));
                    respPar = "error";
                }
            },
            error: function (err) {
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>37 ERROR</center>", inspeccionar(err));
                respPar = "error";
            }
        });
        return respPar;
    } catch (f) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>38 ERROR</center>", f);
        return "error";
    }
}

function cargatablaCCL(griddata1) {
    try {
        document.getElementById("descSMS_1RCCL").innerHTML = "";
        dataRespuesta = "";
        var tablaOT = "";

        tablaOT = "<table BORDER CELLPADDING=10 CELLSPACING=10 id='docume' scr>";
        var cuerpo;
        tablaOT += "<tr align='center' style='background-color:#88282C'><td></td><td><label style='font-weight:bold; font-size:13px;color:#ffffff'></label></td><td></td></tr>"
        for (var i = 0; i < griddata1.length - 1; i++) {
            var seccionFor = griddata1[i].seccion_formulario;
            tablaOT += "<tr style='background-color:#88282C'><td align='center' style='width: 65%;'><label style='font-weight:bold; font-size:13px;color:#ffffff'>" + griddata1[i].nombre_seccion + "</label></td><td style='width: 10%;'></td><td style='width: 25%;'></td></tr>";
            while (griddata1[i].seccion_formulario == seccionFor) {
                var tipoUsa = "";
                if (griddata1[i].tipo_valor == "NUMERICO") {
                    tipoUsa = "number";
                } else {
                    tipoUsa = "text";
                }
                tablaOT += "<tr><td>" + griddata1[i].orden_presentacion + "&nbsp;&nbsp;<label class='w3-text-black' style='font-weight:bold; font-size:13px'>" + griddata1[i].pregunta + "</label></td>";
                if (griddata1[i].tipo_respuesta == "SELECCION" || griddata1[i].tipo_respuesta == "SEV") {
                    var sevUsa = "<td><table style='width: 100%;'><tr>";
                    if (griddata1[i].tipo_respuesta == "SEV") {
                        var valorsev = griddata1[i].sevRespt.split(',');
                        /* window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(valorsev)); */
                        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(valorsev));
                        for (var l = 0; l < griddata1[i].sevResp.length; l++) {
                            if (valorsev[l] === "undefined" || valorsev[l] === null) {
                                valorsev[l] = " ";
                            }
                            sevUsa += "<td><p><input id=SEV" + i + "" + l + " type='" + tipoUsa + "' style='width:100%' placeholder=" + (l + 1) + " value=" + valorsev[l] + " disabled = 'true'></p></td>";
                        }
                        //alert(sevUsa);
                    } else {

                        /* if(griddata1[i].imagen != ""){
                            var pathCC = parametrosCCLPath();
                            var pathCC1 = pathCC["descripcion-2"] + griddata1[i].imagen;
                            sevUsa += "<td><div><center><img " + "src='"+pathCC1+"' style='border:1px"+
                            " solid black;'></center></div></td>";
                        } */
                    }
                    if (griddata1[i].imagen != "" && griddata1[i].tipo_respuesta == "SELECCION") {
                        //alert(i+" - "+griddata1[i].imagen);
                        var pathCC = parametrosCCPath();
                        var pathCC1 = pathCC["descripcion-2"] + griddata1[i].imagen;
                        sevUsa += "<td><div><center><img " + "src='" + pathCC1 + "' style='border:1px" +
                            " solid black;'></center></div></td>";
                    }
                    sevUsa += "</tr></table></td>";
                    var selUsa = seleccionCCL(griddata1[i].selResp, i, griddata1[i].selRespt);
                    //alert(selUsa);
                    tablaOT += sevUsa + "<td>" + selUsa + "</td>";
                } else {
                    if (griddata1[i].tipo_respuesta == "ER") {
                        var sevUsa = "<td><table style='width: 100%;'><tr>";
                        var valorer = griddata1[i].erRespt.split(',');

                        for (var l = 0; l < griddata1[i].erResp.length; l++) {
                            if (valorer[l] === undefined || valorer[l] === null || valorer[l] === "") {
                                valorer[l] = " ";
                            }
                            sevUsa += "<td><p><input id=ER" + i + "" + l + " type='text' size='2' placeholder=" + griddata1[i].erResp[l].erR + " value=" + valorer[l] + " disabled = 'true'></p></td>";
                        }
                        if (griddata1[i].imagen != "" && griddata1[i].tipo_respuesta == "ER") {
                            var pathCC = parametrosCCPath();
                            var pathCC1 = pathCC["descripcion-2"] + griddata1[i].imagen;
                            sevUsa += "<td><div><center><img " + "src='" + pathCC1 + "' style='border:1px" +
                                " solid black;'></center></div></td>";
                        }
                        sevUsa += "</tr></table></td>";
                        var selUsa = seleccionCCL(griddata1[i].selResp, i, griddata1[i].selRespt);
                        tablaOT += sevUsa + "<td>" + selUsa + "</td>";
                    } else {
                        if (griddata1[i].tipo_respuesta == "EV") {
                            var sevUsa = "<td><table style='width: 100%;'><tr>";
                            var valorev = griddata1[i].evRespt.split(',');
                            for (var l = 0; l < griddata1[i].evResp.length; l++) {
                                if (valorev[l] === "undefined" || valorev[l] === null) {
                                    valorev[l] = " ";
                                }
                                sevUsa += "<td><p><input id=EV" + i + "" + l + " type='text'  placeholder=" + griddata1[i].evResp[l].evR + " value=" + valorev[l] + " disabled = 'true'></p></td>";
                            }
                            if (griddata1[i].imagen != "" && griddata1[i].tipo_respuesta == "EV") {
                                var pathCC = parametrosCCPath();
                                var pathCC1 = pathCC["descripcion-2"] + griddata1[i].imagen;
                                sevUsa += "<td><div><center><img " + "src='" + pathCC1 + "' style='border:1px" +
                                    " solid black;'></center></div></td>";
                            }
                            sevUsa += "</tr></table></td>";
                            tablaOT += sevUsa + "<td></td>";
                        }
                    }
                }
                tablaOT += "</tr>";
                i++;
                if (i == tamano) {
                    break;
                }
            }
            i--;
        }
        tablaOT += "</table>";
        tablaOT += observa;
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", tablaOT);
        document.getElementById("descSMS_1RCCL").innerHTML = " ";
        document.getElementById("descSMS_1RCCL").innerHTML = tablaOT;
        document.getElementById("descSMS_1RCCL").style.display = 'block';
        localStorage.setItem("griddataCCL", JSON.stringify(griddata1));
        dataRespuesta = griddata1;
        return tablaOT;
    } catch (e) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(griddata1[i]));
        alert("carga" + i + e);

    }
}

function seleccionCCL(sele1, i, sele2) {
    try {
        var cboMarcaHTML = "<p><select id='" + "SEL" + i + "'class='w3-input w3-border textos' disabled = 'true'>";
        for (var k = 0; k < sele1.length; k++) {
            if (sele2 == sele1[k].sele) {
                cboMarcaHTML += "<option  value='" + sele1[k].sele + "' selected>" + sele1[k].sele + "</option>";
            } else {
                cboMarcaHTML += "<option  value='" + sele1[k].sele + "'>" + sele1[k].sele + "</option>";
            }
        }
        cboMarcaHTML += "</select></p>";
        return cboMarcaHTML;
    } catch (e) {
        alert("selec" + e);
    }
}

function cargadoc(dataItemDoc) {
    try {

        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(dataItemDoc));
        //dataItemDoc.numero_orden
        //ConsultaControlCalidad(dataItemDoc);
        var controlHTML = "<br><table cellpadding='0' cellspacing='0' border='0'>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Orden de reparación: <font color='blue'>" + dataItemDoc.numero_orden + "</font></p></td><td></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Nombre del cliente: <font color='blue'>" + dataItemDoc.nombre_propietario + "</font></p></td></tr>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Vehículo: <font color='blue'>" + dataItemDoc.nombre_modelo + "</font></p></td><td></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Teléfono: <font color='blue'>" + dataItemDoc.telefono_cliente + "</font></p></td></tr>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Chasis: <font color='blue'>" + dataItemDoc.chasis + "</font></p></td><td width='20%'><p><label style='font-family:Arial; font-size:11px'></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Celular: <font color='blue'>" + dataItemDoc.celular_cliente + "</font></p></td></tr>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Kilometraje: <font color='blue'>" + dataItemDoc.kilometraje + "</font></p></td><td></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Fecha: <font color='blue'>" + dataItemDoc.fecha_cita + "</font></p></td></tr></table>";

        var controlHTMLas = "<br><table cellpadding='0' cellspacing='0' border='0'>" +
            "<tr style='border-bottom:3pt solid red;'><td colspan='3'><p><label" +
            " style='font-family:Arial; font-size:14px; color: red'>ASESOR DE SERVICIO</p></td><td>" +
            "</td><td width='80%'></td></tr>" +
            "<tr style='border-bottom:1pt solid red;'><td width='20%'></td><td width='80%'><label" +
            "style='font-family:Arial; font-size:11px; color: red'>" +
            "PLUMAS LIMPIAPARABRISAS</td><tdwidth='80%'></td></tr>" +
            "<tr><td width='20%'><span class='fa-stack fa-lg'><i class='fa fa-check-circle-o fa-stack-1x'" + "style=' color:green'></i></td></tr>" +
            "<tr><td width='20%'><span class='fa-stack fa-lg'><i class='fa fa-circle" +
            " fa-stack-1x'" +
            "style=' color:#EF7F1A'></i></td><td><img src='images/parabrisas.png' style='border:1px" +
            " solid white'></td><td></td></tr>" +
            "<tr><td width='20%'><span class='fa-stack fa-lg'><i class='fa fa-circle fa-stack-1x' style='" + "color:red'></i></td></tr>" +

            "<tr style='border-top:3pt solid red;border-bottom:1pt solid red;'><td width='20%'></td>" +
            "<td width='80%'><label style='font-family:Arial; font-size:11px; color: red;'>PROFUNDIDAD" +
            " DE LABRADO Y PRESION DE NEUMATICOS</td><td></td></tr>" +

            "<tr><td><br><label style='font-family:Arial; font-size:11px;'>DD&nbsp;&nbsp;</label>" +
            "<label style='font-family:Arial; font-size:11px; border-top:3pt solid red;" +
            "border-bottom:3pt solid red;border-right:3pt solid red;border-left:3pt solid red;" +
            "'>60</label></td>" +
            "<td  width='80%'><br><label style='font-family:Arial; font-size:11px;'>DI&nbsp;&nbsp;</label>" +
            "<label style='font-family:Arial; font-size:11px; border-top:3pt solid red;" +
            "border-bottom:3pt solid red;border-right:3pt solid red;border-left:3pt solid" +
            " red'>59</td><td width='20%'></td></tr>" +

            "<tr><td width='40%'><br><div align='center'><img src='images/llanta.png'></div>" +
            "</td><td></td></tr>" +

            "<tr><td><br><label style='font-family:Arial; font-size:11px;'>DD&nbsp;&nbsp;</label>" +
            "<label style='font-family:Arial; font-size:11px; border-top:3pt solid red;" +
            "border-bottom:3pt solid red;border-right:3pt solid red;border-left:3pt solid red;" +
            "'>60</label></td>" +
            "<td  width='80%'><br><label style='font-family:Arial; font-size:11px;'>DI&nbsp;&nbsp;</label>" +
            "<label style='font-family:Arial; font-size:11px; border-top:3pt solid red;" +
            "border-bottom:3pt solid red;border-right:3pt solid red;border-left:3pt solid" +
            " red'>59</td><td width='20%'></td></tr>" +
            "</table>"
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", controlHTML + controlHTMLas);
        //document.getElementById("descSMS_1RCC").innerHTML = controlHTML + controlHTMLas;
        //alert(inspeccionar(dataItemDoc))🔴;
    } catch (error) {
        alert(error);
    }

}

function volverCCL() {
    try {
        var dialogfirmaOTJT = $("#dialogfirmaOTJT").data("kendoDialog");
        dialogfirmaOTJT.close();
    } catch (e) {
    }
    banderaCCL = 1;
    kendo.mobile.application.navigate("components/controlCalidadAprueba/view.html");
}

function formatoImpConCal(bolVisEV) {
    try {
        var inforImprime = JSON.parse(localStorage.getItem("dataItem"));
        var htmlFormatoEV = "";
        var repFirmaCC = "";
        if (document.getElementById("firmaGrafCC").value.includes("_firm") == true && document.getElementById("firmaGrafCC").value.includes("|") == true) {
            var arrImpFirma = document.getElementById("firmaGrafCC").value.split("|");
            repFirmaCC = "<img src='" + arrImpFirma[2] + "' height='80'>";
            //localStorage.setItem("firma",repFirma);
            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> repFirma</center>", repFirma);
        }
        var repFirmaCCA = "";
        if (document.getElementById("firmaGrafCC").value.includes("_firmAS") == true && document.getElementById("firmaGrafCC").value.includes("|") == true) {
            var arrImpFirmaCCA = document.getElementById("firmaGrafCC").value.split("|");
            repFirmaCCA = "<img src='" + arrImpFirmaCCA[4] + "' height='80'>";
            //localStorage.setItem("firmaas",repFirmaAS);
            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> repFirmaAS</center>", repFirmaAS);
        }

        kendo.ui.progress($("#reporteCCScreen"), true);
        setTimeout(function () {
            var tablaDat = cargarTablaConC(JSON.parse(localStorage.getItem("dataItemCCA")));
            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", tablaDat);
            var tablaPreguntasImp = cargarTablaINP(inforImprime); //ConsultarMEUSA(inforImprime);  // ConsultarMEUSA_IMP_2(ktedsEV); // ConsultarMEUSA_IMP(ktedsEV);
            //  ============================================================================================================================
            //  FORMATO GENERAL
            //  ============================================================================================================================
            var ima = localStorage.getItem("ls_url2").toLocaleString() + "/formato_mail/logofirma.jpg";
            htmlFormatoEV = "<table width='100%' cellpadding='2' cellspacing='0'>" +
                "<tr>" +
                "<td>" +
                "<table width='100%' cellpadding='0' cellspacing='0'>" +
                "<tr>" +
                "<td>" + "<img src='" + ima + "' alt='FLC' width='115' height='70'>" /* imgLogoEV() */ + "</td>" +
                "<td style='text-align:center'>" +
                "<h2><label style='color:#C00000'>FORMULARIO PARA CONTROL DE CALIDAD</label></h2>" +
                "</td>" +
                "</tr>" +
                "</table>" +
                "</td>" +
                "</tr>" +
                "<tr style='border-bottom:3pt solid black'>" +
                "<td>" +
                /* "<hr style='color:#C00000;border:2px solid' />" + */
                "</td>" +
                "</tr>" +
                "<tr>" +
                "<td>" +
                tablaDat + //html_ev_01 +
                "</td>" +
                "</tr>" +
                "<tr>" +
                "<td></td><td>" +
                "<tr>" +
                "<td style='background-color:black'>" +
                /* "<p><label style='color:#FFFFFF; font-family:Arial; font-size:12px'> B = BUENO        R = REGULAR     M = MALO    </label></p>" + */
                "</td>" +
                "</tr>" +
                tablaPreguntasImp +
                //html_ev_02 +
                "</td>" +
                "</tr>" +
                "<tr>" +
                "<td style='background-color:#ccffff'>" +
                "<p><label style='color:#FFFFFF; font-family:Arial; font-size:11px'>COMENTARIOS, RECOMENDACIONES U OBSERVACIONES</label></p>" +
                "</td>" +
                "</tr>" +
                "<tr>" +
                "<td><p><label style='font-family:Arial; font-size:11px'>" + inforImprime.observacion_tecnica + "</label></p></td>" +
                "</tr>" +
                //"<tr>" +
                //"<td>&nbsp;" +
                //"</td>" +
                //"</tr>" +
                "<tr>" +
                "<td>" +
                "<table width='100%' cellpadding='3' cellspacing='0' border='0' style='border-color:#C00000'>" +
                "<tr>" +
                "<td width='50%' style='background-color:black'>" +
                "<p><label style='color:white; font-family:Arial; font-size:11px '> ENTREGA:</label></p>" +
                "</td>" +
                "<td style='background-color:black'>" +
                "<p><label style='color:white; font-family:Arial; font-size:11px'> RECIBE:</label></p>" +
                "</td>" +
                "</tr>" +
                "</table>" +
                "</td>" +
                "</tr>" +
                "<tr>" +
                "<td>" +
                "<table width='100%' cellpadding='3' cellspacing='0'>" +
                "<tr>" +
                "<td width='50%'><br/><br/><br/></td>" +
                "<td><br/><br/><br/></td>" +
                "</tr>" +
                "<tr>" +
                "<td>" +
                "               <table style='width:100%' cellspacing='0' cellpadding='0'>" +
                "    <tr> " +
                "      <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "      <td style='text-align:center;font-family:Arial; font-size:7px'><center>" + repFirmaCCA + "</center></td> " +
                "       <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "       <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "       <td style='text-align:center;font-family:Arial; font-size:7px'><center>" + repFirmaCC + "</center></td>" +
                "       <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "    </tr>" +
                "    <tr>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>--------------------------------------</td>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>--------------------------------------</td>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "    </tr>" +
                "    <tr>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>Firma Asesor</td>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>Firma aprobaci&oacute;n del cliente</td>" +
                "     <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>" +
                "    </tr>" +
                "   </table>" +
                "</td>" +
                "</tr>" +
                /* "<tr>" +
                "<td width='50%'><p><label style='font-family:Arial; font-size:11px'>Firma: _______________________________</label></p></td>" +
                "<td><p><label style='font-family:Arial; font-size:11px'>Firma: _______________________________</label></p></td>" +
                "</tr>" +
                "<tr>" +
                "<td width='50%'><p><label style='font-family:Arial; font-size:11px'>      TECNICO</label></p></td>" +
                "<td><p><label style='font-family:Arial; font-size:11px'>    JEFE DE TALLER</label></p></td>" +
                "</tr>" +
                "<tr>" +
                //"<td><p><label style='font-family:Arial; font-size:11px'>Nombre y Apellido: " + localStorage.getItem("ls_usunom").toLocaleString() + "</label></p></td>" +
                //"<td><p><label style='font-family:Arial; font-size:11px'>Nombre y Apellido: " + ev_nombres + "</label></p></td>" +
                "</tr>" +
                "<tr>" +
                //"<td><p><label style='font-family:Arial; font-size:11px'>ID: " + identifica_entrega + "</label></p></td>" +
                //"<td><p><label style='font-family:Arial; font-size:11px'>ID: " + ev_identifica_cliente + "</label></p></td>" +
                "</tr>" + */
                "</table>" +
                "</td>" +
                "</tr>" +
                "</table>";

            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> IMPRIME</center>", htmlFormatoEV);
            if (bolVisEV == false) {
                //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> IMPRIME</center>", htmlFormatoEV);
                try {
                    cordova.plugins.printer.print(
                        htmlFormatoEV,
                        {
                            graystyle: true
                        },
                        function (msg) {
                            kendo.ui.progress($("#avaluoScreen"), false);

                        },
                        function (msg) {
                            kendo.ui.progress($("#avaluoScreen"), false);
                        }
                    );
                } catch (e) {
                    alert(e);
                }
                //volverForm();
            } else {
                //document.getElementById("divExplica").innerHTML = "<table align='center' width='95%' border='0' cellspacing='0' cellpadding='0'><tr><td>" +
                htmlFormatoEV + "</td></tr></table>";
                //document.getElementById("divExplica").style.display = "initial";
                //document.getElementById("headEV").style.display = "none";
            }

            kendo.ui.progress($("#reporteCCScreen"), false);

            // precarga *********************************************************************************************
        }, 2000);

    } catch (eimpev) {
        kendo.ui.progress($("#reporteCCScreen"), false);
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", eimpev);
    }
}

function cargarTablaConC(inforImprime) {
    try {
        /* var ev_yyyy = "";
        var ev_hhmm = "";
        if (ev_yyyy.includes("-") == false || ev_hhmm.includes(":") == false) {
            var today = new Date();
            var dd = today.getDate();
            var mm = today.getMonth() + 1; //Enero is 0

            var yyyy = today.getFullYear();
            if (dd < 10) {
                dd = '0' + dd;
            }
            if (mm < 10) {
                mm = '0' + mm;
            }

            var hhmm = new Date().toLocaleTimeString('en-US', { hour12: false, hour: "numeric", minute: "numeric" });
            ev_yyyy = yyyy + "-" + mm + "-" + dd;
            ev_hhmm = hhmm;
        }

        var arrFecEV = ev_yyyy.split('-');
        var arrHoEv = ev_hhmm.split(':'); */
        var html_ev_01 = "<br><table cellpadding='0' cellspacing='0' border='0'>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Orden de reparación: <font color='blue'>" + inforImprime.numero_orden + "</font></p></td><td></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Nombre del cliente: <font color='blue'>" + inforImprime.nombre_propietario + "</font></p></td></tr>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Vehículo: <font color='blue'>" + inforImprime.nombre_modelo + "</font></p></td><td></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Teléfono: <font color='blue'>" + inforImprime.telefono_cliente + "</font></p></td></tr>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Chasis: <font color='blue'>" + inforImprime.chasis + "</font></p></td><td width='20%'><p><label style='font-family:Arial; font-size:11px'></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Celular: <font color='blue'>" + inforImprime.celular_cliente + "</font></p></td></tr>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Kilometraje: <font color='blue'>" + inforImprime.kilometraje + "</font></p></td><td></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Fecha: <font color='blue'>" + inforImprime.fecha_cita + "</font></p></td></tr></table>";
    } catch (e) {
        alert(e);
    }
    return html_ev_01;
}

function cargarTablaINP(inforusa) {
    try {
        localStorage.removeItem("inforImprime");
        var respOT = "0";
        var tipo_doc = "CONTROL_CALIDAD";
        if (inforusa.chasis.substr(7, 1) == "E") {
            tipo_doc = "CONTROL_CALIDAD_ELEC"
        }
        var UrlMotorEscape = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl43PreguntasCalidadGet/1,json;" +
            inforusa.codigo_empresa + ";" + inforusa.codigo_sucursal + ";" + inforusa.codigo_taller + ";" + inforusa.anio + ";" +
            inforusa.secuencia_orden + ";" + tipo_doc + ";;;;;;";
        var infME;
        //alert(UrlMotorEscape);
        $.ajax({
            url: UrlMotorEscape,
            type: "GET",
            async: false,
            dataType: "json",
            success: function (data) {
                try {
                    infME = (JSON.parse(data.tl43PreguntasCalidadGetResult)).ttl43;
                    localStorage.setItem("inforImprime", JSON.stringify(infME));
                    respOT = "1";
                } catch (e) {
                    alert(e);
                    return;
                }
            },
            error: function (err) {
                alert(err);
                return;
            }
        });
        if (respOT == "1") {
            try {
                var griddata = [];
                tamano = infME.length;
                for (var t = 0; t < infME.length; t++) {
                    if (infME[t].orden_presentacion < 10) {
                        infME[t].orden_presentacion = "0" + infME[t].orden_presentacion;
                    }
                }

                infME.sort(function myfunction(a, b) {
                    return (a.corden_seccion + "" + a.orden_presentacion) - (b.corden_seccion + "" + b.orden_presentacion)
                });
                for (var i = 0; i < infME.length; i++) {
                    var selecResp = new Array();
                    var sevResp = new Array();
                    var erResp = new Array();
                    var evResp = new Array();
                    if (infME[i].tipo_respuesta == "SELECCION" || infME[i].tipo_respuesta == "SEV") {
                        if (infME[i].tipo_respuesta == "SEV") {
                            //alert("etiqu"+infME[i].lista_etiquetas);
                            sevResp = infME[i].lista_etiquetas.split(",");
                            //alert(inspeccionar(sevResp));
                            var sevRes = [];
                            for (var k = 0; k < sevResp.length; k++) {
                                sevRes.push({sevR: sevResp[k]});
                                //alert(inspeccionar(sevRes[k]));
                            }
                            //alert(inspeccionar(sevRes[0]));
                        }
                        //alert("resp"+infME[i].lista_respuesta)
                        selecResp = infME[i].lista_respuesta.split(",");
                        var selRes = [];
                        selRes.push({sele: "Seleccione"});
                        for (var j = 0; j < selecResp.length; j++) {
                            selRes.push({sele: selecResp[j]});
                        }
                    } else {
                        if (infME[i].tipo_respuesta == "ER") {
                            //alert("else eti" + infME[i].lista_etiquetas);
                            erResp = infME[i].lista_etiquetas.split(",");
                            var erRes = [];
                            for (var l = 0; l < erResp.length; l++) {
                                erRes.push({erR: erResp[l]});
                            }
                            //alert("else resp" + infME[i].lista_respuesta);
                            selecResp = infME[i].lista_respuesta.split(",");
                            var selRes = [];
                            selRes.push({sele: "Seleccione"});
                            for (var j = 0; j < selecResp.length; j++) {
                                selRes.push({sele: selecResp[j]});
                            }
                        } else {
                            if (infME[i].tipo_respuesta == "EV") {
                                //alert("else ev"+infME[i].lista_etiquetas);
                                evResp = infME[i].lista_etiquetas.split(",");
                                //alert((evResp.length));
                                var evRes = [];
                                for (var hh = 0; hh < evResp.length; hh++) {
                                    evRes.push({evR: evResp[hh]});
                                }
                                //alert("0:" + evRes[0].evR);
                                //alert("1:" + evRes[1].evR);
                                //alert("2:" + evRes[2].evR);
                                //alert("3:" + evRes[3].evR);
                            }
                        }
                    }
                    griddata.push({
                        codigo_empresa: infME[i].codigo_empresa,
                        codigo_sucursal: infME[i].codigo_sucursal,
                        codigo_taller: infME[i].codigo_taller,
                        anio_tl06: infME[i].anio_tl06,
                        secuencia_orden_tl06: infME[i].secuencia_orden_tl06,
                        secuencia_tl43: infME[i].secuencia_tl43,
                        orden_presentacion: infME[i].orden_presentacion,
                        pregunta: infME[i].pregunta,
                        tipo_formulario: infME[i].tipo_formulario,
                        seccion_formulario: infME[i].seccion_formulario,
                        tipo_respuesta: infME[i].tipo_respuesta,
                        lista_respuesta: infME[i].lista_respuesta,
                        lista_etiquetas: infME[i].lista_etiquetas,
                        tipo_valor: infME[i].tipo_valor,
                        nombre_seccion: infME[i].nombre_seccion,
                        lista_valores: infME[i].lista_valores,
                        respuesta: infME[i].respuesta,
                        secuencia_vh65: infME[i].secuencia_vh65,
                        corden_seccion: infME[i].corden_seccion,
                        selResp: selRes,
                        sevResp: sevRes,
                        erResp: erRes,
                        evResp: evRes,
                        selRespt: infME[i].selRespt,
                        sevRespt: infME[i].sevRespt,
                        erRespt: infME[i].erRespt,
                        evRespt: infME[i].evRespt,
                        nombre_seccion: infME[i].nombre_seccion
                    });
                    //if (infME[i].tipo_respuesta == "EV") { alert(griddata[i].evResp[0].evR); alert(griddata[i].evResp[1].evR); alert(griddata[i].evResp[2].evR); alert(griddata[i].evResp[3].evR); }
                    //alert("sevRespt" + infME[i].sevRespt);
                    //alert("sevRes" + inspeccionar(sevRes));
                    //alert("sevResp"+inspeccionar(griddata[i].sevResp));
                    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", inspeccionar(griddata[i].sevResp));
                }
            } catch (e) {
                alert("1" + e);
            }
            //alert("antes de func");
            var tablaINP = cargatabla_1AVA(griddata);
            var registroME = "";
            return tablaINP;
        }
    } catch (e) {
        alert("21" + e);
        return;
    }
}

function cargatabla_1AVA(griddata1) {
    dataRespuesta = "";
    var tablaOT = "";
    tablaOT += "<table>";
    //<tr style='background-color:#C00000'><td></td><td></td><td>" +
    //"<label style='font-weight:bold; font-size:11px;color:#ffffff'>B=BUENO   R=REGULAR   M=MALO</label></td></tr></table>";
    tablaOT += "<table>"
    for (var i = 0; i < griddata1.length; i++) {
        var seccionFor = griddata1[i].seccion_formulario;
        //  alert(seccionFor);
        tablaOT += "<tr style='background-color:black'><td style='width: 80%;'><p><label style='font-weight:bold; font-size:11px;color:white' >" +
            griddata1[i].seccion_formulario +
            "</label></p></td><td style='width: 1%;'></td><td style='width: 19%;'></td></tr>";
        while (griddata1[i].seccion_formulario == seccionFor) {
            //  alert("_" + griddata1[i].seccion_formulario + "_" + seccionFor + "_");

            //var tipoUsa = "";
            //if (griddata1[i].tipo_valor == "NUMERICO") {
            //    tipoUsa = "number";
            //} else {
            //    tipoUsa = "text";
            //}

            //   tablaOT += "<tr><td><p>" + griddata1[i].orden_presentacion + "&nbsp;&nbsp;" + griddata1[i].pregunta + "</p></td>";
            tablaOT += "<tr><td><p><label style='font-family:Arial; font-size:11px'>" + griddata1[i].pregunta + "</label></p></td>";

            if (griddata1[i].tipo_respuesta == "SELECCION" || griddata1[i].tipo_respuesta == "SEV") {
                var sevUsa = "<td><table style='width: 100%;'><tr>";
                if (griddata1[i].tipo_respuesta == "SEV") {
                    var valorsev = griddata1[i].sevRespt.split(',');
                    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", griddata1[i].erRespt);
                    for (var l = 0; l < griddata1[i].sevResp.length; l++) {
                        //alert("VAL: " + valorsev[l]);
                        if (valorsev[l] === "undefined" || valorsev[l] === null) {
                            valorsev[l] = " ";
                            // alert("VAL1: "+valorsev[l]);
                        }

                        if (l == 4) {
                            sevUsa += "</tr><tr>"
                        }
                        sevUsa += "<td>&nbsp;</td>";
                        //sevUsa += "<td><p><input id=SEV" + i + "" + l + " type='" + tipoUsa + "' style='width:100%' placeholder=" + (l + 1) + " value=" + valorsev[l] + "></p></td>";
                        sevUsa += "<td><p><input id=SEV" + i + "" + l + "style='width:100%' placeholder=" + (l + 1) + " value=" + valorsev[l] + "></p></td>";
                    }
                }
                sevUsa += "</tr></table></td>";
                if (griddata1[i].selRespt.length == 0) {
                    bolImprimeEV = false;
                }
                var selUsa = griddata1[i].selRespt; // seleccionUSA(griddata1[i].selResp, i, griddata1[i].selRespt);
                tablaOT += sevUsa + "<td><p><label style='font-family:Arial; font-size:11px'>" + selUsa + "</label></p></td>";
            } else {
                if (griddata1[i].tipo_respuesta == "ER") {
                    var sevUsa = "<td><table style='width: 100%;'><tr>";
                    var valorer = griddata1[i].erRespt.split(',');
                    for (var l = 0; l < griddata1[i].erResp.length; l++) {
                        sevUsa += "<td>&nbsp;</td>";
                        sevUsa += "<td><p><input id=ER" + i + "" + l + " type='text' size='2' placeholder=" + griddata1[i].erResp[l].erR + " value=" + valorer[l] + "></p></td>";
                    }
                    sevUsa += "</tr></table></td>";
                    if (griddata1[i].selRespt.length == 0) {
                        bolImprimeEV = false;
                    }
                    var selUsa = griddata1[i].selRespt;  //seleccionUSA(griddata1[i].selResp, i, griddata1[i].selRespt);
                    tablaOT += sevUsa + "<td><p><label style='font-family:Arial; font-size:11px'>" + selUsa + "</label></p></td>";
                } else {
                    if (griddata1[i].tipo_respuesta == "EV") {
                        var sevUsa = "<td><table style='width: 100%;'><tr>";
                        var valorev = griddata1[i].evRespt.split(',');
                        for (var l = 0; l < griddata1[i].evResp.length; l++) {
                            sevUsa += "<td>&nbsp;</td>";
                            sevUsa += "<td><p><input id=EV" + i + "" + l + " type='text' size='2' placeholder=" + griddata1[i].evResp[l].evR + " value=" + valorev[l] + "></p></td>";
                        }
                        sevUsa += "</tr></table></td>";
                        tablaOT += sevUsa + "<td></td>";
                    }
                }
            }
            tablaOT += "</tr>";
            i++;
            if (i == tamano) {
                break;
            }
        }
        i--;
    }
    dataRespuesta = griddata1;
    tablaOT += "</table>";
    tablaOT += observa;
    return tablaOT;
}

function imgLogoEV() {
    return "<img src='data:image/png;base64,' height='60' width='129'>";
}

function generarOrdenPDFCCL(strArchivo, tipoPDF) {
    try {
        //document.getElementById("firmaGrafCCL").value = "";
        document.getElementById("firmaGrafCCLJT").value = "";

        //document.getElementById("titFirmaPregCCL").innerHTML = "Desea registrar la firma del cliente ?<br />";
        document.getElementById("titFirmaPregCCLJT").innerHTML = "Desea registrar la firma del jefe de taller ?<br />";
        var camposEV = parametrosEntregaCompletoOTCCL();
        var pathArchivoPDF = camposEV["descripcion-2"] + "\\" + strArchivo + "_" +
            localStorage.getItem("ls_ussucursal").toLocaleString() + "_" +
            localStorage.getItem("ls_usagencia").toLocaleString() + "\\" + strArchivo + "CC";
        var strFirmaJT = "";
        var strBarcode = "";
        if (tipoPDF == "O") {
            //htmlOrden = formatoRep02_mail();
            var canvas = document.createElement("canvas");
            JsBarcode(canvas, strArchivo, {format: "CODE39"});
            strBarcode = canvas.toDataURL("image/png");

            // firma base64
            //strFirma = document.getElementById("firmaOTCCL").value;
            strFirmaJT = document.getElementById("firmaOTCCLJT").value;
        } else {
            //htmlOrden = formatoRep01();
            //  pathArchivoPDF = camposEV["descripcion-2"] + "\\" + strArchivo + "\\" + "PRE_" + strArchivo;
            var arrPaths = urlFtpMayoOT().split("|");
            strBarcode = "ftp://" + arrPaths[1] + "/" + "|" + arrPaths[0] + "\\"; //"";
            pathArchivoPDF = camposEV["descripcion-2"] + "\\" + strArchivo + "\\" + "PRE_" +
                localStorage.getItem("ls_empresa").toLocaleString().substr(0, 5) + "_" +
                localStorage.getItem("ls_ussucursal").toLocaleString() + "_" +
                localStorage.getItem("ls_usagencia").toLocaleString() + "_" + strArchivo + "CC";
        }
        //    var strImgFija = strAutob64();


        //  alert(pathArchivoPDF);

        var params = {
            "strArchivo": pathArchivoPDF, // strArchivo,
            "strHTML": "", //htmlOrden,
            "strBarcode": "", //strBarcode,
            "strFirma": "",
            "strFirmaAS": "",
            "strFirmaJT": strFirmaJT
        };
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>formatoRep02_mail</center>", inspeccionar(params));
        // var Url = "http://localhost:4044/Services/TL/Taller.svc/creaPdf_2";
        var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/creaFirmasCC";

        $.ajax({
            url: Url,
            type: "POST",
            data: JSON.stringify(params),
            dataType: "json",
            headers: {
                'Content-Type': 'application/json;charset=UTF-8'
            },
            success: function (data) {
                try {
                    var dataJson = {
                        "0": data[0],
                        "1": data[1],
                        "2": data[2],
                        "3": data[3]
                    };

                    var data3 = data[3];
                    //alert(data);
                    var respMail = JSON.stringify(data[0]);
                    //window.myalert("<center><i class=\"fa fa-paper-plane\"></i> GENERADO</center>", respMail);
                    if (respMail.includes("Succes")) {
                        var respPdf = "ok";
                        document.getElementById("firmaGrafCCLJT").value = data3;

                        if (document.getElementById("firmaGrafCCLJT").value.includes("CC_firmJT.") == true) {
                            document.getElementById("titFirmaPregCCLJT").innerHTML = "";
                            document.getElementById("imgFirmaOTCCLJT").innerHTML = "";
                            document.getElementById("titFirmaPregCCLJT").innerHTML = "<center><b>La firma del jefe de taller ya fue registrada<b></center>";
                            document.getElementById("confirmFirmaOTCCLJT").style.display = 'none';
                            document.getElementById("cancelFirmaOTCCLJT").style.display = 'none';
                            document.getElementById("btnGuadaCLC").disabled = "true";
                        }
                        // vacia el campo firma base64
                        document.getElementById("firmaOTCCLJT").value = "";
                        //adjuntoArchivosOT_2(document.getElementById("numOT_2").value);
                    } else {
                        //  window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>1 ERROR</center>", data);
                        return;
                    }
                } catch (e) {
                    window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>34 ERROR</center>", e);
                    return;
                }
            },
            error: function (err) {
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>35 ERROR</center>", JSON.stringify(err));
                return;
            }
        });

        //    kendo.ui.progress($("#lector_barrasScreen"), false);
        //    // precarga *********************************************************************************************
        //}, 2000);
    } catch (epdf) {
        alert(epdf);
        kendo.ui.progress($("#lector_barrasScreen"), false);
        document.getElementById("otadjuntos").value = "";
        return;
    }
}

function parametrosEntregaCompletoOTCCL() {
    try {

        var accResp = "";

        //http://192.168.1.50:8089/concesionario/Services/TG/Parametros.svc/ParametroEmpGet/6,;TG;APP_ENTREGAS;;PATH_ENTREGAS

        var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TG/Parametros.svc/ParametroEmpGet/6,;TG;APP_ENTREGAS;;PATH_ENTREGAS";

        // window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> Url</center>", Url);

        var respPar;

        $.ajax({
            url: Url,
            type: "GET",
            dataType: "json",
            async: false,
            success: function (data) {
                try {
                    accResp = JSON.parse(data.ParametroEmpGetResult).tmpParamEmp;
                    respPar = accResp[0];
                } catch (e) {
                    window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>36 ERROR</center>", "parametrosEntregaCompletoOTCCL" + inspeccionar(e));
                    respPar = "error";
                }
            },
            error: function (err) {
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>37 ERROR</center>", inspeccionar(err));
                respPar = "error";
            }
        });

        return respPar;
    } catch (f) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>38 ERROR</center>", f);
        return "error";
    }
}
