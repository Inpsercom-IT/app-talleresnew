'use strict';
var observa = "";
var htmlFormatoEV = "";
app.reporteCLC = kendo.observable({
    onShow: function () {
        try {
            llamarNuevoestilo("lydReporte");
            llamarColorTexto(".w3-text-red");
            llamarNuevoestilo("confirmFirmaOTCLC")
            var dataItemDoc = JSON.parse(localStorage.getItem("dataItem"));
            localStorage.setItem("banderaEst", "PENDIENTE");
            localStorage.removeItem("griddataCLC");
            try {
                localStorage.removeItem("mensa");
            } catch (error) {

            }

            /*  try {
                 localStorage.setItem("tablaPDF", "");
             } catch (error) {
                 alert(error);
             } */
            var otREL = "";
            var otRELLIS = "";
            if (dataItemDoc.relacionadas_control_calidad !== "") {
                // alert(dataItemDoc.relacionadas_control_calidad);
                var listaOTREL = dataItemDoc.relacionadas_control_calidad.split(',');
                for (let index = 0; index < listaOTREL.length; index++) {
                    if (listaOTREL[index] !== dataItemDoc.numero_orden) {
                        otRELLIS = otRELLIS + " " + listaOTREL[index];
                    }
                }
                otREL = "  / OTs Relaciondas: " + otRELLIS;
            }
            $("#NoOrdenCLC").text("Número de OT:  " + dataItemDoc.numero_orden + otREL);
            document.getElementById("tabstripCLC").style.display = "none";
            document.getElementById("btnFooterCLC").innerHTML = "<button id='btnGuadaCLC1' onclick='imprimeAP();' class='w3-btn'><i id='icnPie0' class='fa fa-print' aria-hidden='true'></i> IMPRIMIR</button>" +
                "<button id='btnGuadaCLC0' onclick='guardarCLC();' class='w3-btn'><i id='icnPie2' class='fa fa-pencil' aria-hidden='true'></i> GUARDAR</button>" +
                "<button id='btnGuadaCLC2' onclick='enviaMailDocCC();' class='w3-btn'><i id='icnPie3' class='fa fa-pencil' aria-hidden='true'></i> ENVIAR MAIL</button>";
            document.getElementById("btnGuadaCLC0").disabled = true;
            llamarNuevoestilo("btnGuadaCLC");
            llamarNuevoestiloIconB("icnPie");
            if (localStorage.getItem("banderaE") == "true") {
                document.getElementById("liEncuestaCENRCA").style.display = "none";
                //document.getElementById("liFirmaCEN").style.display = "none";
                //document.getElementById("btnGuadaCLC0").disabled = true;
            } else {
                document.getElementById("liEncuestaCENRCA").style.display = 'initial';
                //document.getElementById("liFirmaCEN").style.display = 'initial';
                //document.getElementById("btnGuadaCLC0").disabled = false;
            }
            kendo.ui.progress($("#reporteCLCScreen"), true);
            setTimeout(function () {
                var arrLi = ["liDatosRCA","liActividadesRCA","liDetalleRCA", "liVideoRCA","liEncuestaCENRCA","liFirmaCENRCA"];
                var idLi = 1;
                for (var x = 0; x < arrLi.length; x++) {
                    document.getElementById(arrLi[x]).removeAttribute("style");
                    if (parseInt(localStorage.getItem("ls_dimensionW").toLocaleString()) < 361) {
                        document.getElementById("l" + idLi).style.fontSize = "12px";
                        idLi++;
                    }
                }
                ConsultaControlCalidadCLC(dataItemDoc);
                datosot(dataItemDoc);
                infoDetalleOTCLCENCU(dataItemDoc);
                actividadesOT(dataItemDoc);
                encuestaOT(dataItemDoc);
                document.getElementById('liEncuestaCENRCA').addEventListener('click', function () {
                    if (localStorage.getItem("banderaEst").toString() == "ENTREGADO") {
                        document.getElementById("btnGuadaCLC0").disabled = true;
                    } else {
                        document.getElementById("btnGuadaCLC0").disabled = false;
                    }

                });
                document.getElementById('liDatosRCA').addEventListener('click', function () {
                    document.getElementById("btnGuadaCLC0").disabled = true;
                });
                document.getElementById('liActividadesRCA').addEventListener('click', function () {
                    document.getElementById("btnGuadaCLC0").disabled = true;
                });
                document.getElementById('liDetalleRCA').addEventListener('click', function () {
                    document.getElementById("btnGuadaCLC0").disabled = true;
                });
                document.getElementById('liVideoRCA').addEventListener('click', function () {
                    document.getElementById("btnGuadaCLC0").disabled = true;
                });
                // FIRMA DIGITAL>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                document.getElementById('confirmFirmaOTCLC0').addEventListener('click', function () {
                    firmaDigitalOTCLC();
                });
                document.getElementById('confirmFirmaOTCLC1').addEventListener('click', function () {
                    document.getElementById("firmaOTCLC").value = "";
                    document.getElementById("imgFirmaOTCLC").innerHTML = "";
                });

                //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><
                kendo.ui.progress($("#reporteCLCScreen"), false);

            }, 2000);

        } catch (error) {
            kendo.ui.progress($("#reporteCLCScreen"), false);
            alert("onshow" + error);
        }

    },
    afterShow: function () {
    },
    ini: function () {

    }
});
app.localization.registerView('reporteCLC');

function verpdfDOC() {
    try {
        var strArchivo = JSON.parse(localStorage.getItem("dataItem")).numero_orden;
        var camposEV = parametrosEntregaCompletoOTCLCL();
        var pathArchivoPDF = camposEV["descripcion-2"] + "\\" + strArchivo + "_" +
            localStorage.getItem("ls_ussucursal").toLocaleString() + "_" +
            localStorage.getItem("ls_usagencia").toLocaleString() + "\\" + strArchivo + "CC.pdf";

    } catch (error) {
        alert("verpdfDOC" + error);
    }
}

function validaTablaCCAP() {
    var dataRespuesta = JSON.parse(localStorage.getItem("griddataEncuestaCC"));
    try {
        document.getElementById("observacionCCE").value

        if (document.getElementById("nombreEC").value == "") {
            bandera = 1;
            pregunta = "Nombre";
            return bandera + ";" + pregunta;
        }
        if (document.getElementById("telefonoFI").value == "" && document.getElementById("telefonCe").value == "") {
            bandera = 1;
            pregunta = "Teléfono Fijo y Teléfono Celular";
            return bandera + ";" + pregunta;
        }
        /* if (document.getElementById("telefonCe").value == ""){
            bandera = 1;
            pregunta = "Teléfono Celular";
            return bandera + ";" + pregunta;
        } */
        var bandera = 0;
        var pregunta = "";

        for (var i = 0; i <= dataRespuesta.length - 1; i++) {
            if (dataRespuesta[i].tipo_respuesta == "SELECCION" || dataRespuesta[i].tipo_respuesta == "SEV") {
                if (dataRespuesta[i].tipo_respuesta == "SEV") {
                    for (var l = 0; l < dataRespuesta[i].sevResp.length; l++) {
                        if (document.getElementById("SEV" + i + "" + l) == null) {
                            bandera = 1;
                            pregunta = dataRespuesta[i].pregunta.toLocaleString();
                            return bandera + ";" + pregunta;
                        }
                    }
                }
                var j = 0;
                var band = 0;
                if (dataRespuesta[i].tipo_objeto == "RADIO_BUTTON") {
                    while (j < dataRespuesta[i].selResp.length) {
                        if (dataRespuesta[i].selResp[j].sele !== "Seleccione") {
                            if (document.getElementById("E" + dataRespuesta[i].selResp[j].sele + i).checked) {
                                band = 1;
                                j = dataRespuesta[i].selResp.length;
                            }
                        }
                        j += 1;
                    }
                } else {
                    if (document.getElementById("SELE" + i).value.toLocaleString() !== "Seleccione") {
                        band = 1;
                    }
                }
                /* while (j < dataRespuesta[i].selResp.length) {
                    if(dataRespuesta[i].selResp[j].sele !== "Seleccione"){
                        if (document.getElementById("E"+dataRespuesta[i].selResp[j].sele + i).checked) {
                            band = 1;
                        }
                    }
                    j+=1;
                } */
                if (band == 0) {
                    bandera = 1;
                    pregunta = dataRespuesta[i].pregunta.toLocaleString();
                    return bandera + ";" + pregunta;
                }


            }
            if (dataRespuesta[i].tipo_respuesta == "ER") {
                for (var k = 0; k < dataRespuesta[i].erResp.length; k++) {
                    if (document.getElementById("ER" + i + "" + k) == null) {
                        bandera = 1;
                        pregunta = dataRespuesta[i].pregunta.toLocaleString();
                        return bandera + ";" + pregunta;
                    }
                }
                var j = 0;
                var band1 = 0;
                while (j < dataRespuesta[i].selResp.length) {
                    if (dataRespuesta[i].selResp[j].sele !== "Seleccione") {
                        if (document.getElementById(dataRespuesta[i].selResp[j].sele + i).checked) {
                            band1 = 1;
                        }
                    }
                    j += 1;
                }
                if (band1 == 0) {
                    bandera = 1;
                    pregunta = dataRespuesta[i].pregunta.toLocaleString();
                    return bandera + ";" + pregunta;
                }


            }
            if (dataRespuesta[i].tipo_respuesta == "EV") {

                for (var m = 0; m < dataRespuesta[i].evResp.length; m++) {
                    if (document.getElementById("EVE" + i + "" + m) == null) {
                        bandera = 1;
                        pregunta = dataRespuesta[i].pregunta.toLocaleString();
                        return bandera + ";" + pregunta;
                    }
                }
            }

        }
        return bandera + ";" + pregunta
    } catch (e) {
        alert("disEC" + e);
    }
}

function guardarCCETFIN() {
    try {
        var dataRespuesta = JSON.parse(localStorage.getItem("griddataEncuestaCC"));

        var validaCC = validaTablaCCAP();
        var validCC = validaCC.split(';');

        if (validCC[0] == 1) {
            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "Pregunta sin resposnter: " + validCC[1]);
            return;
        }

        //alert(localStorage.getItem("mensa"));
        /* if(document.getElementById("titFirmaPregCLC").innerText.trim() == "Desea registrar la firma del cliente ?"){
            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "Debe Registrar Firma Cliente");
            return;
        } */

        if (localStorage.getItem("mensa") == "Desea registrar la firma del cliente ?") {
            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "Debe Registrar Firma Cliente");
            return;
        }
        for (var i = 0; i < dataRespuesta.length; i++) {
            var sevRespts = [];
            var erRespts = [];
            var evRespts = [];
            if (dataRespuesta[i].tipo_respuesta == "SELECCION" || dataRespuesta[i].tipo_respuesta == "SEV") {
                if (dataRespuesta[i].tipo_respuesta == "SEV") {
                    //alert(dataRespuesta[i].sevResp.length);
                    for (var l = 0; l < dataRespuesta[i].sevResp.length; l++) {
                        if (document.getElementById("SEV" + i + "" + l) != null) {
                            //dataRespuesta[i].sevResp[l] = document.getElementById("SEV" + i + "" + l).value.toLocaleString();
                            sevRespts[l] = document.getElementById("SEVE" + i + "" + l).value.toLocaleString();
                            //alert(i + "sev " + inspeccionar(dataRespuesta[i].sevResp[l]));
                        }
                    }
                }
                var j = 0;
                while (j < dataRespuesta[i].selResp.length) {
                    if (dataRespuesta[i].selResp[j].sele !== "Seleccione") {
                        if (dataRespuesta[i].tipo_objeto == "RADIO_BUTTON") {
                            if (document.getElementById("E" + dataRespuesta[i].selResp[j].sele + i).checked) {
                                dataRespuesta[i].selRespt = document.getElementById("E" + dataRespuesta[i].selResp[j].sele + i).value;
                                j = dataRespuesta[i].selResp.length;
                            }
                        } else {
                            dataRespuesta[i].selRespt = document.getElementById("SELE" + i).value.toLocaleString();
                            break;
                        }

                    }
                    j += 1;
                }
            }
            if (dataRespuesta[i].tipo_respuesta == "ER") {
                for (var k = 0; k < dataRespuesta[i].erResp.length; k++) {
                    if (document.getElementById("ERE" + i + "" + k) != null) {
                        //dataRespuesta[i].erResp[l].erR = document.getElementById("ER" + i + "" + l).value;
                        erRespts[k] = document.getElementById("ER" + i + "" + k).value;
                    }
                    //alert(inspeccionar(erRespts));
                }
                var j = 0;
                while (j < dataRespuesta[i].selResp.length) {
                    if (dataRespuesta[i].selResp[j].sele !== "Seleccione") {
                        if (document.getElementById(dataRespuesta[i].selResp[j].sele + i).checked) {
                            dataRespuesta[i].selRespt = document.getElementById(dataRespuesta[i].selResp[j].sele + i).value;
                            j = dataRespuesta[i].selResp.length;
                        }
                    }
                    j += 1;
                }
                //dataRespuesta[i].selRespt = document.getElementById("SEL" + i).value.toLocaleString();
            }
            if (dataRespuesta[i].tipo_respuesta == "EV") {
                //var sevUsa = "<td><table style='width: 100%;'><tr>";
                for (var m = 0; m < dataRespuesta[i].evResp.length; m++) {
                    if (document.getElementById("EVE" + i + "" + m) != null) {
                        //dataRespuesta[i].evResp[l].evR = document.getElementById("EV" + i + "" + l).value;
                        evRespts[m] = document.getElementById("EVE" + i + "" + m).value;
                    }
                }
            }
            dataRespuesta[i].sevRespt = sevRespts;
            dataRespuesta[i].erRespt = erRespts;
            dataRespuesta[i].evRespt = evRespts
        }

        // alert(dataRespuesta.length);
        var para = [];
        if (dataRespuesta.length > 0) {
            try {
                for (var i = 0; i < dataRespuesta.length; i++) {
                    //alert(inspeccionar(dataRespuesta[i]));
                    //alert(inspeccionar(dataRespuesta[i].sevRespt));
                    var sevRespuesta = dataRespuesta[i].sevRespt;
                    var erRespuesta = dataRespuesta[i].erRespt;
                    var evRespuesta = dataRespuesta[i].evRespt;
                    var sevRespuestaS = "";
                    if (sevRespuesta != null) {
                        for (var j = 0; j < sevRespuesta.length; j++) {
                            sevRespuestaS += sevRespuesta[j];
                            sevRespuestaS += ",";
                        }
                    }
                    var erRespuestaS = "";
                    if (erRespuesta != null) {
                        for (var j = 0; j < erRespuesta.length; j++) {
                            erRespuestaS += erRespuesta[j];
                            erRespuestaS += ",";
                        }
                    }

                    var evRespuestaS = "";
                    if (evRespuesta != null) {
                        for (var j = 0; j < evRespuesta.length; j++) {
                            evRespuestaS += evRespuesta[j];
                            evRespuestaS += ",";
                        }
                    }
                    //alert(sevRespuestaS+" - "+erRespuestaS+" - "+evRespuestaS);
                    para[i] = {
                        "modo": "4",
                        "codigo_empresa": dataRespuesta[i].codigo_empresa,
                        "codigo_sucursal": dataRespuesta[i].codigo_sucursal,
                        "codigo_taller": dataRespuesta[i].codigo_taller,
                        "anio_tl06": dataRespuesta[i].anio_tl06,
                        "secuencia_orden_tl06": dataRespuesta[i].secuencia_orden_tl06,
                        "secuencia_tl43": dataRespuesta[i].secuencia_tl43,
                        "orden_presentacion": dataRespuesta[i].orden_presentacion,
                        "pregunta": dataRespuesta[i].pregunta,
                        "tipo_formulario": dataRespuesta[i].tipo_formulario,
                        "seccion_formulario": dataRespuesta[i].seccion_formulario,
                        "tipo_respuesta": dataRespuesta[i].tipo_respuesta,
                        "lista_respuesta": dataRespuesta[i].lista_respuesta,
                        "lista_etiquetas": dataRespuesta[i].lista_etiquetas,
                        "tipo_valor": dataRespuesta[i].tipo_valor,
                        "lista_valores": dataRespuesta[i].lista_valores,
                        "respuesta": dataRespuesta[i].respuesta,
                        "secuencia_vh65": dataRespuesta[i].secuencia_vh65,
                        "corden_seccion": dataRespuesta[i].corden_seccion,
                        "selRespt": dataRespuesta[i].selRespt,
                        "sevRespt": sevRespuestaS,
                        "erRespt": erRespuestaS,
                        "evRespt": evRespuestaS,
                        "nombre_seccion": dataRespuesta[i].nombre_seccion,
                        "imagen": dataRespuesta[i].imagen,
                        "observacion_calidad": "",
                        "observacion_calidad_csi": document.getElementById("observacionCCE").value,
                        "nombre_cliente_csi": document.getElementById("nombreEC").value,
                        "telefono_fijo_csi": document.getElementById("telefonoFI").value,
                        "telefono_movil_csi": document.getElementById("telefonCe").value
                    };
                    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(para[i]));
                }

            } catch (e) {
                alert("antessalio" + e);
            }

            var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl43PreguntasCalidadSet";
            //var Url = "http://localhost:4044" + "/Services/TL/Taller.svc/tl43PreguntasCalidadSet";
            //alert(Url);
            try {
                localStorage.removeItem("banderaEst");
            } catch (error) {

            }

            $.ajax({
                url: Url,
                type: "POST",
                data: JSON.stringify(para),
                async: false,
                dataType: "json",
                //Content-Type: application/json
                headers: {
                    'Content-Type': 'application/json;charset=UTF-8'
                },
                success: function (datas) {
                    if (datas.substr(0, 1) == "1") {
                        alert("se actualizaron los datos");
                        document.getElementById("btnGuadaCLC0").disabled = true;
                        localStorage.setItem("banderaEst", "ENTREGADO");
                        grabarFirmaDigital(document.getElementById("firmaOTCLC").value);
                        ConsultaControlCalidadCLC(JSON.parse(localStorage.getItem("dataItem")));


                    } else {
                        alert(datas.substr(2, datas.length - 2));
                        return;
                    }
                },
                error: function (err) {
                    alert("HOY" + inspeccionar(err));
                    alert("Error en servicio clientes");
                    return;
                } //alert(err);
            });
        }
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(inforUsa));

    } catch (e) {
        alert("salio" + e);
    }
}

function seleccionCCE(sele1, i, sele2, sn) {
    try {
        var cboMarcaHTML = "<p><select id='" + "SELE" + i + "'class='w3-input w3-border textos'>";
        for (var k = 0; k < sele1.length; k++) {
            if (sele2 == sele1[k].sele) {
                cboMarcaHTML += "<option  value='" + sele1[k].sele + "' selected>" + sele1[k].sele + "</option>";
            } else {
                cboMarcaHTML += "<option  value='" + sele1[k].sele + "'>" + sele1[k].sele + "</option>";
            }
        }
        cboMarcaHTML += "</select></p>";
        return cboMarcaHTML;
    } catch (e) {
        alert("selec" + e);
    }
}

function seleccionCCCAPR(sele1, i, sele2, sn) {
    try {
        var cboMarcaHTML = "";
        cboMarcaHTML += "<form name=fcolores>";

        for (var k = 0; k < sele1.length; k++) {
            if (sele2 == sele1[k].sele) {
                if (sele1[k].sele !== "Seleccione") {
                    cboMarcaHTML += "&nbsp;<input type='radio' id='" + sn + sele1[k].sele + i + "' name='" + "SEL" + i + "' value='" + sele1[k].sele + "' checked/><label for='" + sele1[k].sele + "'>&nbsp;" + sele1[k].sele + "</label>&nbsp;&nbsp;";
                }
            } else {
                if (sele1[k].sele !== "Seleccione") {
                    cboMarcaHTML += "&nbsp;<input type='radio' name='" + "SEL" + i + "' id='" + sn + sele1[k].sele + i + "' value='" + sele1[k].sele + "'/>" +
                        "<label for='" + sele1[k].sele + "'>&nbsp;" + sele1[k].sele + "</label>&nbsp;&nbsp;";

                }
            }
        }
        cboMarcaHTML += "</form>";
        //var numch = "SEL"+i;
        //   alert(numch);
        //var mycheck = document.getElementById(numch);
        //  alert(inspeccionar(mycheck));
        return cboMarcaHTML;
    } catch (e) {
        alert("selec" + e);
    }
}

function patrull(inforusa) {

    var UrlPat = localStorage.getItem("ls_url1").toLocaleString() + "/Services/VH/Vehiculos.svc/vh01VehiculoGet/" +
        "1,json;;;;;" + inforusa.chasis + ";;;;";
    var infME;
    var resp = "SI";
    $.ajax({
        url: UrlPat,
        type: "GET",
        async: false,
        dataType: "json",
        success: function (data) {
            try {
                infME = (JSON.parse(data.vh01VehiculoGetResult)).tvh01;   //(JSON.parse
                resp = infME[0].es_patrullero;
                /* if (infME[0].anio_ga62 == "0" && infME[0].secuencia_ga62 == "0") {
                    resp="NO";
                } */

            } catch (e) {
                alert(e);
                return;
            }
        },
        error: function (err) {
            alert("patrull" + err);
            return;
        }
    });
    return resp;
}

function encuestaOT(inforusa) {
    kendo.ui.progress($("#reporteCLCScreen"), true);
    setTimeout(function () {
        try {

            var patrullero = patrull(inforusa);
            if (patrullero == "SI") {
                var tipo_doc = "ENTREGA_TALLER_PATRU";
            } else {
                var tipo_doc = "ENTREGA_TALLER";
            }
            var respOT = "0";


            var UrlMotorEscape = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl43PreguntasCalidadGet/1,json;" +
                inforusa.codigo_empresa + ";" + inforusa.codigo_sucursal + ";" + inforusa.codigo_taller + ";" + inforusa.anio + ";" +
                inforusa.secuencia_orden + ";" + tipo_doc + ";;;;;;";
            var infME;

            $.ajax({
                url: UrlMotorEscape,
                type: "GET",
                async: false,
                dataType: "json",
                success: function (data) {
                    try {
                        infME = (JSON.parse(data.tl43PreguntasCalidadGetResult)).ttl43;   //(JSON.parse
                        localStorage.setItem("inforImprimePDF", JSON.stringify(infME));
                        /* window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(infME[1]));
                        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(infME[2]));
                        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(infME[3])); */
                        respOT = "1";
                    } catch (e) {
                        return;
                    }
                },
                error: function (err) {
                    alert(err);
                    return;
                }
            });


            if (respOT == "1") {
                try {

                    var obsrvaCCSI = infME[0].observacion_calidad_CSI;
                    var griddata = [];
                    tamano = infME.length;

                    for (var t = 0; t < infME.length; t++) {
                        if (infME[t].orden_presentacion < 10) {
                            infME[t].orden_presentacion = "0" + infME[t].orden_presentacion;
                        }
                    }
                    infME.sort(function myfunction(a, b) {
                        return (a.corden_seccion + "" + a.orden_presentacion) - (b.corden_seccion + "" + b.orden_presentacion)
                    });

                    for (var i = 0; i < infME.length; i++) {
                        var selecResp = new Array();
                        var sevResp = new Array();
                        var erResp = new Array();
                        var evResp = new Array();
                        //alert(infME[i].imagen);
                        if (infME[i].tipo_respuesta == "SELECCION" || infME[i].tipo_respuesta == "SEV") {
                            if (infME[i].tipo_respuesta == "SEV") {
                                //alert("etiqu"+infME[i].lista_etiquetas);
                                sevResp = infME[i].lista_etiquetas.split(",");
                                //alert(inspeccionar(sevResp));
                                var sevRes = [];
                                for (var k = 0; k < sevResp.length; k++) {
                                    sevRes.push({sevR: sevResp[k]});
                                    //alert(inspeccionar(sevRes[k]));
                                }
                                //alert(inspeccionar(sevRes[0]));
                            }
                            //alert("resp"+infME[i].lista_respuesta)
                            selecResp = infME[i].lista_respuesta.split(",");
                            var selRes = [];
                            selRes.push({sele: "Seleccione"});
                            for (var j = 0; j < selecResp.length; j++) {
                                selRes.push({sele: selecResp[j]});
                            }
                        } else {
                            if (infME[i].tipo_respuesta == "ER") {
                                //alert("else eti" + infME[i].lista_etiquetas);
                                erResp = infME[i].lista_etiquetas.split(",");
                                var erRes = [];
                                for (var l = 0; l < erResp.length; l++) {
                                    erRes.push({erR: erResp[l]});
                                }
                                //alert("else resp" + infME[i].lista_respuesta);
                                selecResp = infME[i].lista_respuesta.split(",");
                                var selRes = [];
                                selRes.push({sele: "Seleccione"});
                                for (var j = 0; j < selecResp.length; j++) {
                                    selRes.push({sele: selecResp[j]});
                                }
                            } else {
                                if (infME[i].tipo_respuesta == "EV") {
                                    //alert("else ev"+infME[i].lista_etiquetas);
                                    evResp = infME[i].lista_etiquetas.split(",");
                                    //alert((evResp.length));
                                    var evRes = [];
                                    for (var hh = 0; hh < evResp.length; hh++) {
                                        evRes.push({evR: evResp[hh]});
                                    }
                                }
                            }
                        }

                        griddata.push({
                            codigo_empresa: infME[i].codigo_empresa,
                            codigo_sucursal: infME[i].codigo_sucursal,
                            codigo_taller: infME[i].codigo_taller,
                            anio_tl06: infME[i].anio_tl06,
                            secuencia_orden_tl06: infME[i].secuencia_orden_tl06,
                            secuencia_tl43: infME[i].secuencia_tl43,
                            orden_presentacion: infME[i].orden_presentacion,
                            pregunta: infME[i].pregunta,
                            tipo_formulario: infME[i].tipo_formulario,
                            seccion_formulario: infME[i].seccion_formulario,
                            tipo_respuesta: infME[i].tipo_respuesta,
                            lista_respuesta: infME[i].lista_respuesta,
                            lista_etiquetas: infME[i].lista_etiquetas,
                            tipo_valor: infME[i].tipo_valor,
                            nombre_seccion: infME[i].nombre_seccion,
                            lista_valores: infME[i].lista_valores,
                            respuesta: infME[i].respuesta,
                            secuencia_vh65: infME[i].secuencia_vh65,
                            corden_seccion: infME[i].corden_seccion,
                            selResp: selRes,
                            sevResp: sevRes,
                            erResp: erRes,
                            evResp: evRes,
                            selRespt: infME[i].selRespt,
                            sevRespt: infME[i].sevRespt,
                            erRespt: infME[i].erRespt,
                            evRespt: infME[i].evRespt,
                            nombre_seccion: infME[i].nombre_seccion,
                            imagen: infME[i].imagen,
                            observacion_calidad_CSI: obsrvaCCSI,
                            tipo_objeto: infME[i].tipo_objeto
                        });
                        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(griddata[i]));
                    }
                } catch (e) {
                    alert("1" + e);
                }
                localStorage.setItem("griddataEC", JSON.stringify(griddata));

                tablaINP = cargatablaECCCET(griddata);

                //document.getElementById("tabstripEC").style.display = 'block';
                //desactivaTablaEC(false);
            }
        } catch (e) {
            kendo.ui.progress($("#reporteCLCScreen"), false);
            alert("212" + e);
            return;
        }

    }, 2000);
    kendo.ui.progress($("#reporteCLCScreen"), false);
    return tablaINP;
}

function ValidaCelularCC() {
    try {
        if (document.getElementById("telefonCe").value != "") {
            var result = /[09][0-9]{9}$/.test(document.getElementById("telefonCe").value);
            if (result == false) {
                document.getElementById("telefonCe").focus();
                document.getElementById("telefonCe").style.borderColor = "red";
            } else {
                document.getElementById("telefonCe").style.borderColor = "";
            }
        }
    } catch (f) {
        mens("Error validaci" + String.fromCharCode(243) + "n celular", "mens");
        return;
    }
}

function ValidaFijoCC() {
    try {
        if (document.getElementById("telefonoFI").value != "") {
            var result = /[0][0-9][0-9]{7}$/.test(document.getElementById("telefonoFI").value);
            if (result == false) {
                document.getElementById("telefonoFI").focus();
                document.getElementById("telefonoFI").style.borderColor = "red";
            } else {
                document.getElementById("telefonoFI").style.borderColor = "";
            }
        }
    } catch (f) {
        mens("Error validaci" + String.fromCharCode(243) + "n celular", "mens");
        return;
    }
}

function cargatablaECCCET(griddata1) {
    try {

        document.getElementById("encuestaOTCC").innerHTML = "";
        dataRespuesta = "";
        var j = 0;
        var tablaOT = "<table BORDER CELLPADDING=10 CELLSPACING=10 id='docu' scr>";
        tablaOT += "<tr id='tblEncuesta"+j+"'><td align='center' style='width: 50%;'><label style='font-weight:bold; font-size:13px;color:#ffffff'>CLIENTE CSI (CUSTOMER SATISFACTION INDEX)&emsp;&emsp;</label></td><td align='center'>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;</td></tr>";

        tablaOT += "<tr><td style='width: 100%;'>&nbsp;&nbsp;<label class='w3-text-black' style='font-weight:bold;" + "font-size:13px'>NOMBRE:</label></td>";
        tablaOT += "<td style='width: 100%;'><input id='nombreEC' type='text' class='w3-text-black' style='min-width:100%; font-weight:bold;" + "font-size:13px' value='" + JSON.parse(localStorage.getItem("dataItem")).nombre_cliente_csi + "'></input></td></tr>";

        tablaOT += "<tr><td style='width: 100%;'>&nbsp;&nbsp;<label class='w3-text-black' style='font-weight:bold;" + "font-size:13px'>CELULAR:</label></td>";
        tablaOT += "<td  style='width: 100%;'><input id='telefonCe'  type='number' class='w3-text-black' style='min-width:100%; font-weight:bold;" + "font-size:13px' value='" + JSON.parse(localStorage.getItem("dataItem")).celular_cliente + "' onblur='ValidaCelularCC();'></input></td></tr>";

        tablaOT += "<tr><td style='width: 100%;'>&nbsp;&nbsp;<label class='w3-text-black' style='font-weight:bold;" + "font-size:13px'>FIJO:</label></td>";
        tablaOT += "<td  style='width: 100%;'><input id='telefonoFI'  type='number' class='w3-text-black' style='min-width:100%; font-weight:bold;" + "font-size:13px' value='" + JSON.parse(localStorage.getItem("dataItem")).telefono_cliente + "'onblur='ValidaFijoCC();'></input></td></tr></table>";

        tablaOT += "<br><table BORDER CELLPADDING=10 CELLSPACING=10 id='documeE' scr>";
        var cuerpo;
        j=j+1;
        tablaOT += "<tr align='center'  id='tblEncuesta"+j+"'><td></td><td><label style='font-weight:bold; font-size:13px;color:#ffffff'></label></td></tr>";
        for (var i = 0; i < griddata1.length - 1; i++) {
            var seccionFor = griddata1[i].seccion_formulario;
            j=j+1
            tablaOT += "<tr  id='tblEncuesta"+j+"'><td align='center' style='width: 50%;'><label style='font-weight:bold; font-size:13px;color:#ffffff'>" + griddata1[i].nombre_seccion + "</label></td><td style='width: 25%;'></td></tr>";

            while (griddata1[i].seccion_formulario == seccionFor) {
                var tipoUsa = "";
                if (griddata1[i].tipo_valor == "NUMERICO") {
                    tipoUsa = "number";
                } else {
                    tipoUsa = "text";
                }
                tablaOT += "<tr><td>" + griddata1[i].orden_presentacion + "&nbsp;&nbsp;<label class='w3-text-black' style='font-weight:bold; font-size:13px'>" + griddata1[i].pregunta + "</label></td>";

                if (griddata1[i].tipo_respuesta == "SELECCION" || griddata1[i].tipo_respuesta == "SEV") {
                    var sevUsa = "<td><table style='width: 100%;'><tr>";
                    if (griddata1[i].tipo_respuesta == "SEV") {
                        var valorsev = griddata1[i].sevRespt.split(',');
                        /* window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(valorsev)); */
                        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(valorsev));
                        for (var l = 0; l < griddata1[i].sevResp.length; l++) {
                            if (valorsev[l] === "undefined" || valorsev[l] === null) {
                                valorsev[l] = " ";
                            }
                            /* sevUsa += "<td><p><input id=SEVE" + i + "" + l + " type='" + tipoUsa + "' style='width:100%' placeholder=" + (l + 1) + " value=" + valorsev[l] + "></p></td>"; */
                        }

                    } else {
                        /* if(griddata1[i].imagen != ""){
                            var pathCC = parametrosECPath();
                            var pathCC1 = pathCC["descripcion-2"] + griddata1[i].imagen;
                            sevUsa += "<td><div><center><img " + "src='"+pathCC1+"' style='border:1px"+
                            " solid black;'></center></div></td>";
                        } */
                    }
                    sevUsa += "</tr></table></td>";
                    //var selUsa = seleccionCCE(griddata1[i].selResp, i, griddata1[i].selRespt);

                    if (griddata1[i].tipo_objeto == "RADIO_BUTTON") {
                        var selUsa = seleccionCCCAPR(griddata1[i].selResp, i, griddata1[i].selRespt, "E");
                    } else {
                        var selUsa = seleccionCCE(griddata1[i].selResp, i, griddata1[i].selRespt, "E");
                    }

                    //tablaOT += sevUsa + "<td>" + selUsa + "</td>";
                    tablaOT += "<td>" + selUsa + "</td>";
                } else {
                    if (griddata1[i].tipo_respuesta == "ER") {
                        var sevUsa = "<td><table style='width: 100%;'><tr>";
                        var valorer = griddata1[i].erRespt.split(',');

                        for (var l = 0; l < griddata1[i].erResp.length; l++) {
                            if (valorer[l] === undefined || valorer[l] === null || valorer[l] === "") {
                                valorer[l] = " ";
                            }
                            /* sevUsa += "<td><p><input id=ERE" + i + "" + l + " type='text' size='2' placeholder=" + griddata1[i].erResp[l].erR + " value=" + valorer[l] + "></p></td>"; */
                        }
                        sevUsa += "</tr></table></td>";
                        //var selUsa = seleccionCCE(griddata1[i].selResp, i, griddata1[i].selRespt);
                        if (griddata1[i].tipo_objeto == "RADIO_BUTTON") {
                            var selUsa = seleccionCCCAPR(griddata1[i].selResp, i, griddata1[i].selRespt, "E");
                        } else {
                            var selUsa = seleccionCCE(griddata1[i].selResp, i, griddata1[i].selRespt, "E");
                        }
                        tablaOT += "<td>" + selUsa + "</td>";
                    } else {
                        if (griddata1[i].tipo_respuesta == "EV") {
                            var sevUsa = "<td><table style='width: 100%;'><tr>";
                            var valorev = griddata1[i].evRespt.split(',');

                            for (var l = 0; l < griddata1[i].evResp.length; l++) {
                                if (valorev[l] === "undefined" || valorev[l] === null) {
                                    valorev[l] = " ";
                                }
                                sevUsa += "<td><p><input id=EVE" + i + "" + l + " type='text'  placeholder=" + griddata1[i].evResp[l].evR + " value=" + valorev[l] + "></p></td>";
                            }
                            sevUsa += "</tr></table></td>";
                            tablaOT += sevUsa; //+ "<td></td>";
                        }
                    }
                }
                tablaOT += "</tr>";
                i++;
                if (i == tamano) {
                    break;
                }
            }
            i--;
        }
        tablaOT += "</table>";

        document.getElementById("encuestaOTCC").innerHTML = " ";
        document.getElementById("encuestaOTCC").innerHTML = tablaOT;
        document.getElementById("encuestaOTCC").style.display = 'block';
        localStorage.setItem("griddataEncuestaCC", JSON.stringify(griddata1));
        dataRespuesta = griddata1;
        llamarNuevoestilo("tblEncuesta");
        return tablaOT;
    } catch (e) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>11116 ERROR</center>", inspeccionar(griddata1[i]));
        alert("carga" + i + e);

    }
}

function actividadesOT(dataItem) {
    var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/ga/Garantias.svc/ga64RevisionGet/1," + localStorage.getItem("ls_idempresa").toLocaleString() + ";" + dataItem.tipo_mantenimiento + ";";
    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", Url);
    var infor;
    $.ajax({
        url: Url,
        type: "GET",
        async: false,
        dataType: "json",
        success: function (data) {
            try {
                infor = (JSON.parse(data.ga64RevisionGetResult));
            } catch (e) {
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>38 ERROR</center>", e);
                return;
            }
        },
        error: function (err) {
            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>39 ERROR</center>", err);
            return;
        }
    });
    document.getElementById("actividadesOT").innerHTML = "";
    try {
        kendo.ui.progress($("#reporteCLCScreen"), true);
        setTimeout(function () {
            var htmlActividad = "&nbsp;&nbsp;&nbsp;&nbsp;<table width='100%' cellpadding='0' cellspacing='0' border='0px'>" +
                "<tr style='border:1px solid'><td><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;CODIGO:&nbsp;&nbsp;</label></td>" +
                "<td><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;ACTIVIDAD:&nbsp;&nbsp;</label></td></tr>";
            for (let index = 0; index < infor.length; index++) {
                htmlActividad += "<tr style='border-bottom:1px solid'><td style='width: 25%;'>" +
                    "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + infor[index].CodigoClase + "&nbsp;&nbsp;</label>" +
                    "</td><td style='width: 100%;'><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + infor[index].NombreClase + "&nbsp;&nbsp;</label></td></tr>";
            }
            document.getElementById("actividadesOT").innerHTML = htmlActividad + "</table>";
        }, 1000);
    } catch (error) {
        alert("actividadesOT" + error);
    }

}

/*--------------------------------------------------------------------
Fecha: 15/02/2019
Descripcion: Presenta datos de la OT
Parametros:
--------------------------------------------------------------------*/
function datosot(dataItem) {
    document.getElementById("datosOT").innerHTML = "";
    try {
        var kilm3 = new Intl.NumberFormat("de-DE", {maximumSignificantDigits: 3}).format(dataItem.kilometraje);

        kendo.ui.progress($("#reporteCLCScreen"), true);
        setTimeout(function () {
            document.getElementById("datosOT").innerHTML = "&nbsp;&nbsp;&nbsp;&nbsp;<table width='100%' cellpadding='0' cellspacing='0' border='0px'>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<br><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;SECCION:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.seccion_orden_trabajo + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<br><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;TIPO DE TRABAJO:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.tipo_trabajo + " - " + dataItem.nombre_tipo_trabajo + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<br><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;TIPO DE MANTENIMIENTO:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.tipo_mantenimiento + " - " + dataItem.nombre_mantenimiento + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<br><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;KILOMETRAJE:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + kilm3 + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<br><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;REPARACIONES SOLICITADAS:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.reparaciones_solicitadas + "&nbsp;&nbsp;</label>" +
                "</td></tr>" +
                "<tr style='border-bottom:1px solid'><td>" +
                "<br><label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;OBSERVACIONES:&nbsp;&nbsp;</label>" +
                "</td><td>" +
                "<label style='font-weight:bold; font-size:13px;color:black'>&nbsp;&nbsp;" + dataItem.observaciones + "&nbsp;&nbsp;</label>" +
                "</td></tr></table>";
        }, 1000);
    } catch (error) {
        alert("datosot" + error);
    }

}

/*--------------------------------------------------------------------
Fecha: 15/02/2019
Descripcion: Presenta el detalle de la OT
Parametros: URL del servicio
--------------------------------------------------------------------*/
function infoDetalleOTCLCENCU(dataItem) {
    kendo.ui.progress($("#reporteCLCScreen"), true);
    setTimeout(function () {
        // precarga ***********************
        try {
            $("#imgFirmaOTCC").empty();
        } catch (error) {

        }
        var descri = (parseInt(localStorage.getItem("ls_dimensionW").toLocaleString()) * 30) / 100;
        var cant = (parseInt(localStorage.getItem("ls_dimensionW").toLocaleString()) * 7) / 100;
        var cla = (parseInt(localStorage.getItem("ls_dimensionW").toLocaleString()) * 15) / 100;

        var urlDot = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl07DetalleOTGet/" + dataItem.codigo_empresa + "," + dataItem.anio + "," + dataItem.secuencia_orden + ",," + localStorage.getItem("ls_ussucursal").toLocaleString() + "," + localStorage.getItem("ls_usagencia").toLocaleString();

        /* var urlDot = "http://localhost:4044/Services/TL/Taller.svc/tl07DetalleOTGet/" + dataItem.codigo_empresa + "," + dataItem.anio + "," + dataItem.secuencia_orden + ",," + localStorage.getItem("ls_ussucursal").toLocaleString() + "," + localStorage.getItem("ls_usagencia").toLocaleString(); */
        var UrlDetalleOT = urlDot;

        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", UrlDetalleOT);

        var infordetEVCC = "";
        $.ajax({
            url: UrlDetalleOT,
            type: "GET",
            async: false,
            dataType: "json",
            success: function (data) {
                try {
                    if (inspeccionar(data).includes("DetalleOT") == true && data !== null) {
                        try {
                            infordetEVCC = (JSON.parse(data.tl07DetalleOTGetResult)).DetalleOT;
                        } catch (error) {

                        }

                        if (inspeccionar(infordetEVCC).length > 0) {
                            $("#imgFirmaOTCC").kendoGrid({
                                dataSource: {
                                    pageSize: 20,
                                    data: infordetEVCC,
                                    aggregate: [
                                        {field: "descripcion", aggregate: "count", width: descri},
                                        {field: "cantidad", aggregate: "sum", width: cant},
                                        {field: "subtotal", aggregate: "sum", width: cant}
                                    ]
                                },
                                pageable: false,
                                columns: [
                                    {
                                        field: "descripcion",
                                        title: "Descripci&oacute;n",
                                        footerTemplate: "TOTAL:",
                                        width: descri
                                    },
                                    {field: "clase_registro", title: "Clase Registro", width: cla},
                                    {field: "cantidad", title: "Cantidad", width: cant},
                                    {
                                        field: "subtotal",
                                        format: "{0:c}",
                                        title: "Total",
                                        footerTemplate: "#= kendo.toString(sum, '$0.00') #",
                                        width: cant
                                    }
                                ]
                            });

                            kendo.ui.progress($("#reporteCLCScreen"), false);
                        } else {
                            kendo.ui.progress($("#reporteCLCScreen"), false);
                            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "<center>No existe el detalle de la<br/>Orden de Trabajo <b>" + dataItem.numero_orden + "</b></center>");
                            //   kendo.mobile.application.navigate("components/admOt/view.html");
                        }

                    } else {
                        kendo.ui.progress($("#reporteCLCScreen"), false);
                        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "<center>No existe el detalle de la<br/>Orden de Trabajo <b>" + dataItem.numero_orden + "</b></center>");
                        //    kendo.mobile.application.navigate("components/admOt/view.html");
                    }

                } catch (e) {
                    kendo.ui.progress($("#reporteCLCScreen"), false);
                    // window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "No existe el detalle de la Orden de Trabajo");
                    return;
                }
            },
            error: function (err) {
                kendo.ui.progress($("#reporteCLCScreen"), false);
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>111 ERROR</center>", err);
                return;
            }
        });

        // precarga ***********************
    }, 1000);

}

function guardarCLC() {
    kendo.ui.progress($("#reporteCLCScreen"), true);
    setTimeout(function () {
        //var firmaccE = document.getElementById("titFirmaPregCLC").innerText.trim();
        generarOrdenPDFCLCEN(JSON.parse(localStorage.getItem("dataItem")).numero_orden, "O");
        /* if(firmaccE == "Desea registrar la firma del cliente ?"){
            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "Debe Registrar Firma Cliente");

        }else{ */
        guardarCCETFIN();
        //}
        kendo.ui.progress($("#reporteCLCScreen"), false);
    }, 2000);

}

function validaTablaCLC() {
    var dataRespuesta = JSON.parse(localStorage.getItem("griddataCLC"));
    try {
        var bandera = 0;
        var pregunta = "";
        for (var i = 0; i <= dataRespuesta.length - 1; i++) {
            if (dataRespuesta[i].tipo_respuesta == "SELECCION" || dataRespuesta[i].tipo_respuesta == "SEV") {
                if (dataRespuesta[i].tipo_respuesta == "SEV") {
                    for (var l = 0; l < dataRespuesta[i].sevResp.length; l++) {
                        if (document.getElementById("SEV" + i + "" + l) == null) {
                            bandera = 1;
                            pregunta = dataRespuesta[i].pregunta.toLocaleString();
                            return bandera + ";" + pregunta;
                        }
                    }
                }
                if (document.getElementById("SEL" + i).value == "Seleccione") {
                    bandera = 1;
                    pregunta = dataRespuesta[i].pregunta.toLocaleString();
                    return bandera + ";" + pregunta;
                }

            }
            if (dataRespuesta[i].tipo_respuesta == "ER") {
                for (var k = 0; k < dataRespuesta[i].erResp.length; k++) {
                    if (document.getElementById("ER" + i + "" + k) == null) {
                        bandera = 1;
                        pregunta = dataRespuesta[i].pregunta.toLocaleString();
                        return bandera + ";" + pregunta;
                    }
                }
                if (document.getElementById("SEL" + i).value == "Seleccione") {
                    bandera = 1;
                    pregunta = dataRespuesta[i].pregunta.toLocaleString();
                    return bandera + ";" + pregunta;
                }

            }
            if (dataRespuesta[i].tipo_respuesta == "EV") {

                for (var m = 0; m < dataRespuesta[i].evResp.length; m++) {
                    if (document.getElementById("EV" + i + "" + m) == null) {
                        bandera = 1;
                        pregunta = dataRespuesta[i].pregunta.toLocaleString();
                        return bandera + ";" + pregunta;
                    }
                }
            }

        }
        return bandera + ";" + pregunta
    } catch (e) {
        alert("dis2" + e);
    }
}

function desactivaTablaCLC(sn) {
    var dataRespuesta = JSON.parse(localStorage.getItem("griddataCLC"));
    try {
        for (var i = 0; i <= dataRespuesta.length - 1; i++) {
            if (dataRespuesta[i].tipo_respuesta == "SELECCION" || dataRespuesta[i].tipo_respuesta == "SEV") {
                if (dataRespuesta[i].tipo_respuesta == "SEV") {
                    for (var l = 0; l < dataRespuesta[i].sevResp.length; l++) {
                        if (document.getElementById("SEV" + i + "" + l) != null) {
                            document.getElementById("SEV" + i + "" + l).disabled = sn;
                        }
                    }
                }
                var j = 0;
                while (j < dataRespuesta[i].selResp.length) {
                    if (dataRespuesta[i].selResp[j].sele !== "Seleccione") {
                        if (dataRespuesta[i].tipo_objeto == "RADIO_BUTTON") {
                            document.getElementById("D" + dataRespuesta[i].selResp[j].sele + i).disabled = sn;
                        } else {
                            document.getElementById("SEL" + i).disabled = sn;
                            break;
                        }

                    }
                    j += 1;
                }
                //document.getElementById("SEL" + i).disabled = sn;
            }
            if (dataRespuesta[i].tipo_respuesta == "ER") {
                for (var k = 0; k < dataRespuesta[i].erResp.length; k++) {
                    if (document.getElementById("ER" + i + "" + k) != null) {
                        document.getElementById("ER" + i + "" + k).disabled = sn;
                    }
                }
                var j = 0;
                while (j < dataRespuesta[i].selResp.length) {
                    if (dataRespuesta[i].selResp[j].sele !== "Seleccione") {
                        document.getElementById(dataRespuesta[i].selResp[j].sele + i).disabled = sn;
                    }
                    j += 1;
                }
                //document.getElementById("SEL" + i).disabled = sn;
            }
            if (dataRespuesta[i].tipo_respuesta == "EV") {

                for (var m = 0; m < dataRespuesta[i].evResp.length; m++) {
                    if (document.getElementById("EV" + i + "" + m) != null) {
                        document.getElementById("EV" + i + "" + m).disabled = sn;
                    }
                }
            }

        }
        document.getElementById("observacionCLC").disabled = sn;

        //document.getElementById("btnEstadoCLC").disabled = sn;

        if (JSON.parse(localStorage.getItem("dataItem")).estado_entrega == "ENTREGADO") {
            document.getElementById("btnGuadaCLC0").disabled = sn;
        }
    } catch (e) {
        alert("dis1CA" + e);
    }
}


/*--------------------------------------------------------------------
Fecha: 06/09/2018
Descripcion: Presenta dialog para ingresar la firma electronica
--------------------------------------------------------------------*/
function firmaDigitalOTCLC() {
    kendo.ui.progress($("#reporteCLCScreen"), true);
    setTimeout(function () {
        // OT - FIRMA
        try {
            var dialogfirmaDigitalOTCC = $("#dialogfirmaDigitalOTCLC").data("kendoDialog");
            dialogfirmaDigitalOTCC.close();
        } catch (ed010) {
        }
        var htmlFirmaOTCC = "<table align='center' width='100%' style='border-bottom:1px solid'>" +
            "<tr>" +
            "<td>" +
            "<div id='idwrapOTCLC' class='wrapper'><canvas id='signature-padCC' width='400' height='300'></canvas></div>" +
            "</td>" +
            "</tr>" +
            "</table>";

        dialogfirmaDigitalOTCC = $("#dialogfirmaDigitalOTCLC").kendoDialog({
            width: "450px",
            buttonLayout: "normal",
            title: "INGRESE LA FIRMAS",
            closable: false,
            modal: false,
            content: htmlFirmaOTCC,
            actions: [
                {
                    text: '<font style=\"font-size:12px\"> <button id="btnFirmasAprovacion0" class=\"w3-btn\"> &nbsp;&nbsp;CONFIRMAR&nbsp;&nbsp;</button></font>',
                    action: accFirmConfCLC
                },
                {
                    text: '<font style=\"font-size:12px\"><button id="btnFirmasAprovacion1" class=\"w3-btn\"> CANCELAR</button></font>',
                    primary: true
                }
            ]
        });
        dialogfirmaDigitalOTCC.data("kendoDialog").open();
        llamarNuevoestilo("btnFirmasAprovacion");
        canvas = document.getElementById('signature-padCC');
        signaturePad = new SignaturePad(canvas, {
            minWidth: 0.5,
            maxWidth: 0.5,
            dotSize: 1,
            backgroundColor: 'rgb(255, 255, 255)'
        });
        kendo.ui.progress($("#reporteCLCScreen"), false);
    }, 2000);
}


/*--------------------------------------------------------------------
Fecha: 06/09/2018
Descripcion: Registra la firma en una imagen
--------------------------------------------------------------------*/
function accFirmConfAS() {
    if (signaturePad.isEmpty()) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "<center>Ingrese la firma correspondiente</center>");
        return;
    }

    document.getElementById("firmaOTCCA").value = "";
    document.getElementById("imgFirmaOTCCA").innerHTML = "";

    var data = signaturePad.toDataURL('image/jpeg');
    document.getElementById("firmaOTCCA").value = data;

    signaturePad.clear();

    // OT - FIRMA
    try {
        var dialogfirmaOTAS = $("#dialogfirmaOTCCA").data("kendoDialog");
        dialogfirmaOTAS.close();
    } catch (ed010) {
    }

    document.getElementById("imgFirmaOTCCA").innerHTML = "<img src='" + document.getElementById("firmaOTCCA").value + "' width='400' height='300'>";
}

/*--------------------------------------------------------------------
Fecha: 06/09/2018
Descripcion: Registra la firma en una imagen
--------------------------------------------------------------------*/
function accFirmConfCLC() {
    if (signaturePad.isEmpty()) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", "<center>Ingrese la firma correspondiente</center>");
        return;
    }

    document.getElementById("firmaOTCLC").value = "";
    document.getElementById("imgFirmaOTCLC").innerHTML = "";

    var data = signaturePad.toDataURL('image/jpeg');
    document.getElementById("firmaOTCLC").value = data;
    signaturePad.clear();

    // OT - FIRMA
    try {
        var dialogfirmaDigitalOTCC = $("#dialogfirmaDigitalOTCC").data("kendoDialog");
        dialogfirmaDigitalOTCC.close();
    } catch (ed010) {
    }
    document.getElementById("imgFirmaOTCLC").innerHTML = "<img src='" + document.getElementById("firmaOTCLC").value + "' width='400' height='300'>";
}

function imprimeAP() {
    formatoImpConCalAP(false);
}

var tablaINP = "";

function ConsultaControlCalidadCLC(inforusa) {
    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", inspeccionar(inforusa));
    kendo.ui.progress($("#reporteCLCScreen"), true);
    setTimeout(function () {
        try {
            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(inforusa));
            var tabstripCCL = $("#tabstripCLC").kendoTabStrip().data("kendoTabStrip");
            tabstripCCL.select(0);
            var respOT = "0";
            var tipo_doc = "CONTROL_CALIDAD";

            if (inforusa.chasis.substr(7, 1) == "E") {
                tipo_doc = "CONTROL_CALIDAD_ELEC"
            }
            var UrlMotorEscape = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl43PreguntasCalidadGet/1,json;" +
                inforusa.codigo_empresa + ";" + inforusa.codigo_sucursal + ";" + inforusa.codigo_taller + ";" + inforusa.anio + ";" +
                inforusa.secuencia_orden + ";" + tipo_doc + ";;;;;;";
            var infME;
            document.getElementById("firmaGrafCLC").value = "";
            document.getElementById("imgFirmaOTCLC").innerHTML = "";

            document.getElementById("confirmFirmaOTCLC0").style.display = 'initial';
            document.getElementById("confirmFirmaOTCLC1").style.display = 'initial';
            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>0 ALERTA</center>", UrlMotorEscape);
            $.ajax({
                url: UrlMotorEscape,
                type: "GET",
                async: false,
                dataType: "json",
                success: function (data) {
                    /* window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>0 ALERTA</center>", inspeccionar(data)); */
                    try {
                        infME = (JSON.parse(data.tl43PreguntasCalidadGetResult)).ttl43;   //(JSON.parse(data.ComboParametroEmpGetResult));
                        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>0 ALERTA</center>", inspeccionar(infME[0]));
                        localStorage.setItem("inforImprimePDF1", JSON.stringify(infME));
                        respOT = "1";
                    } catch (e) {
                        return;
                    }
                },
                error: function (err) {
                    alert(err);
                    return;
                }
            });
            var obsrvaCC = infME[0].observacion_calidad;
            if (respOT == "1") {
                try {
                    var griddata = [];
                    tamano = infME.length;

                    for (var t = 0; t < infME.length; t++) {
                        if (infME[t].orden_presentacion < 10) {
                            infME[t].orden_presentacion = "0" + infME[t].orden_presentacion;
                        }
                    }
                    infME.sort(function myfunction(a, b) {
                        return (a.corden_seccion + "" + a.orden_presentacion) - (b.corden_seccion + "" + b.orden_presentacion)
                    });

                    for (var i = 0; i < infME.length; i++) {
                        var selecResp = new Array();
                        var sevResp = new Array();
                        var erResp = new Array();
                        var evResp = new Array();
                        //alert(infME[i].imagen);
                        if (infME[i].tipo_respuesta == "SELECCION" || infME[i].tipo_respuesta == "SEV") {
                            if (infME[i].tipo_respuesta == "SEV") {
                                //alert("etiqu"+infME[i].lista_etiquetas);
                                sevResp = infME[i].lista_etiquetas.split(",");
                                //alert(inspeccionar(sevResp));
                                var sevRes = [];
                                for (var k = 0; k < sevResp.length; k++) {
                                    sevRes.push({sevR: sevResp[k]});
                                    //alert(inspeccionar(sevRes[k]));
                                }
                                //alert(inspeccionar(sevRes[0]));
                            }
                            //alert("resp"+infME[i].lista_respuesta)
                            selecResp = infME[i].lista_respuesta.split(",");

                            var selRes = [];
                            selRes.push({sele: "Seleccione"});
                            for (var j = 0; j < selecResp.length; j++) {
                                selRes.push({sele: selecResp[j]});
                            }

                        } else {
                            if (infME[i].tipo_respuesta == "ER") {
                                //alert("else eti" + infME[i].lista_etiquetas);
                                erResp = infME[i].lista_etiquetas.split(",");
                                var erRes = [];
                                for (var l = 0; l < erResp.length; l++) {
                                    erRes.push({erR: erResp[l]});
                                }
                                //alert("else resp" + infME[i].lista_respuesta);
                                selecResp = infME[i].lista_respuesta.split(",");
                                var selRes = [];
                                selRes.push({sele: "Seleccione"});
                                for (var j = 0; j < selecResp.length; j++) {
                                    selRes.push({sele: selecResp[j]});
                                }
                            } else {
                                if (infME[i].tipo_respuesta == "EV") {
                                    //alert("else ev"+infME[i].lista_etiquetas);
                                    evResp = infME[i].lista_etiquetas.split(",");
                                    //alert((evResp.length));
                                    var evRes = [];
                                    for (var hh = 0; hh < evResp.length; hh++) {
                                        evRes.push({evR: evResp[hh]});
                                    }
                                    //alert("0:" + evRes[0].evR);
                                    //alert("1:" + evRes[1].evR);
                                    //alert("2:" + evRes[2].evR);
                                    //alert("3:" + evRes[3].evR);
                                }
                            }
                        }
                        griddata.push({
                            codigo_empresa: infME[i].codigo_empresa,
                            codigo_sucursal: infME[i].codigo_sucursal,
                            codigo_taller: infME[i].codigo_taller,
                            anio_tl06: infME[i].anio_tl06,
                            secuencia_orden_tl06: infME[i].secuencia_orden_tl06,
                            secuencia_tl43: infME[i].secuencia_tl43,
                            orden_presentacion: infME[i].orden_presentacion,
                            pregunta: infME[i].pregunta,
                            tipo_formulario: infME[i].tipo_formulario,
                            seccion_formulario: infME[i].seccion_formulario,
                            tipo_respuesta: infME[i].tipo_respuesta,
                            lista_respuesta: infME[i].lista_respuesta,
                            lista_etiquetas: infME[i].lista_etiquetas,
                            tipo_valor: infME[i].tipo_valor,
                            nombre_seccion: infME[i].nombre_seccion,
                            lista_valores: infME[i].lista_valores,
                            respuesta: infME[i].respuesta,
                            secuencia_vh65: infME[i].secuencia_vh65,
                            corden_seccion: infME[i].corden_seccion,
                            selResp: selRes,
                            sevResp: sevRes,
                            erResp: erRes,
                            evResp: evRes,
                            selRespt: infME[i].selRespt,
                            sevRespt: infME[i].sevRespt,
                            erRespt: infME[i].erRespt,
                            evRespt: infME[i].evRespt,
                            nombre_seccion: infME[i].nombre_seccion,
                            imagen: infME[i].imagen,
                            observacion_calidad: obsrvaCC,
                            tipo_objeto: infME[i].tipo_objeto
                        });

                        //if (infME[i].tipo_respuesta == "EV") { alert(griddata[i].evResp[0].evR); alert(griddata[i].evResp[1].evR); alert(griddata[i].evResp[2].evR); alert(griddata[i].evResp[3].evR); }
                        //alert("sevRespt" + infME[i].sevRespt);
                        //alert("sevRes" + inspeccionar(sevRes));
                        //alert("sevResp"+inspeccionar(griddata[i].sevResp));
                        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", inspeccionar(griddata[i].sevResp));

                    }
                } catch (e) {
                    alert("1" + e);
                }
                localStorage.setItem("griddataCLC", JSON.stringify(griddata));
                tablaINP = cargatablaCLC(griddata);
                document.getElementById("observacionCLC").value = griddata[0].observacion_calidad;
                var registroME = "";
                generarOrdenPDFCLCEN(inforusa.numero_orden, "O");
                document.getElementById("tabstripCLC").style.display = 'block';

                if (inforusa.estado_calidad == "FINALIZADO" || inforusa.estado_calidad == "") {
                    desactivaTablaCLC(true);
                }
                if (JSON.parse(localStorage.getItem("dataItem")).estado_entrega == "ENTREGADO") {
                    document.getElementById("btnGuadaCLC0").disabled = true;
                }
            }
        } catch (e) {
            alert("213" + e);
            return;
        }

        // precarga *********************************************************************************************
    }, 2000);
    kendo.ui.progress($("#reporteCLCScreen"), false);
    /* if (inforusa.estado_calidad == "REVISADO" && inforusa.estado_calidad !== "") {
        desactivaTablaCLC(true);
    } */
    return tablaINP;
}

function parametrosCLCPath() {
    try {
        var accResp = "";
        //http://192.168.1.50:8089/concesionario/Services/TG/Parametros.svc/ParametroEmpGet/6,;TG;APP_ENTREGAS;;PATH_ENTREGAS
        var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TG/Parametros.svc/ParametroEmpGet/6,;TL;PATH_CONTROL_CALIDAD;;PATH_CONTROL_CALIDAD";
        var respPar;
        $.ajax({
            url: Url,
            type: "GET",
            dataType: "json",
            async: false,
            success: function (data) {
                try {
                    accResp = JSON.parse(data.ParametroEmpGetResult).tmpParamEmp;
                    respPar = accResp[0];
                } catch (e) {
                    window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>36 ERROR</center>", "parametrosCLCPath" + inspeccionar(e));
                    respPar = "error";
                }
            },
            error: function (err) {
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>37 ERROR</center>", inspeccionar(err));
                respPar = "error";
            }
        });
        return respPar;
    } catch (f) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>38 ERROR</center>", f);
        return "error";
    }
}

function cargatablaCLC(griddata1) {
    try {
        document.getElementById("descSMS_1RCLC").innerHTML = "";
        dataRespuesta = "";
        var tablaOT = "";

        tablaOT = "<table BORDER CELLPADDING=10 CELLSPACING=10 id='docume' scr>";
        var cuerpo;
        var j=0
        tablaOT += "<tr align='center'  id='tblPregunta"+j+"'><td></td><td><label style='font-weight:bold; font-size:13px;color:#ffffff'></label></td><td></td></tr>";
        for (var i = 0; i < griddata1.length - 1; i++) {
            j=j+1;
            var seccionFor = griddata1[i].seccion_formulario;
            tablaOT += "<tr id='tblPregunta"+j+"'><td align='center' style='width: 65%;'><label style='font-weight:bold; font-size:13px;color:#ffffff'>" + griddata1[i].nombre_seccion + "</label></td><td style='width: 10%;'></td><td style='width: 25%;'></td></tr>";
            while (griddata1[i].seccion_formulario == seccionFor) {
                var tipoUsa = "";
                if (griddata1[i].tipo_valor == "NUMERICO") {
                    tipoUsa = "number";
                } else {
                    tipoUsa = "text";
                }
                tablaOT += "<tr><td>" + griddata1[i].orden_presentacion + "&nbsp;&nbsp;<label class='w3-text-black' style='font-weight:bold; font-size:13px'>" + griddata1[i].pregunta + "</label></td>";
                if (griddata1[i].tipo_respuesta == "SELECCION" || griddata1[i].tipo_respuesta == "SEV") {
                    var sevUsa = "<td><table style='width: 100%;'><tr>";
                    if (griddata1[i].tipo_respuesta == "SEV") {
                        var valorsev = griddata1[i].sevRespt.split(',');
                        /* window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(valorsev)); */
                        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(valorsev));
                        for (var l = 0; l < griddata1[i].sevResp.length; l++) {
                            if (valorsev[l] === "undefined" || valorsev[l] === null) {
                                valorsev[l] = " ";
                            }
                            /* sevUsa += "<td><p><input id=SEV" + i + "" + l + " type='" + tipoUsa + "' style='width:100%' placeholder=" + (l + 1) + " value=" + valorsev[l] + " disabled = 'true'></p></td>"; */
                        }
                        //alert(sevUsa);
                    } else {
                        /* if(griddata1[i].imagen != ""){
                            var pathCC = parametrosCLCPath();
                            var pathCC1 = pathCC["descripcion-2"] + griddata1[i].imagen;
                            sevUsa += "<td><div><center><img " + "src='"+pathCC1+"' style='border:1px"+
                            " solid black;'></center></div></td>";
                        } */
                    }
                    if (griddata1[i].imagen != "" && griddata1[i].tipo_respuesta == "SELECCION") {
                        //alert(i+" - "+griddata1[i].imagen);
                        var pathCC = parametrosCCPath();
                        var pathCC1 = pathCC["descripcion-2"] + griddata1[i].imagen;
                        sevUsa += "<td><div><center><img " + "src='" + pathCC1 + "' style='border:1px" +
                            " solid black;'></center></div></td>";
                    }
                    sevUsa += "</tr></table></td>";
                    if (griddata1[i].tipo_objeto == "RADIO_BUTTON") {
                        var selUsa = seleccionCCCAPR(griddata1[i].selResp, i, griddata1[i].selRespt, "D");
                    } else {
                        var selUsa = seleccionCLC(griddata1[i].selResp, i, griddata1[i].selRespt, "D");
                    }

                    tablaOT += sevUsa + "<td>" + selUsa + "</td>";
                } else {
                    if (griddata1[i].tipo_respuesta == "ER") {
                        var sevUsa = "<td><table style='width: 100%;'><tr>";
                        var valorer = griddata1[i].erRespt.split(',');

                        for (var l = 0; l < griddata1[i].erResp.length; l++) {
                            if (valorer[l] === undefined || valorer[l] === null || valorer[l] === "") {
                                valorer[l] = " ";
                            }
                            sevUsa += "<td><p><input id=ER" + i + "" + l + " type='text' size='2' placeholder=" + griddata1[i].erResp[l].erR + " value=" + valorer[l] + " disabled = 'true'></p></td>";
                        }
                        if (griddata1[i].imagen != "" && griddata1[i].tipo_respuesta == "ER") {
                            var pathCC = parametrosCCPath();
                            var pathCC1 = pathCC["descripcion-2"] + griddata1[i].imagen;
                            sevUsa += "<td><div><center><img " + "src='" + pathCC1 + "' style='border:1px" +
                                " solid black;'></center></div></td>";
                        }
                        sevUsa += "</tr></table></td>";
                        if (griddata1[i].tipo_objeto == "RADIO_BUTTON") {
                            var selUsa = seleccionCCCAPR(griddata1[i].selResp, i, griddata1[i].selRespt, "D");
                        } else {
                            var selUsa = seleccionCLC(griddata1[i].selResp, i, griddata1[i].selRespt, "D");
                        }
                        tablaOT += sevUsa + "<td>" + selUsa + "</td>";
                    } else {
                        if (griddata1[i].tipo_respuesta == "EV") {
                            var sevUsa = "<td><table style='width: 100%;'><tr>";
                            var valorev = griddata1[i].evRespt.split(',');
                            for (var l = 0; l < griddata1[i].evResp.length; l++) {
                                if (valorev[l] === "undefined" || valorev[l] === null) {
                                    valorev[l] = " ";
                                }
                                sevUsa += "<td><p><input id=EV" + i + "" + l + " type='text'  placeholder=" + griddata1[i].evResp[l].evR + " value=" + valorev[l] + " disabled = 'true'></p></td>";
                            }
                            if (griddata1[i].imagen != "" && griddata1[i].tipo_respuesta == "EV") {
                                var pathCC = parametrosCCPath();
                                var pathCC1 = pathCC["descripcion-2"] + griddata1[i].imagen;
                                sevUsa += "<td><div><center><img " + "src='" + pathCC1 + "' style='border:1px" +
                                    " solid black;'></center></div></td>";
                            }
                            sevUsa += "</tr></table></td>";
                            tablaOT += sevUsa + "<td></td>";
                        }
                    }
                }
                tablaOT += "</tr>";
                i++;
                if (i == tamano) {
                    break;
                }
            }
            i--;
        }
        tablaOT += "</table>";
        tablaOT += observa;
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", tablaOT);
        document.getElementById("descSMS_1RCLC").innerHTML = " ";
        document.getElementById("descSMS_1RCLC").innerHTML = tablaOT;
        document.getElementById("descSMS_1RCLC").style.display = 'block';
        localStorage.setItem("griddataLC", JSON.stringify(griddata1));
        dataRespuesta = griddata1;
        llamarNuevoestilo("tblPregunta");
        return tablaOT;
    } catch (e) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>1112 ERROR</center>", inspeccionar(griddata1[i]));
        alert("carga" + i + e);

    }
}

function seleccionCLC(sele1, i, sele2) {
    try {
        var cboMarcaHTML = "<p><select id='" + "SEL" + i + "'class='w3-input w3-border textos' disabled = 'true'>";
        for (var k = 0; k < sele1.length; k++) {
            if (sele2 == sele1[k].sele) {
                cboMarcaHTML += "<option  value='" + sele1[k].sele + "' selected>" + sele1[k].sele + "</option>";
            } else {
                cboMarcaHTML += "<option  value='" + sele1[k].sele + "'>" + sele1[k].sele + "</option>";
            }
        }
        cboMarcaHTML += "</select></p>";
        return cboMarcaHTML;
    } catch (e) {
        alert("selec" + e);
    }
}

function cargadoc(dataItemDoc) {
    try {

        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", inspeccionar(dataItemDoc));
        //dataItemDoc.numero_orden
        //ConsultaControlCalidad(dataItemDoc);

        var controlHTML = "<br><table cellpadding='0' cellspacing='0' border='0'>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Orden de reparación: <font color='blue'>" + dataItemDoc.numero_orden + "</font></p></td><td></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Nombre del cliente: <font color='blue'>" + dataItemDoc.nombre_propietario + "</font></p></td></tr>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Vehículo: <font color='blue'>" + dataItemDoc.nombre_modelo + "</font></p></td><td></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Teléfono: <font color='blue'>" + dataItemDoc.telefono_cliente + "</font></p></td></tr>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Chasis: <font color='blue'>" + dataItemDoc.chasis + "</font></p></td><td width='20%'><p><label style='font-family:Arial; font-size:11px'></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Celular: <font color='blue'>" + dataItemDoc.celular_cliente + "</font></p></td></tr>" +
            "<tr style='border-bottom:1pt solid black;'><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Kilometraje: <font color='blue'>" + dataItemDoc.kilometraje + "</font></p></td><td></td><td width='20%'><p><label style='font-family:Arial; font-size:11px; font-color: black'>Fecha: <font color='blue'>" + dataItemDoc.fecha_recepcion + "</font></p></td></tr></table>";

        var controlHTMLas = "<br><table cellpadding='0' cellspacing='0' border='0'>" +
            "<tr style='border-bottom:3pt solid red;'><td colspan='3'><p><label" +
            " style='font-family:Arial; font-size:14px; color: red'>ASESOR DE SERVICIO</p></td><td>" +
            "</td><td width='80%'></td></tr>" +
            "<tr style='border-bottom:1pt solid red;'><td width='20%'></td><td width='80%'><label" +
            "style='font-family:Arial; font-size:11px; color: red'>" +
            "PLUMAS LIMPIAPARABRISAS</td><tdwidth='80%'></td></tr>" +
            "<tr><td width='20%'><span class='fa-stack fa-lg'><i class='fa fa-check-circle-o fa-stack-1x'" + "style=' color:green'></i></td></tr>" +
            "<tr><td width='20%'><span class='fa-stack fa-lg'><i class='fa fa-circle" +
            " fa-stack-1x'" +
            "style=' color:#EF7F1A'></i></td><td><img src='images/parabrisas.png' style='border:1px" +
            " solid white'></td><td></td></tr>" +
            "<tr><td width='20%'><span class='fa-stack fa-lg'><i class='fa fa-circle fa-stack-1x' style='" + "color:red'></i></td></tr>" +

            "<tr style='border-top:3pt solid red;border-bottom:1pt solid red;'><td width='20%'></td>" +
            "<td width='80%'><label style='font-family:Arial; font-size:11px; color: red;'>PROFUNDIDAD" +
            " DE LABRADO Y PRESION DE NEUMATICOS</td><td></td></tr>" +

            "<tr><td><br><label style='font-family:Arial; font-size:11px;'>DD&nbsp;&nbsp;</label>" +
            "<label style='font-family:Arial; font-size:11px; border-top:3pt solid red;" +
            "border-bottom:3pt solid red;border-right:3pt solid red;border-left:3pt solid red;" +
            "'>60</label></td>" +
            "<td  width='80%'><br><label style='font-family:Arial; font-size:11px;'>DI&nbsp;&nbsp;</label>" +
            "<label style='font-family:Arial; font-size:11px; border-top:3pt solid red;" +
            "border-bottom:3pt solid red;border-right:3pt solid red;border-left:3pt solid" +
            " red'>59</td><td width='20%'></td></tr>" +

            "<tr><td width='40%'><br><div align='center'><img src='images/llanta.png'></div>" +
            "</td><td></td></tr>" +

            "<tr><td><br><label style='font-family:Arial; font-size:11px;'>DD&nbsp;&nbsp;</label>" +
            "<label style='font-family:Arial; font-size:11px; border-top:3pt solid red;" +
            "border-bottom:3pt solid red;border-right:3pt solid red;border-left:3pt solid red;" +
            "'>60</label></td>" +
            "<td  width='80%'><br><label style='font-family:Arial; font-size:11px;'>DI&nbsp;&nbsp;</label>" +
            "<label style='font-family:Arial; font-size:11px; border-top:3pt solid red;" +
            "border-bottom:3pt solid red;border-right:3pt solid red;border-left:3pt solid" +
            " red'>59</td><td width='20%'></td></tr>" +
            "</table>"
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>11113 ERROR</center>", controlHTML + controlHTMLas);
        //document.getElementById("descSMS_1RCC").innerHTML = controlHTML + controlHTMLas;
        //alert(inspeccionar(dataItemDoc))🔴;
    } catch (error) {
        alert("cargadoc" + error);
    }

}

function volverCLC() {
    //banderaCC = 1;
    kendo.mobile.application.navigate("components/admOt/view.html");
}

function formatoImpConCalAP(bolVisEV) {
    try {
        var inforImprime = JSON.parse(localStorage.getItem("dataItem"));
        var dataFirmas = JSON.parse(localStorage.getItem("firmasTotal"));
        document.getElementById("firmaGrafCLC").value = "";
        document.getElementById("firmaGrafCLC").value = dataFirmas[1];
        var htmlFormatoEV = "";
        var repFirmaCC = "";

        if (document.getElementById("firmaGrafCLC").value.includes("CC_firm.") == true && document.getElementById("firmaGrafCLC").value.includes("|") == true) {
            var arrImpFirma = document.getElementById("firmaGrafCLC").value.split("|");
            repFirmaCC = "<img src='" + arrImpFirma[2] + "' height='80'>";
            //localStorage.setItem("firma",repFirma);
            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> repFirma</center>", repFirmaCC);
        }
        document.getElementById("firmaGrafCLC").value = "";
        document.getElementById("firmaGrafCLC").value = dataFirmas[2];
        var repFirmaCCA = "";
        if (document.getElementById("firmaGrafCLC").value.includes("CC_firmAS.") == true && document.getElementById("firmaGrafCLC").value.includes("|") == true) {
            var arrImpFirmaCCA = document.getElementById("firmaGrafCLC").value.split("|");
            repFirmaCCA = "<img src='" + arrImpFirmaCCA[2] + "' height='80'>";
            //localStorage.setItem("firmaas",repFirmaAS);
            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> repFirmaAS</center>", repFirmaAS);
        }
        var repFirmaJT = "";
        document.getElementById("firmaGrafCLC").value = "";
        document.getElementById("firmaGrafCLC").value = dataFirmas[3];
        var repFirmaJT = document.getElementById("firmaGrafCLC").value.split("|");
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> repFirmaAS</center>", repFirmaJT);
        if (document.getElementById("firmaGrafCLC").value.includes("CC_firmJT.") == true && document.getElementById("firmaGrafCLC").value.includes("|") == true) {
            var arrImpFirmaCCEJ = document.getElementById("firmaGrafCLC").value.split("|");
            repFirmaJT = "<img src='" + arrImpFirmaCCEJ[2] + "' height='80'>";
            //localStorage.setItem("firmaas",repFirmaAS);
            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> repFirmaAS</center>", repFirmaAS);
        }
        kendo.ui.progress($("#reporteCLCScreen"), true);
        setTimeout(function () {
            var tablaDat = cargarTablaConC001(JSON.parse(localStorage.getItem("dataItem")));
            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", tablaDat);
            var tablaPreguntasImp = cargarTablaINP11(inforImprime); //cargarTablaINP11(inforImprime); // cargarTablaINPpdf(inforImprime);
            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> repFirmaAS</center>", tablaPreguntasImp);
            var ima = localStorage.getItem("ls_url2").toLocaleString() + "/formato_mail/logo_flc.jpg";
            htmlFormatoEV = "<table width='100%' border='0' cellspacing='0' cellpadding='0'>" +
                "<tr><td>" +
                "<table width='100%' cellpadding='0' cellspacing='0'>" +
                "<tr>" +
                "<td>" + "<img src='" + ima + "' alt='FLC' width='115' height='70'>" + "</td>" +
                "<td style='text-align:center'>" +
                "<h2><label style='color:#C00000'>FORMULARIO PARA CONTROL DE CALIDAD</label></h2>" +
                "</td>" +
                "</tr>" +
                "</table>" +
                "</td>" +
                "</tr>" +
                "<tr><td>" +
                "<table  width='100%' border='1' cellspacing='0' cellpadding='0' bordercolor='#000000'>" +
                "<tr>" +
                "<td>" +
                "<table width='100%' border='0' cellspacing='0' cellpadding='0'>" +
                "<tr>" +
                "<td>" +
                tablaDat + 
                "</td>" + 
                "</tr>" +  
                "<tr>" +
                "<td>" + "<hr size='10'style='color: #000000;'"+
                //"<table width='100%' border='0' cellspacing='0' cellpadding='0'><tr><td>" +
                tablaPreguntasImp + 
                //"</td></tr></table>" +
            "</td>" +
            "</tr>" +
            "</table>" +
            "</td>" +
            "</tr>" +
            "</table>" +
            "</td>" +
            "</tr>" +

            "<tr>" +
            "<td style='background-color:#000000'>" +
            "<p><label style='color:white; font-family:Arial; font-size:10px'>OBSERVACIONES</label></p>" +
            "</td>" +
            "</tr>" +
            "<tr>" +
            "<td><p><label style='font-family:Arial; font-size:11px'>" + document.getElementById("observacionCLC").value + "</label></p></td>" +
            "</tr>" +
            "<tr>" +
            "<td>" +
            "<table width='100%' cellpadding='3' cellspacing='0' border='0' style='border-color:#C00000'>" +
            "<tr>" +
            "<td width='50%' style='background-color:#000000'><center>" +
            "<p><label style='color:white; font-family:Arial; font-size:11px '>ENTREGA:</label></p>" +
            "</center></td>" +
            "<td width='50%' style='background-color:#000000'><center>" +
            "<p><label style='color:white; font-family:Arial; font-size:11px'> RECIBE:</label></p>" +
            "</center></td>" +
            "</tr>" +
            "</table>" +
            "</td>" +
            "</tr>" + "<tr><td><table><tr><td></td></tr></table></td></tr>"+ "<tr><td><table><tr><td></td></tr></table></td></tr>"+"<tr><td><table><tr><td></td></tr></table></td></tr>"+ "<tr><td><table><tr><td></td></tr></table></td></tr>"+
            "<tr>" +
            "<td>" +
            "<table width='100%' cellpadding='0' cellspacing='0' border='0' style='border-color:#C00000'>" +
            "<tr>" +
            "<td width='50%'>" +
            "<center>" + repFirmaCCA + "</center></td> " +
            "<td width='50%'>" +
            "<center>" + repFirmaCC + "</center></td>" +

            "</tr>" +
            "<tr>" +
            "<td width='50%'>" +
            "<center>--------------------------------------</center></td> " +
            "<td width='50%'>" +
            "<center>------------------------------------------------</center></td>" +
            "</tr>" +
            "<tr>" +
            "<td width='50%'>" +
            "<center>Firma Asesor</center></td> " +
            "<td width='50%'>" +
            "<center>Firma aprobaci&oacute;n del cliente</center></td>" +

            "</tr>" +
            "</table>" +
            "</td>" +
            "</tr>" +
            "</table>";
            //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> IMPRIME</center>", htmlFormatoEV);
            if (bolVisEV == false) {
                var htmlOrdenaux = localStorage.getItem("tablaPDFCCA");

                try {
                    cordova.plugins.printer.print(
                        htmlFormatoEV,
                        {
                            graystyle: true
                        },
                        function (msg) {
                            kendo.ui.progress($("#reporteCLCScreen"), false);

                        },
                        function (msg) {
                            kendo.ui.progress($("#reporteCLCScreen"), false);
                        }
                    );
                } catch (e) {
                    alert("imprime" + e);
                }
                //volverForm();
            } else {
                kendo.ui.progress($("#reporteCLCScreen"), false);
                var regresoHTML = htmlFormatoEV + "</td></tr></table>";
                //alert(bolVisEV);
                //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> IMPRIME</center>", regreso);
                return regresoHTML;
            }


            // precarga *********************************************************************************************
        }, 2000);
        kendo.ui.progress($("#reporteCLCScreen"), false);
    } catch (eimpev) {
        kendo.ui.progress($("#reporteCLCScreen"), false);
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>11114 ERROR</center>", eimpev);
    }
}

function toast(text) {
    var isMac = navigator.userAgent.toLowerCase().includes('macintosh');
    text = text !== null ? text : 'finished or canceled';

    setTimeout(function () {
        if (window.Windows !== undefined) {
            showWinDialog(text);
        } else if (!isMac && window.plugins && window.plugins.toast) {
            window.plugins.toast.showShortBottom(String(text));
        } else {
            alert(text);
        }
    }, 500);
};

function showWinDialog(text) {
    if (dialog) {
        dialog.content = text;
        return;
    }

    dialog = new Windows.UI.Popups.MessageDialog(text);

    dialog.showAsync().done(function () {
        dialog = null;
    });
};

function callback(params) {
    alert("regreso");
}

function cargarTablaConC001(inforImprime) {
    try {
        var fechaCre = JSON.parse(localStorage.getItem("inforImprimePDF1"));
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>MIO ERROR</center>", inspeccionar(fechaCre[0]));
        var html_ev_01 =
            "<table cellspacing='0' cellpadding='0'>" +
            "<tr>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Orden de reparación: <font color='blue'>" + inforImprime.numero_orden + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            "</td>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Nombre del cliente: <font color='blue'>" + inforImprime.nombre_propietario + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            "</td>" +
            "</tr>" +
            "<tr bgcolor='#ccccff' border-bottom: 5px ridge black;>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Vehículo: <font color='blue'>" + inforImprime.nombre_modelo + "</font></span><span style='font-style:normal;font-weight:bold;font-size:8pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            "</td>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Teléfono: <font color='blue'>" + inforImprime.telefono_cliente + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            "</td>" +
            "</tr>" +
            "<tr>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Chasis: <font color='blue'>" + inforImprime.chasis + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            "</td>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Celular: <font color='blue'>" + inforImprime.celular_cliente + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            "</td>" +
            "</tr>" +
            "<tr bgcolor='#ccccff' border-bottom: '1px'>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Kilometraje: <font color='blue'>" + inforImprime.kilometraje + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            "</td>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Fecha Recep.: <font color='blue'>" + inforImprime.fecha_recepcion + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            "</td>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Fecha Registro: <font color='blue'>" + fechaCre[0].fecha_creacion + 
            "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            "</td>" +
            "</tr>" +
            "</table>";

        /* "<table cellpadding='0' cellspacing='0' >" +
             "<tr style='border-bottom: thin short'>"+
                 "<td width='50%'>"+

                     "<label style='font-family:Arial; font-size:11px; font-color: black'>"+
                     "Orden de reparación: <font color='blue'>"+ inforImprime.numero_orden+"</font></label>"+

                 "</td>"+
                 "<td width='50%'>"+

                     "<label style='font-family:Arial; font-size:11px; font-color: black'>Nombre del clientezzz: "+
                     "<font color='blue'>" + inforImprime.nombre_propietario + "</font></label>"+

                 "</td>"+
             "</tr>"+
             "<tr style='border-bottom:1pt solid black;'>"+
                 "<td width='50%'>"+

                     "<label style='font-family:Arial; font-size:11px; font-color: black'>Vehículo: "+
                     "<font color='blue'>"+inforImprime.nombre_modelo+"</font></label>"+

                 "</td>"+
                 "<td width='50%'>"+

                     "<label style='font-family:Arial; font-size:11px; font-color: black'>Teléfono: "+
                     "<font color='blue'>"+inforImprime.telefono_cliente+"</font></label>"+

                 "</td>"+
             "</tr>" +
             "<tr style='border-bottom:1pt solid black;'>"+
                 "<td width='50%'>"+

                     "<label style='font-family:Arial; font-size:11px; font-color: black'>Chasis: "+
                     "<font color='blue'>"+inforImprime.chasis+"</font></label>"+

                 "</td>"+
                 "<td width='50%'>"+

                         "<label style='font-family:Arial; font-size:11px; font-color: black'>Celular: "+
                         "<font color='blue'>"+inforImprime.celular_cliente+"</font></label>"+

                 "</td>"+
             "</tr>" +
             "<tr style='border-bottom:1pt solid black;'>"+
                 "<td width='50%'>"+

                     "<label style='font-family:Arial; font-size:11px; font-color: black'>Kilometraje: "+
                     "<font color='blue'>"+inforImprime.kilometraje+"</font></label>"+

                 "</td"+
                 "<td width='50%'>"+

                     "<label style='font-family:Arial; font-size:11px; font-color: black'>Fecha: "+
                     "<font color='blue'>"+inforImprime.fecha_cita+"</font></label>"+

                 "</td>"+
             "</tr>"+
         "</table>"; */
    } catch (e) {
        alert("cargarTablaConC001" + e);
    }
    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>MIO ERROR</center>", html_ev_01);
    return html_ev_01;
}

function cargarTablaConC0(inforImprime) {
    try {
        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>MIO ERROR</center>", inspeccionar(inforImprime));
        var html_ev_01 = /* "<br>" + */
            "<table cellspacing='0' cellpadding='0'>" +
            /* "<tr padding: '8pt' text-align: 'left' border-bottom: '1px solid #ddd'>"+
                "<td width='50%' border-bottom: '1px solid #ddd'>"+ bgcolor='#ccccff'*/
            "<tr>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Orden de reparación: <font color='blue'>" + inforImprime.numero_orden + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            /* "<p>"+ */
            /* "<label style='font-family:Arial; font-size:1px; font-color: black'>"+
            "Orden de reparación: <font color='blue'>"+ inforImprime.numero_orden+"</font></label>"+ */
            /* "</p>"+ <td></td>*/
            "</td>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Nombre del cliente: <font color='blue'>" + inforImprime.nombre_cliente + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            /* "<p>"+ */
            /* "<label style='font-family:Arial; font-size:1px; font-color: black'>Nombre del cliente: "+
            "<font color='blue'>" + inforImprime.nombre_propietario + "</font></label>"+ */
            /* "</p>"+ */
            "</td>" +
            "</tr>" +
            "<tr bgcolor='#ccccff' border-bottom: 5px ridge black;>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Vehículo: <font color='blue'>" + inforImprime.nombre_modelo + "</font></span><span style='font-style:normal;font-weight:bold;font-size:8pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            /* "<p>"+ */
            /* "<label style='font-family:Arial; font-size:1px; font-color: black'>Vehículo: "+
            "<font color='blue'>"+inforImprime.nombre_modelo+"</font></label>"+ */
            /* "</p>"+ <td></td>*/
            "</td>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Teléfono: <font color='blue'>" + inforImprime.telefono_cliente + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            /* "<p>"+ */
            /* "<label style='font-family:Arial; font-size:1px; font-color: black'>Teléfono: "+
            "<font color='blue'>"+inforImprime.telefono_cliente+"</font></label>"+ */
            /* "</p>"+ */
            "</td>" +
            "</tr>" +
            "<tr>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Chasis: <font color='blue'>" + inforImprime.chasis + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            /* "<p>"+ */
            /* "<label style='font-family:Arial; font-size:1px; font-color: black'>Chasis: "+
            "<font color='blue'>"+inforImprime.chasis+"</font></label>"+ */
            /* "</p>"+ <td></td>*/
            "</td>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Celular: <font color='blue'>" + inforImprime.celular_cliente + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            /* "<p>"+ */
            /*  "<label style='font-family:Arial; font-size:1px; font-color: black'>Celular: "+
             "<font color='blue'>"+inforImprime.celular_cliente+"</font></label>"+ */
            /*  "</p>"+ */
            "</td>" +
            "</tr>" +
            "<tr bgcolor='#ccccff' border-bottom: '1px'>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Kilometraje: <font color='blue'>" + inforImprime.kilometraje + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            /* "<p>"+ */
            /*  "<label style='font-family:Arial; font-size:1px; font-color: black'>Kilometraje: "+
             "<font color='blue'>"+inforImprime.kilometraje+"</font></label>"+ */
            /* "</p>"+ ><td></td>*/
            "</td>" +
            "<td>" +
            "<div><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'>Fecha: <font color='blue'>" + inforImprime.fecha_recepcion + "</font></span><span style='font-style:normal;font-weight:bold;font-size:7pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
            /* "<p>"+ */
            /* "<label style='font-family:Arial; font-size:1px; font-color: black'>Fecha: "+
            "<font color='blue'>"+inforImprime.fecha_cita+"</font></label>"+ */
            /* "</p>"+ */
            "</td>" +
            "</tr>" +
            "</table>";
    } catch (e) {
        alert("cargarTablaConC0" + e);
    }
    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>MIO ERROR</center>", html_ev_01);
    return html_ev_01;
}

function cargarTablaINPpdf(inforusa) {
    try {
        localStorage.removeItem("inforImprime");
        var respOT = "0";
        var tipo_doc = "CONTROL_CALIDAD";

        if (inforusa.chasis.substr(7, 1) == "E") {
            tipo_doc = "CONTROL_CALIDAD_ELEC"
        }
        var UrlMotorEscape = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl43PreguntasCalidadGet/1,json;" +
            inforusa.codigo_empresa + ";" + inforusa.codigo_sucursal + ";" + inforusa.codigo_taller + ";" + inforusa.anio + ";" +
            inforusa.secuencia_orden + ";" + tipo_doc + ";;;;;;";
        var infME;
        $.ajax({
            url: UrlMotorEscape,
            type: "GET",
            async: false,
            dataType: "json",
            success: function (data) {
                try {
                    infME = (JSON.parse(data.tl43PreguntasCalidadGetResult)).ttl43;
                    localStorage.setItem("inforImprimePDF", JSON.stringify(infME));
                    respOT = "1";
                } catch (e) {
                    alert(e);
                    return;
                }
            },
            error: function (err) {
                alert(err);
                return;
            }
        });
        if (respOT == "1") {
            try {
                var griddata = [];
                tamano = infME.length;
                for (var t = 0; t < infME.length; t++) {
                    if (infME[t].orden_presentacion < 10) {
                        infME[t].orden_presentacion = "0" + infME[t].orden_presentacion;
                    }
                }

                infME.sort(function myfunction(a, b) {
                    return (a.corden_seccion + "" + a.orden_presentacion) - (b.corden_seccion + "" + b.orden_presentacion)
                });
                for (var i = 0; i < infME.length; i++) {
                    var selecResp = new Array();
                    var sevResp = new Array();
                    var erResp = new Array();
                    var evResp = new Array();
                    if (infME[i].tipo_respuesta == "SELECCION" || infME[i].tipo_respuesta == "SEV") {
                        if (infME[i].tipo_respuesta == "SEV") {
                            //alert("etiqu"+infME[i].lista_etiquetas);
                            sevResp = infME[i].lista_etiquetas.split(",");
                            //alert(inspeccionar(sevResp));
                            var sevRes = [];
                            for (var k = 0; k < sevResp.length; k++) {
                                sevRes.push({sevR: sevResp[k]});
                                //alert(inspeccionar(sevRes[k]));
                            }
                            //alert(inspeccionar(sevRes[0]));
                        }
                        //alert("resp"+infME[i].lista_respuesta)
                        selecResp = infME[i].lista_respuesta.split(",");
                        var selRes = [];
                        selRes.push({sele: "Seleccione"});
                        for (var j = 0; j < selecResp.length; j++) {
                            selRes.push({sele: selecResp[j]});
                        }
                    } else {
                        if (infME[i].tipo_respuesta == "ER") {
                            //alert("else eti" + infME[i].lista_etiquetas);
                            erResp = infME[i].lista_etiquetas.split(",");
                            var erRes = [];
                            for (var l = 0; l < erResp.length; l++) {
                                erRes.push({erR: erResp[l]});
                            }
                            //alert("else resp" + infME[i].lista_respuesta);
                            selecResp = infME[i].lista_respuesta.split(",");
                            var selRes = [];
                            selRes.push({sele: "Seleccione"});
                            for (var j = 0; j < selecResp.length; j++) {
                                selRes.push({sele: selecResp[j]});
                            }
                        } else {
                            if (infME[i].tipo_respuesta == "EV") {
                                //alert("else ev"+infME[i].lista_etiquetas);
                                evResp = infME[i].lista_etiquetas.split(",");
                                //alert((evResp.length));
                                var evRes = [];
                                for (var hh = 0; hh < evResp.length; hh++) {
                                    evRes.push({evR: evResp[hh]});
                                }
                                //alert("0:" + evRes[0].evR);
                                //alert("1:" + evRes[1].evR);
                                //alert("2:" + evRes[2].evR);
                                //alert("3:" + evRes[3].evR);
                            }
                        }
                    }

                    griddata.push({
                        codigo_empresa: infME[i].codigo_empresa,
                        codigo_sucursal: infME[i].codigo_sucursal,
                        codigo_taller: infME[i].codigo_taller,
                        anio_tl06: infME[i].anio_tl06,
                        secuencia_orden_tl06: infME[i].secuencia_orden_tl06,
                        secuencia_tl43: infME[i].secuencia_tl43,
                        orden_presentacion: infME[i].orden_presentacion,
                        pregunta: infME[i].pregunta,
                        tipo_formulario: infME[i].tipo_formulario,
                        seccion_formulario: infME[i].seccion_formulario,
                        tipo_respuesta: infME[i].tipo_respuesta,
                        lista_respuesta: infME[i].lista_respuesta,
                        lista_etiquetas: infME[i].lista_etiquetas,
                        tipo_valor: infME[i].tipo_valor,
                        nombre_seccion: infME[i].nombre_seccion,
                        lista_valores: infME[i].lista_valores,
                        respuesta: infME[i].respuesta,
                        secuencia_vh65: infME[i].secuencia_vh65,
                        corden_seccion: infME[i].corden_seccion,
                        selResp: selRes,
                        sevResp: sevRes,
                        erResp: erRes,
                        evResp: evRes,
                        selRespt: infME[i].selRespt,
                        sevRespt: infME[i].sevRespt,
                        erRespt: infME[i].erRespt,
                        evRespt: infME[i].evRespt,
                        nombre_seccion: infME[i].nombre_seccion,
                        imagen: infME[i].imagen,
                        observacion_calidad: inforusa.observacion_calidad
                    });
                    //if (infME[i].tipo_respuesta == "EV") { alert(griddata[i].evResp[0].evR); alert(griddata[i].evResp[1].evR); alert(griddata[i].evResp[2].evR); alert(griddata[i].evResp[3].evR); }
                    //alert("sevRespt" + infME[i].sevRespt);
                    //alert("sevRes" + inspeccionar(sevRes));
                    //alert("sevResp"+inspeccionar(griddata[i].sevResp));
                    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", inspeccionar(griddata[i].sevResp));
                }
            } catch (e) {
                alert("1" + e);
            }
            //alert("antes de func");

            var tablaINP = cargatabla_1AVApdf(griddata);
            var registroME = "";
            return tablaINP;
        }
    } catch (e) {
        alert("214" + e);
        return;
    }
}

function cargarTablaINP11(inforusa) {
    try {
        localStorage.removeItem("inforImprime");
        var respOT = "0";
        var tipo_doc = "CONTROL_CALIDAD";

        if (inforusa.chasis.substr(7, 1) == "E") {
            tipo_doc = "CONTROL_CALIDAD_ELEC"
        }
        var UrlMotorEscape = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl43PreguntasCalidadGet/1,json;" +
            inforusa.codigo_empresa + ";" + inforusa.codigo_sucursal + ";" + inforusa.codigo_taller + ";" + inforusa.anio + ";" +
            inforusa.secuencia_orden + ";" + tipo_doc + ";;;;;;";
        var infME;
        $.ajax({
            url: UrlMotorEscape,
            type: "GET",
            async: false,
            dataType: "json",
            success: function (data) {
                try {
                    infME = (JSON.parse(data.tl43PreguntasCalidadGetResult)).ttl43;
                    localStorage.setItem("inforImprimePDF", JSON.stringify(infME));
                    respOT = "1";
                } catch (e) {
                    alert(e);
                    return;
                }
            },
            error: function (err) {
                alert(err);
                return;
            }
        });
        if (respOT == "1") {
            try {
                var griddata = [];
                tamano = infME.length;
                for (var t = 0; t < infME.length; t++) {
                    if (infME[t].orden_presentacion < 10) {
                        infME[t].orden_presentacion = "0" + infME[t].orden_presentacion;
                    }
                }

                infME.sort(function myfunction(a, b) {
                    return (a.corden_seccion + "" + a.orden_presentacion) - (b.corden_seccion + "" + b.orden_presentacion)
                });
                for (var i = 0; i < infME.length; i++) {
                    var selecResp = new Array();
                    var sevResp = new Array();
                    var erResp = new Array();
                    var evResp = new Array();
                    if (infME[i].tipo_respuesta == "SELECCION" || infME[i].tipo_respuesta == "SEV") {
                        if (infME[i].tipo_respuesta == "SEV") {
                            //alert("etiqu"+infME[i].lista_etiquetas);
                            sevResp = infME[i].lista_etiquetas.split(",");
                            //alert(inspeccionar(sevResp));
                            var sevRes = [];
                            for (var k = 0; k < sevResp.length; k++) {
                                sevRes.push({sevR: sevResp[k]});
                                //alert(inspeccionar(sevRes[k]));
                            }
                            //alert(inspeccionar(sevRes[0]));
                        }
                        //alert("resp"+infME[i].lista_respuesta)
                        selecResp = infME[i].lista_respuesta.split(",");
                        var selRes = [];
                        selRes.push({sele: "Seleccione"});
                        for (var j = 0; j < selecResp.length; j++) {
                            selRes.push({sele: selecResp[j]});
                        }
                    } else {
                        if (infME[i].tipo_respuesta == "ER") {
                            //alert("else eti" + infME[i].lista_etiquetas);
                            erResp = infME[i].lista_etiquetas.split(",");
                            var erRes = [];
                            for (var l = 0; l < erResp.length; l++) {
                                erRes.push({erR: erResp[l]});
                            }
                            //alert("else resp" + infME[i].lista_respuesta);
                            selecResp = infME[i].lista_respuesta.split(",");
                            var selRes = [];
                            selRes.push({sele: "Seleccione"});
                            for (var j = 0; j < selecResp.length; j++) {
                                selRes.push({sele: selecResp[j]});
                            }
                        } else {
                            if (infME[i].tipo_respuesta == "EV") {
                                //alert("else ev"+infME[i].lista_etiquetas);
                                evResp = infME[i].lista_etiquetas.split(",");
                                //alert((evResp.length));
                                var evRes = [];
                                for (var hh = 0; hh < evResp.length; hh++) {
                                    evRes.push({evR: evResp[hh]});
                                }
                                //alert("0:" + evRes[0].evR);
                                //alert("1:" + evRes[1].evR);
                                //alert("2:" + evRes[2].evR);
                                //alert("3:" + evRes[3].evR);
                            }
                        }
                    }

                    griddata.push({
                        codigo_empresa: infME[i].codigo_empresa,
                        codigo_sucursal: infME[i].codigo_sucursal,
                        codigo_taller: infME[i].codigo_taller,
                        anio_tl06: infME[i].anio_tl06,
                        secuencia_orden_tl06: infME[i].secuencia_orden_tl06,
                        secuencia_tl43: infME[i].secuencia_tl43,
                        orden_presentacion: infME[i].orden_presentacion,
                        pregunta: infME[i].pregunta,
                        tipo_formulario: infME[i].tipo_formulario,
                        seccion_formulario: infME[i].seccion_formulario,
                        tipo_respuesta: infME[i].tipo_respuesta,
                        lista_respuesta: infME[i].lista_respuesta,
                        lista_etiquetas: infME[i].lista_etiquetas,
                        tipo_valor: infME[i].tipo_valor,
                        nombre_seccion: infME[i].nombre_seccion,
                        lista_valores: infME[i].lista_valores,
                        respuesta: infME[i].respuesta,
                        secuencia_vh65: infME[i].secuencia_vh65,
                        corden_seccion: infME[i].corden_seccion,
                        selResp: selRes,
                        sevResp: sevRes,
                        erResp: erRes,
                        evResp: evRes,
                        selRespt: infME[i].selRespt,
                        sevRespt: infME[i].sevRespt,
                        erRespt: infME[i].erRespt,
                        evRespt: infME[i].evRespt,
                        nombre_seccion: infME[i].nombre_seccion,
                        imagen: infME[i].imagen
                    });
                    //if (infME[i].tipo_respuesta == "EV") { alert(griddata[i].evResp[0].evR); alert(griddata[i].evResp[1].evR); alert(griddata[i].evResp[2].evR); alert(griddata[i].evResp[3].evR); }
                    //alert("sevRespt" + infME[i].sevRespt);
                    //alert("sevRes" + inspeccionar(sevRes));
                    //alert("sevResp"+inspeccionar(griddata[i].sevResp));
                    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ALERTA</center>", inspeccionar(griddata[i].sevResp));
                }
            } catch (e) {
                alert("1" + e);
            }
            //alert("antes de func");
            var tablaINP = cargatabla_ControlCalidad(griddata);
            var registroME = "";
            return tablaINP;
        }
    } catch (e) {
        alert("215" + e);
        return;
    }
}

function parametrosCLCP() {
    try {
        var accResp = "";
        //http://192.168.1.50:8089/concesionario/Services/TG/Parametros.svc/ParametroEmpGet/6,;TG;APP_ENTREGAS;;PATH_ENTREGAS
        var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TG/Parametros.svc/ParametroEmpGet/6,;TL;PATH_CONTROL_CALIDAD;;PATH_CONTROL_CALIDAD";
        var respPar;
        $.ajax({
            url: Url,
            type: "GET",
            dataType: "json",
            async: false,
            success: function (data) {
                try {
                    accResp = JSON.parse(data.ParametroEmpGetResult).tmpParamEmp;
                    respPar = accResp[0];
                } catch (e) {
                    /*  window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>36 ERROR</center>","parametrosCLCP" + inspeccionar(e)); */
                    respPar = "error";
                }
            },
            error: function (err) {
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>37 ERROR</center>", inspeccionar(err));
                respPar = "error";
            }
        });
        return respPar;
    } catch (f) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>38 ERROR</center>", f);
        return "error";
    }
}

function cargatabla_1AVApdf(griddata1) {
    try {
        dataRespuesta = "";
        var tablaOT = "<table cellspacing='0' cellpadding='0'>";
        var tamano = griddata1.length;

        for (var i = 0; i < griddata1.length; i++) {
            var seccionFor = griddata1[i].seccion_formulario;
            tablaOT += "<tr bgcolor='#aef9e2'>" +
                "<td>" +
                "<div><span style='font-style:normal;font-weight:bold;font-size:10pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> <font color='black'><u>" + griddata1[i].nombre_seccion + "</u></font></span><span style='font-style:normal;font-weight:bold;font-size:10pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
                "</td>" +
                "</tr>";
            var bandSeccion = 0;
            var j = i;
            var l = 0;
            var k = 3;
            while (griddata1[j].seccion_formulario == seccionFor) {
                j++;
                l++;
                if (j == tamano) {
                    break;
                }
            }
            if (l < 4) {
                bandSeccion = 1;
                k = 1;
            }
            while (griddata1[i].seccion_formulario == seccionFor) {
                tablaOT += "<tr><td><table cellspacing='0' cellpadding='0'><tr>";
                for (var index = 0; index < k; index++) {
                    if (griddata1[i].seccion_formulario == seccionFor) {
                        tablaOT += "<td>" +
                            "<div><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'>" + griddata1[i].pregunta + "</font></span><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                            "</td>";
                        if (griddata1[i].tipo_respuesta == "SELECCION" || griddata1[i].tipo_respuesta == "SEV") {
                            var sevUsa = ""; //"<td>";  //<table><tr>";
                            if (griddata1[i].tipo_respuesta == "SEV") {
                                var valorsev = griddata1[i].sevRespt.split(',');
                                //alert(griddata1[i].sevResp.length);
                                for (var l = 0; l < griddata1[i].sevResp.length; l++) {
                                    //alert("VAL: " + valorsev[l]);
                                    if (valorsev[l] === "undefined" || valorsev[l] === null) {
                                        valorsev[l] = "NN";
                                        // alert("VAL1: "+valorsev[l]);
                                    }

                                    if (l == 4) {
                                        sevUsa += "</tr><tr>"
                                    }
                                    //sevUsa += "<td></td>";
                                    sevUsa += "<td>" +
                                        "<div><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'>" + valorsev[l] + "</font></span><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                        "</td>";
                                }
                            } else {
                                if (griddata1[i].imagen != "") {
                                    var pathCC = parametrosCLCP();
                                    var pathCC1 = pathCC["descripcion-2"] + griddata1[i].imagen;
                                    sevUsa += "<td><div><center><img " + "src='" + pathCC1 + "' style='border:1" +
                                        " solid black;'></center></div></td>";
                                }
                            }
                            //alert("sevUsa"+sevUsa);
                            //sevUsa += "</td>"; //</tr></table></td>";
                            if (griddata1[i].selRespt.length == 0) {
                                bolImprimeEV = false;
                            }
                            var selUsa = griddata1[i].selRespt; // seleccionUSA(griddata1[i].selResp, i, griddata1[i].selRespt);
                            tablaOT += sevUsa + "<td width='10%'>" +
                                "<div><span style='font-style:normal;font-weight:light;font-size:7pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'>" + selUsa + "</font></span><span style='font-style:normal;font-weight:light;font-size:7pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                "</td>";
                            //alert("table:"+tablaOT);
                        } else {
                            if (griddata1[i].tipo_respuesta == "ER") {
                                var sevUsa = "<td><table style='width: 100%;' cellspacing='0' cellpadding='0'><tr>";
                                var valorer = griddata1[i].erRespt.split(',');
                                for (var l = 0; l < griddata1[i].erResp.length; l++) {
                                    //sevUsa += "<td>&nbsp;</td>";
                                    sevUsa += "<td><p><label style='font-family:Arial; font-size:11px'>" + valorer[l] + "</label></p></td>";
                                }
                                sevUsa += "</tr></table></td>";
                                if (griddata1[i].selRespt.length == 0) {
                                    bolImprimeEV = false;
                                }
                                var selUsa = griddata1[i].selRespt;  //seleccionUSA(griddata1[i].selResp, i, griddata1[i].selRespt);
                                tablaOT += sevUsa + "<td><p><label style='font-family:Arial; font-size:11px'>" + selUsa + "</label></p></td>";
                            } else {
                                if (griddata1[i].tipo_respuesta == "rrrrEV") {
                                    var sevUsa = "<td><table style='width: 100%;'><tr>";
                                    var valorev = griddata1[i].evRespt.split(',');
                                    for (var l = 0; l < griddata1[i].evResp.length; l++) {
                                        //sevUsa += "<td>&nbsp;</td>";
                                        sevUsa += "<td><p><label style='font-family:Arial; font-size:11px'>" + valorev[l] + "</label></p></td>";
                                    }
                                    sevUsa += "</tr></table></td>";
                                    tablaOT += sevUsa + "<td></td>";
                                }
                            }
                        }
                        i++;
                        if (i == tamano) {
                            if (bandSeccion == 1) {
                                index = 3;
                                bandSeccion = 0;
                            }
                            if (index == 1) {
                                tablaOT += "<td>" +
                                    "<div><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                    "</td>";
                                tablaOT += "<td width='10%'>" +
                                    "<div><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                    "</td>";
                            } else {
                                if (index == 0) {
                                    tablaOT += "<td>" +
                                        "<div><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                        "</td>";
                                    tablaOT += "<td width='10%'>" +
                                        "<div><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                        "</td>";
                                    tablaOT += "<td>" +
                                        "<div><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                        "</td>";
                                    tablaOT += "<td width='10%'>" +
                                        "<div><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:6pt;font-family:BCDEEE+Calibri-light;color:#000000'></span></div>" +
                                        "</td>";
                                }
                            }
                            index = 4;
                            //break;
                        }
                        //tablaOT += "</tr></table></td></tr>";
                    } else {
                        if (bandSeccion == 1) {
                            index = 3;
                            bandSeccion = 0;
                        }
                        if (index == 2) {
                            tablaOT += "<td>" +
                                "<div><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                "</td>";
                            tablaOT += "<td  width='10%'>" +
                                "<div><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                "</td>";
                        } else {
                            if (index == 1) {
                                tablaOT += "<td>" +
                                    "<div><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                    "</td>";
                                tablaOT += "<td width='10%'>" +
                                    "<div><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                    "</td>";
                                tablaOT += "<td>" +
                                    "<div><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                    "</td>";
                                tablaOT += "<td width='10%'>" +
                                    "<div><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'></font></span><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
                                    "</td>";
                            }
                        }
                        index = 4;
                    }
                }
                tablaOT += "</tr></table></td></tr>";
                if (i == tamano) {
                    break;
                }
            }
            i = i - 1;
        }
    } catch (error) {
        alert("cargatabla_1AVApdf" + error);
    }
    dataRespuesta = griddata1;
    tablaOT += "</table>";
    /* window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>38001 ERROR</center>", tablaOT); */
    tablaOT += "<table cellspacing='0' cellpadding='0'><tr bgcolor='#aef9e2'>" +
        "<td>" +
        "<div><span style='font-style:normal;font-weight:bold;font-size:11pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'><u>OBSERVACION</u></font></span><span style='font-style:normal;font-weight:bold;font-size:11pt;font-family:BCDEEE+Calibri-Bold;color:#000000'> </span></div>" +
        "</td>" +
        "</tr>";
    tablaOT += "<tr><td>" +
        "<div><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> <font color='black'>" + griddata1[0].observacion_calidad + "</font></span><span style='font-style:normal;font-weight:light;font-size:8pt;font-family:BCDEEE+Calibri-light;color:#000000'> </span></div>" +
        "</td>" +
        "</tr></table>";
    //alert(tablaOT);
    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>38001 ERROR</center>", tablaOT);
    return tablaOT;
}

function cargatabla_ControlCalidad(griddata1) {
    dataRespuesta = "";
    var tablaOT = "";
    tablaOT += "<table>";
    tamano = griddata1.length;
    tablaOT += "<table>"
    for (var i = 0; i < griddata1.length; i++) {
        var seccionFor = griddata1[i].seccion_formulario;
        tablaOT += "<tr style='background-color:#DFDFDF'><td style='width: 80%;' colspan='4'><p><label style='font-weight:bold; font-size:11px;color:black' >" +
            griddata1[i].nombre_seccion +
            //"</label></p></td><td style='width: 1%;'></td><td style='width: 19%;'></td></tr>";
            "</label></p></td></tr>";
        
        while (griddata1[i].seccion_formulario == seccionFor) {
            tablaOT += "<tr>";
            var secform = griddata1[i].seccion_formulario;
            //alert(seccionFor+"   "+griddata1[i].seccion_formulario);
            for (var index = 0; index <= 1; index++) {
                tablaOT += "<td style='width: 25%;' colspan='4'><p><label style='font-family:Arial; font-size:11px'>" + griddata1[i].pregunta + "</label></p></td>";

                if (griddata1[i].tipo_respuesta == "SELECCION" || griddata1[i].tipo_respuesta == "SEV") {
                    var sevUsa = "<td><table style='width: 100%;'><tr>";
                    if (griddata1[i].tipo_respuesta == "SEV") {
                        var valorsev = griddata1[i].sevRespt.split(',');
                        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", griddata1[i].erRespt);
                        for (var l = 0; l < griddata1[i].sevResp.length; l++) {
                            //alert("VAL: " + valorsev[l]);
                            if (valorsev[l] === "undefined" || valorsev[l] === null) {
                                valorsev[l] = " ";
                                // alert("VAL1: "+valorsev[l]);🔴
                            }

                            if (l == 4) {
                                sevUsa += "</tr><tr>"
                            }
                            sevUsa += "<td>&nbsp;</td>";
                            //sevUsa += "<td><p><input id=SEV" + i + "" + l + " type='" + tipoUsa + "' style='width:100%' placeholder=" + (l + 1) + " value=" + valorsev[l] + "></p></td>";
                            sevUsa += "<td><p><input id=SEV" + i + "" + l + "style='width:100%' placeholder=" + (l + 1) + " value=" + valorsev[l] + "></p></td>";
                        }
                    } else {
                        if (griddata1[i].imagen != "") {
                            var pathCC = parametrosCLCP();
                            var pathCC1 = pathCC["descripcion-2"] + griddata1[i].imagen;
                            sevUsa += "<td><div><center><img " + "src='" + pathCC1 + "' style='border:1px" +
                                " solid black;'></center></div></td>";
                        }
                    }

                    sevUsa += "</tr></table></td>";
                    if (griddata1[i].selRespt.length == 0) {
                        bolImprimeEV = false;
                    }
                    var selUsa = griddata1[i].selRespt; // seleccionUSA(griddata1[i].selResp, i, griddata1[i].selRespt);
                    tablaOT += sevUsa + "<td><p><label style='font-family:Arial; font-size:11px'>" + selUsa + "</label></p></td>";
                    //alert(tablaOT);
                    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>1 ERROR</center>", tablaOT);
                } else {
                    if (griddata1[i].tipo_respuesta == "ER") {
                        var sevUsa = "<td><table style='width: 100%;'><tr>";
                        var valorer = griddata1[i].erRespt.split(',');
                        for (var l = 0; l < griddata1[i].erResp.length; l++) {
                            sevUsa += "<td>&nbsp;</td>";
                            sevUsa += "<td><p><input id=ER" + i + "" + l + " type='text' size='2' placeholder=" + griddata1[i].erResp[l].erR + " value=" + valorer[l] + "></p></td>";
                        }
                        sevUsa += "</tr></table></td>";
                        if (griddata1[i].selRespt.length == 0) {
                            bolImprimeEV = false;
                        }
                        var selUsa = griddata1[i].selRespt;  //seleccionUSA(griddata1[i].selResp, i, griddata1[i].selRespt);
                        tablaOT += sevUsa + "<td><p><label style='font-family:Arial; font-size:11px'>" + selUsa + "</label></p></td>";
                        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>2 ERROR</center>", tablaOT);
                    } else {
                        if (griddata1[i].tipo_respuesta == "EV") {
                            var sevUsa = "<td><table style='width: 100%;'><tr>";
                            var valorev = griddata1[i].evRespt.split(',');
                            for (var l = 0; l < griddata1[i].evResp.length; l++) {
                                sevUsa += "<td>&nbsp;</td>";
                                sevUsa += "<td><p><input id=EV" + i + "" + l + " type='text' size='2' placeholder=" + griddata1[i].evResp[l].evR + " value=" + valorev[l] + "></p></td>";
                            }
                            sevUsa += "</tr></table></td>";
                            tablaOT += sevUsa + "<td></td>";
                            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> ERROR</center>", tablaOT);
                        }
                    }
                }
                i++;
                if (i == tamano) {
                    break;
                }
                if (griddata1[i].seccion_formulario != seccionFor) {
                    index = 1;
                }
            }
            tablaOT += "</tr>"
            //i++;
            if (i == tamano) {
                break;
            }
        }
        i--;
    }
    dataRespuesta = griddata1;
    tablaOT += "</table>"
    //alert(observa);
    //tablaOT += observa;
    //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>38 ERROR</center>", tablaOT);
    return tablaOT;
}

function imgLogoEV() {
    return "<img src='data:image/png;base64,' height='60' width='129'>";
}

var dialogOT_2;

function enviaMailDocCC() {
    try {

        var contMailOT = "<table cellspacing='10' align='center' width='100%'>" +
            "<tr><td style='height:40px'><p><label class='w3-text-red'><b>Para</b></label><input class='w3-input w3-border textos' id='mail_1' name='mail_1' type='text' value='" + JSON.parse(localStorage.getItem("dataItem")).email_cliente + "'></p></td></tr>" +
            "</table><input id='tipomail' name='tipomail' type='hidden' value='R'>  <input id='smscel' type='hidden' value='' /><input id='obsdiagnostico' type='hidden' value='' />";

        dialogOT_2 = $("#dialogMailOT").kendoDialog({
            width: "350px",
            buttonLayout: "normal",
            title: "<center><i class=\"fa fa-envelope-o\"></i> RECEPCI&Oacute;N DE VEH&Iacute;CULO</center>",
            closable: false,
            modal: false,
            content: contMailOT,
            actions: [
                {
                    text: '<font style=\"font-size:12px\"> <button  class=\"w3-btn w3-red\"> &nbsp;&nbsp;ENVIAR&nbsp;&nbsp;</button></font>',
                    action: enviarMailCC
                },
                {
                    text: '<font style=\"font-size:12px\"><button  class=\"w3-btn w3-red\"> CANCELAR</button></font>',
                    primary: true
                }
            ]
        });
        dialogOT_2.data("kendoDialog").open();
        llamarColorBotonGeneral(".w3-red");
        llamarColorTexto(".w3-text-red");
    } catch (error) {
        alert("enviaMailDocCC" + error);
    }
}


function formatoImpPDF() {
    htmlFormatoEV = "";
    try {
        var inforImprime = JSON.parse(localStorage.getItem("dataItem"));
        var repFirmaCCA = "";
        var repFirmaCC = "";
        /* kendo.ui.progress($("#reporteCLCScreen"), true);
        setTimeout(function () { */
        var tablaDatpdf = cargarTablaConC0(JSON.parse(localStorage.getItem("dataItem")));
        var tablaPreguntasImp = cargarTablaINPpdf(inforImprime);
        repFirmaCC = "F1RM_CL1";
        repFirmaCCA = "F1RM_CAS";
        var ima = localStorage.getItem("ls_url2").toLocaleString() + "/formato_mail/logo_flc.jpg";
        htmlFormatoEV = "<table width='100%' border='0' cellspacing='0' cellpadding='0'>" +
            "<tr><td>" +
            "<img src='" + ima + "' alt='FLC' width='115' height='70'>" +
            "</td>" +
            "<td style='text-align:center'>" +
            "<div style='position:absolute;top:0.83in;left:3.34in;width:2.59in;line-height:0.15in;'><span style='font-style:normal;font-weight:bold;font-size:12pt;font-family:CIDFont+F1;color:red'><u>FORMULARIO PARA CONTROL DE CALIDAD</u></span><span style='font-style:normal;font-weight:bold;font-size:12pt;font-family:CIDFont+F1;color:#0070c0'> </span></div>" +
            "</td></tr></table>" +
            "<table  width='100%' border='1' cellspacing='0' cellpadding='0' bordercolor='#000000'>" +
            "<tr>" +
            "<td>" +
            "<table width='100%' border='0' cellspacing='0' cellpadding='0'>" +
            "<tr>" +
            "<td>" +
            tablaDatpdf +
            "</td>" +
            "</tr>" +
            "<tr>" +
            "<td>" +
            tablaPreguntasImp +
            "</td>" +
            "</tr>" +
            "</table>" +
            "</td>" +
            "</tr>" +
            "</table>" +
            "                  <br><table style='width:100%' cellspacing='0' cellpadding='0'>                                                " +
            "                     <tr>                                                                                                       " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>**NOTA: Si su vehículo no esta equipado con un sistema o subsistema descrito en este control de calidad, este se marcará como OK</td>                              " +
            "                     </tr></table>                                                                                              " +
            "                  <br><table style='width:100%' cellspacing='0' cellpadding='0'>                                                " +
            "                     <tr>                                                                                                       " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'><center>F1RM_CAS</center></td>           " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +

            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'><center>F1RM_CL1</center></td>           " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +

            "                     </tr>                                                                                                      " +

            "                     <tr>                                                                                                       " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
            "                      <td style='text-align:center;font-family:Arial; font-size:7px'>--------------------------------------</td>" +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +

            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
            "                      <td style='text-align:center;font-family:Arial; font-size:7px'>--------------------------------------</td>" +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
            "                     </tr>                                                                                                      " +
            "                     <tr>                                                                                                       " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>Firma Asesor</td>                        " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +

            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>Firma aprobaci&oacute;n del cliente</td> " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +

            "                     </tr>                                                                                                      " +
            "                     <tr>                                                                                                       " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>Firma Asesor</td>                        " +
            "                        <td style='text-align:center;font-family:Arial; font-size:7px'>Firma aprobaci&oacute;n del cliente</td> " +
            "                     </tr>                                                                                                      " +
            "                  </table>                                                                                                      ";


        kendo.ui.progress($("#reporteCLCScreen"), false);
        var regresoHTML = htmlFormatoEV //+ "</tr></tr></table>";
        localStorage.setItem("tablaPDFCCA", regresoHTML);
        /*  kendo.ui.progress($("#reporteCLCScreen"), false);

 }, 2000);  */

    } catch (eimpev) {
        kendo.ui.progress($("#reporteCLCScreen"), false);
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>11115 ERROR</center>", eimpev);
    }
    /* alert("2"+inspeccionar(htmlFormatoEV));
    return htmlFormatoEV; */

}

function generarOrdenPDFCLCEN(strArchivo, tipoPDF) {
    try {
        document.getElementById("firmaGrafCLC").value = "";

        document.getElementById("titFirmaPregCLC").innerHTML = "Desea registrar la firma del cliente ?<br />";
        var htmlOrden11 = "";
        var camposEV = parametrosEntregaCompletoOTCLCL();
        var pathArchivoPDF = camposEV["descripcion-2"] + "\\" + strArchivo + "_" +
            localStorage.getItem("ls_ussucursal").toLocaleString() + "_" +
            localStorage.getItem("ls_usagencia").toLocaleString() + "\\" + strArchivo + "CC";
        var strFirma = "";
        var strFirmaAS = "";
        var strFirmaJT = "";
        var strBarcode = "";
        if (tipoPDF == "O") {
            /* kendo.ui.progress($("#reporteCLCScreen"), true);
            setTimeout(function () {  */
            if (localStorage.getItem("tablaPDFCCA") != undefined) {
                localStorage.removeItem("tablaPDFCCA");
            }
            //setInterval(function(){formatoImpPDF()},10000);
            /* -------------------------------------------------------------------*/
            htmlFormatoEV = "";
            try {
                var inforImprime = JSON.parse(localStorage.getItem("dataItem"));
                var repFirmaCCA = "";
                var repFirmaCC = "";
                /* kendo.ui.progress($("#reporteCLCScreen"), true);
                setTimeout(function () { */
                var tablaDatpdf = cargarTablaConC0(JSON.parse(localStorage.getItem("dataItem")));
                var tablaPreguntasImp = cargarTablaINPpdf(inforImprime);
                repFirmaCC = "F1RM_CL1";
                repFirmaCCA = "F1RM_CAS";
                var ima = localStorage.getItem("ls_url2").toLocaleString() + "/formato_mail/logo_flc.jpg";
                htmlFormatoEV = "<table width='100%' border='0' cellspacing='0' cellpadding='0'>" +
                    "<tr><td>" +
                    "<img src='" + ima + "' alt='FLC' width='115' height='70'>" +
                    "</td>" +
                    "<td style='text-align:center'>" +
                    "<div style='position:absolute;top:0.83in;left:3.34in;width:2.59in;line-height:0.15in;'><span style='font-style:normal;font-weight:bold;font-size:12pt;font-family:CIDFont+F1;color:red'><u>FORMULARIO PARA CONTROL DE CALIDAD</u></span><span style='font-style:normal;font-weight:bold;font-size:12pt;font-family:CIDFont+F1;color:#0070c0'> </span></div>" +
                    "</td></tr></table>" +
                    "<table  width='100%' border='1' cellspacing='0' cellpadding='0' bordercolor='#000000'>" +
                    "<tr>" +
                    "<td>" +
                    "<table width='100%' border='0' cellspacing='0' cellpadding='0'>" +
                    "<tr>" +
                    "<td>" +
                    tablaDatpdf +
                    "</td>" +
                    "</tr>" +
                    "<tr>" +
                    "<td>" +
                    tablaPreguntasImp +
                    "</td>" +
                    "</tr>" +
                    "</table>" +
                    "</td>" +
                    "</tr>" +
                    "</table>" +
                    "                  <br><table style='width:100%' cellspacing='0' cellpadding='0'>                                                " +
                    "                     <tr>                                                                                                       " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>**NOTA: Si su vehículo no esta equipado con un sistema o subsistema descrito en este control de calidad, este se marcará como OK</td>                              " +
                    "                     </tr></table>                                                                                              " +
                    "                  <br><table style='width:100%' cellspacing='0' cellpadding='0'>                                                " +
                    "                     <tr>                                                                                                       " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'><center>F1RM_CAS</center></td>           " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +

                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'><center>F1RM_CL1</center></td>           " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +

                    "                     </tr>                                                                                                      " +

                    "                     <tr>                                                                                                       " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
                    "                      <td style='text-align:center;font-family:Arial; font-size:7px'>--------------------------------------</td>" +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +

                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
                    "                      <td style='text-align:center;font-family:Arial; font-size:7px'>--------------------------------------</td>" +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
                    "                     </tr>                                                                                                      " +
                    "                     <tr>                                                                                                       " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>Firma Asesor</td>                        " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +

                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>Firma aprobaci&oacute;n del cliente</td> " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>&nbsp;</td>                              " +

                    "                     </tr>                                                                                                      " +
                    "                     <tr>                                                                                                       " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>Firma Asesor</td>                        " +
                    "                        <td style='text-align:center;font-family:Arial; font-size:7px'>Firma aprobaci&oacute;n del cliente</td> " +
                    "                     </tr>                                                                                                      " +
                    "                  </table>                                                                                                      ";


                kendo.ui.progress($("#reporteCLCScreen"), false);
                var regresoHTML = htmlFormatoEV //+ "</tr></tr></table>";
                localStorage.setItem("tablaPDFCCA", regresoHTML);
                /*  kendo.ui.progress($("#reporteCLCScreen"), false);

            }, 2000);  */

            } catch (eimpev) {
                kendo.ui.progress($("#reporteCLCScreen"), false);
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>11115 ERROR</center>", eimpev);
            }

            /*--------------------------------------------------------------------- */
            //formatoImpPDF();
            /* kendo.ui.progress($("#reporteCLCScreen"), false);
        }, 10000);  */
            htmlOrden11 = htmlFormatoEV; //localStorage.getItem("tablaPDFCCA");
            var canvas = document.createElement("canvas");
            JsBarcode(canvas, strArchivo, {format: "CODE39"});
            strBarcode = canvas.toDataURL("image/png");

            // firma base64
            strFirma = document.getElementById("firmaOTCLC").value;
            strFirmaAS = ""; //document.getElementById("firmaOTCLCA").value;
            strFirmaJT = "";
        } else {
            //htmlOrden = formatoRep01();
            //  pathArchivoPDF = camposEV["descripcion-2"] + "\\" + strArchivo + "\\" + "PRE_" + strArchivo;
            var arrPaths = urlFtpMayoOT().split("|");
            strBarcode = "ftp://" + arrPaths[1] + "/" + "|" + arrPaths[0] + "\\"; //"";
            pathArchivoPDF = camposEV["descripcion-2"] + "\\" + strArchivo + "\\" + "PRE_" +
                localStorage.getItem("ls_empresa").toLocaleString().substr(0, 5) + "_" +
                localStorage.getItem("ls_ussucursal").toLocaleString() + "_" +
                localStorage.getItem("ls_usagencia").toLocaleString() + "_" + strArchivo + "CC";
        }
        //    var strImgFija = strAutob64();
        var params = {
            "strArchivo": pathArchivoPDF, // strArchivo,
            "strHTML": htmlOrden11,
            "strBarcode": "", //strBarcode,
            "strFirma": strFirma,
            "strFirmaAS": "",
            "strFirmaJT": ""
        };

        //window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>pdf2</center>", inspeccionar(params));
        //var Url = "http://localhost:4044/Services/TL/Taller.svc/creaFirmasCC";
        var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/creaFirmasCC";

        $.ajax({
            url: Url,
            type: "POST",
            data: JSON.stringify(params),
            dataType: "json",
            headers: {
                'Content-Type': 'application/json;charset=UTF-8'
            },
            success: function (data) {

                try {
                    var dataJson = {
                        "0": data[0],
                        "1": data[1],
                        "2": data[2],
                        "3": data[3]
                    };
                    localStorage.setItem("firmasTotal", JSON.stringify(dataJson));

                    var data1 = data[1];
                    //r respMail = JSON.stringify(data);
                    //window.myalert("<center><i class=\"fa fa-paper-plane\"></i> GENERADO</center>", respMail);
                    if (data[0].includes("Succes")) {
                        var respPdf = "ok";
                        // window.myalert("<center><i class=\"fa fa-paper-plane\"></i> GENERADO</center>", "El PDF fue creado correctamente");
                        //window.myalert("<center><i class=\"fa fa-paper-plane\"></i> GENERADO</center>",inspeccionar(data));
                        try {
                            if (data1 !== null) {
                                var dataarr = data1.split("|");
                            } else {
                                var dataarr = "";
                            }

                            //window.myalert("<center><i class=\"fa fa-paper-plane\"></i> GENERADO</center>",inspeccionar(dataarr));

                        } catch (error) {
                            alert("firma" + error);
                        }

                        document.getElementById("firmaGrafCLC").value = data1;
                        //alert(document.getElementById("firmaGrafCC").value);
                        // Imagen de la firma solo si existe
                        if (document.getElementById("firmaGrafCLC").value.includes("CC_firm.") == true) {
                            document.getElementById("titFirmaPregCLC").innerHTML = "<center><b>La firma del cliente ya fue registrada<b></center>";
                            localStorage.setItem("mensa", document.getElementById("titFirmaPregCLC").innerText.trim());
                            document.getElementById("confirmFirmaOTCLC0").style.display = 'none';
                            document.getElementById("confirmFirmaOTCLC1").style.display = 'none';
                            document.getElementById("imgFirmaOTCLC").style.display = 'none';
                        }
                        // vacia el campo firma base64
                        document.getElementById("firmaOTCLC").value = "";
                        //adjuntoArchivosOT_2(document.getElementById("numOT_2").value);
                    } else {
                        //  window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>1 ERROR</center>", data);
                        return;
                    }
                } catch (e) {
                    window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>34 ERROR</center>", e);
                    return;
                }
            },
            error: function (err) {
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>35 ERROR</center>", JSON.stringify(err));
                return;
            }
        });

        //    kendo.ui.progress($("#lector_barrasScreen"), false);
        //    // precarga *********************************************************************************************
        //}, 2000);
    } catch (epdf) {
        alert("generarOrdenPDFCLCEN" + epdf);
        kendo.ui.progress($("#reporteCLCScreen"), false);
        document.getElementById("otadjuntos").value = "";
        return;
    }
}
function grabarFirmaDigital(strFirma){
    var datosot = JSON.parse(localStorage.getItem("dataItem"));
    if (datosot.tipo_trabajo == "RP" || datosot.tipo_trabajo == "RO") {
        var llave = datosot.nombre_sucursal+","+datosot.chasis+","+datosot.numero_orden+","+datosot.anio+","+ datosot.secuencia_orden+","+datosot.codigo_empresa+","+datosot.codigo_sucursal+","+datosot.codigo_taller;
        var params = {
            "llave": llave, 
            "imagen": strFirma.split(',')[1]
        };
        var Url = localStorage.getItem("ls_url1").toLocaleString() + "/Services/TL/Taller.svc/CrearFirmaDigital";
        $.ajax({
            url: Url,
            type: "POST",
            data: JSON.stringify(params),
            dataType: "json",
            headers: {
                'Content-Type': 'application/json;charset=UTF-8'
            },
            success: function (data) {

                try {
                    if (data.includes("Succes")) {
                        alert("Se guardo correctamente imagen");
                    } else {
                        return;
                    }
                } catch (e) {
                    window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>34 ERROR</center>", e);
                    return;
                }
            },
            error: function (err) {
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>35 ERROR</center>", JSON.stringify(err));
                return;
            }
        });
    } 
    
    
}
function parametrosEntregaCompletoOTCLCL() {
    try {
        var accResp = "";
        //http://192.168.1.50:8089/concesionario/Services/TG/Parametros.svc/ParametroEmpGet/6,;TG;APP_ENTREGAS;;PATH_ENTREGAS
        var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TG/Parametros.svc/ParametroEmpGet/6,;TG;APP_ENTREGAS;;PATH_ENTREGAS";
        // window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i> Url</center>", Url);
        var respPar;
        $.ajax({
            url: Url,
            type: "GET",
            dataType: "json",
            async: false,
            success: function (data) {
                try {
                    accResp = JSON.parse(data.ParametroEmpGetResult).tmpParamEmp;
                    respPar = accResp[0];
                } catch (e) {
                    window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>36 ERROR</center>", "parametrosEntregaCompletoOTCLCL" + inspeccionar(e));
                    respPar = "error";
                }
            },
            error: function (err) {
                window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>37 ERROR</center>", inspeccionar(err));
                respPar = "error";
            }
        });
        return respPar;
    } catch (f) {
        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>38 ERROR</center>", f);
        return "error";
    }
}

function enviarMailCC() {
    try {
        localStorage.removeItem("resultadoCC");
        var documento;
        var strArchivo = JSON.parse(localStorage.getItem("dataItem")).numero_orden;
        var camposEV = parametrosEntregaCompletoOTCLCL();
        var pathArchivoPDF = camposEV["descripcion-2"] + "\\" + strArchivo + "_" +
            localStorage.getItem("ls_ussucursal").toLocaleString() + "_" +
            localStorage.getItem("ls_usagencia").toLocaleString() + "\\" + strArchivo + "CC.pdf";
        var _mail = document.getElementById("mail_1").value;
        if ((_mail) && (_mail != "")) {
            try {
                if (document.getElementById("mail_1").value != "") {
                    var result = /^\w+([\.\+\-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,4})+$/.test(document.getElementById("mail_1").value);
                    if (result == false) {
                        document.getElementById("mail_1").focus();
                        document.getElementById("mail_1").style.borderColor = "red";
                    } else {
                        var diips = pathArchivoPDF;
                        for (var i = 0; i < diips.length; i++) {
                            diips = diips.replace(':', '!');
                            diips = diips.replace('/', '-');
                        }
                        documento = "7;" + "contacto@inpsercom.com" + ";;" + diips + ";" + _mail;
                        EnvioMailCC(documento, pathArchivoPDF);
                        //alert("02"+localStorage.getItem("resultadoCC"));
                        var envio = localStorage.getItem("resultadoCC");
                        //alert("01"+envio);
                        /*  if (envio.substring(0, 1) == "0") {
                             alert(envio.substring(2, envio.length - 2)) */
                        /*  mensajePrm("timeAlert", 0, "<img id='autoInpse2'  width='60' height='26' src='resources/Kia-logo.png'>",
                          "ERROR", "<span align='justify'>" + envio.substring(2, envio.length - 2) + "</b></span>", true, true); */
                        //}
                        //mens("Prefactura enviada al correo especificado", "mens");
                        //dialogOT_2.data("kendoDialog").close();
                        //kendo.mobile.application.navigate("components/AgregarVin/view.html");
                    }
                }
            } catch (f) {
                mens("Error validaci" + String.fromCharCode(243) + "n mail", "mens");
                return;
            }
        }
    } catch (error) {
        alert("aqu" + error);
    }
    //kendo.ui.progress($("#envioPrefacturaScreen"), false);
}

function EnvioMailCC(documento, pathArchivoPDF) {
    try {

        var strArchivo = JSON.parse(localStorage.getItem("dataItem"))
        if ((documento !== "") && (documento)) {
            var resultado = "";
            //alert("2");
            var llavemail = localStorage.getItem("ls_usulog").toLocaleString() + ";" +
                localStorage.getItem("ls_idempresa").toLocaleString() + ";" +
                localStorage.getItem("ls_ussucursal").toLocaleString() + ";" +
                localStorage.getItem("ls_usagencia").toLocaleString() + ";" +
                strArchivo.placa + ";" +
                strArchivo.chasis + ";" +
                strArchivo.numero_orden + "|18";

            var params = {
                "mail_receptor": document.getElementById("mail_1").value,
                "mail_copia": "",
                "observaciones": "Envio de documento control calidad",
                "adjuntos": pathArchivoPDF, // celular
                "imagenes": "",
                "videos": "",
                "otros": llavemail
            };

            //var Url = "http://localhost:4044/Services/TL/Taller.svc/tl06EnvioMailSet";
            var Url = localStorage.getItem("ls_url2").toLocaleString() + "/Services/TL/Taller.svc/tl06EnvioMailSet";

            $.ajax({
                url: Url,
                type: "POST",
                data: JSON.stringify(params),
                dataType: "json",
                headers: {
                    'Content-Type': 'application/json;charset=UTF-8'
                },
                success: function (data) {
                    try {
                        resultado = JSON.stringify(data);
                        localStorage.setItem("resultadoCC", JSON.stringify(data));
                        if (data.substring(0, 1) == "1") {
                            window.myalert("<center><i class=\"fa fa-paper-plane\"></i> ENVIADO</center>", "El mensaje fue enviado correctamente");
                        } else {
                            window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>33 ERROR</center>", data);
                            return;
                        }
                    } catch (e) {
                        window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>34 ERROR</center>", e);
                        return;
                    }
                },
                error: function (err) {
                    alert(JSON.stringify(err));
                    window.myalert("<center><i class=\"fa fa-exclamation-triangle\"></i>35 ERROR</center>", JSON.stringify(err));
                    return;
                }
            });
            return;
        }
    } catch (f) {
        mens("Error conexi" + String.fromCharCode(243) + "n servicio Veh" + String.fromCharCode(237) + "culo", "mens"); //kendo.ui.progress($("#envioPrefacturaScreen"), false); return;
    }
    //dialogOT_2.data("kendoDialog").close();
    //kendo.ui.progress($("#envioPrefacturaScreen"), false);
}

function onDeviceReady() {
    $(document).ready(function () {
        $("#tabstripCLC").kendoTabStrip({

            // Evento selecciona tab
            select: onSelecciona,

            animation: {
                open: {
                    effects: "fadeIn"
                }
            }
        });
    });

    //---------------------------------------------------------------------------

    navigator.splashscreen.hide();
    var app = new App();
    app.run();
}
